...
function npmLaunchOptions(): { readonly command: "npm"; readonly shell: boolean } {
  return {
    command: "npm",
...
export async function installRegistryScaffolderWithNpm(options: {
  ...
  const npmLaunch = npmLaunchOptions()
  await runPackagedCommand({
...
export async function withPackagedNpmServer<T>(
  ...
  try {
    try {
      child = spawn(npmLaunch.command, args, {
        cwd: options.appRoot,
        detached: true,
        env,
        shell: npmLaunch.shell,
