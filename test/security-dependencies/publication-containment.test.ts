...
describe("publication containment", () => {
	...
	it.each([
		...
		[
			"workflow path",
			(value: any) => (value.workflows.chart.path = ".github/workflows/x.yml"),
		],
		[
			"workflow state",
			(value: any) => (value.workflows.release.state = "active"),
		],
		[
			"non-completed run",
			(value: any) => (value.workflows.release.nonCompleted = 1),
		],
		["source run", (value: any) => (value.workflows.chart.sourceShaRuns = 1)],
		[
			"incomplete run count",
			(value: any) => (value.workflows.release.retrievedRuns = 451),
		],
		["default head", (value: any) => (value.defaultSha = "0".repeat(40))],
		[
			"inventory source",
			(value: any) => (value.inventory.sourceSha = "0".repeat(40)),
		],
		[
			"missing inventory package",
			(value: any) => value.inventory.packages.pop(),
		],
		[
			"extra inventory package",
			(value: any) => value.inventory.packages.push("extra"),
		],
		[
	...
	it("rejects duplicate package and incident identities before sorting", () => {
		...
		packageDuplicate.npm.packages[1] = structuredClone(
			packageDuplicate.npm.packages[0],
		);
		expect(() =>
	...
	it("collects the complete normalized proof through narrow transports", async () => {
		...
		expect(receipt.incidents.chart.jobs.every((job: any) => job.noOp)).toBe(
			true,
		);
		expect(() =>
	...
	it.each([
		...
		await expect(collectWithGithubOptions(options)).rejects.toThrow(
			/UNPROVABLE/u,
		);
	});
...
function githubTransport(options: any = {}) {
	...
	return async ({
		...
		if (path.includes("/git/matching-refs/tags/")) {
			return jsonResponse(
				options.tagRef === undefined ? [] : [{ ref: options.tagRef }],
			);
		}
...
async function collectWithGithubOptions(
	options: any,
	npmRequest = npmTransport([]),
) {
	const budget = createEvidenceBudget({
...
function npmTransport(
	...
	return async ({ url }: { url: string }) => {
		...
		if (path.endsWith("/0.8.22"))
			return npmResponse(404, "version not found: 0.8.22");
		const name = decodeURIComponent(path);
		const packument = { "dist-tags": { latest: "0.8.21" }, name };
		return npmResponse(
			200,
			options.decoratePackument?.(packument) ?? packument,
		);
	};
}
