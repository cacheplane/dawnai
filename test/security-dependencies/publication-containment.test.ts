...
import { createEvidenceBudget, createGitHubReader } from "../../scripts/security/github-evidence.mjs"
import {
  analyzeChartLog,
...
describe("publication containment", () => {
  it("accepts only the complete exact containment receipt", () => {
    expect(verifyPublicationSnapshot(validSnapshot(), { expectedDefaultSha: defaultSha })).toEqual(
      validSnapshot(),
    )
  })
  ...
  it.each([
    ...
    ["one target document present", (value: any) => (value.npm.packages[0].targetDocumentAbsent = false)],
    ["one attestation present", (value: any) => (value.npm.packages[0].targetAttestationAbsent = false)],
    ["one packument identity drift", (value: any) => (value.npm.packages[0].packumentName = "other")],
    ...
    mutate(snapshot)
    expect(() =>
      verifyPublicationSnapshot(snapshot, { expectedDefaultSha: defaultSha }),
  ...
  it("rejects duplicate package and incident identities before sorting", () => {
    ...
    expect(() =>
      verifyPublicationSnapshot(packageDuplicate, { expectedDefaultSha: defaultSha }),
    ).toThrow(/UNPROVABLE/u)

    const incidentDuplicate = structuredClone(validSnapshot())
    incidentDuplicate.incidents.release[1].id = incidentDuplicate.incidents.release[0].id
    expect(() =>
      verifyPublicationSnapshot(incidentDuplicate, { expectedDefaultSha: defaultSha }),
    ).toThrow(/UNPROVABLE/u)
  })
  ...
  it("collects the complete normalized proof through narrow transports", async () => {
    ...
    const github = createGitHubReader({ budget, repo: "cacheplane/dawnai", transport: githubTransport() })
    ...
    const receipt = await collectPublicationContainment({
      ...
      github,
      inventory: { packages: [...INVENTORY_PACKAGES], ref: "HEAD", sourceSha, version: "0.8.21" },
  ...
  it.each([
    ...
    [
      "package release tag",
      { release: { id: 44, name: "SDK 0.8.22", tag_name: "@dawn-ai/sdk@0.8.22" } },
    ],
    [
...
describe("bounded chart logs", () => {
  ...
  it.each([
    ...
    expect(() =>
      analyzeChartLog(log, "dawn-app 0.1.0 already published, skipping"),
    ).toThrow(/UNPROVABLE/u)
  })
})
...
function githubTransport(options: any = {}) {
  const releaseRuns = [
    run(31356780088, 260503756, ".github/workflows/release.yml", "Release", "cancelled", "3f4e3f9f62a3b48030a385bd0e7d720b8b26afdb"),
    run(31356940801, 260503756, ".github/workflows/release.yml", "Release", "cancelled", "b6adaa982b25adf5fac61733a13ac65320c70bcd"),
    run(31357014583, 260503756, ".github/workflows/release.yml", "Release", "cancelled", "cfa55478cf8e35dc8a00ae7041c0c12479fda2d9"),
  ]
  const chartRun = run(31356780047, 309127405, ".github/workflows/publish-chart.yml", "Publish Chart", "success", "3f4e3f9f62a3b48030a385bd0e7d720b8b26afdb")
  const firstSteps = [
  ...
  ].map(([number, name, conclusion]) => ({ conclusion, name, number, status: "completed" }))
  const chartJobs = [
    { conclusion: "success", id: 93357835324, name: "publish (dawn-sandbox-infra)", status: "completed", steps: [] },
    { conclusion: "success", id: 93357835326, name: "publish (dawn-app)", status: "completed", steps: [] },
  ]
  let mainReads = 0
  ...
  return async ({ responseType, url }: { responseType: string; url: string }) => {
    ...
    if (responseType === "text") {
      if (path.includes("93357835324")) return textResponse("dawn-sandbox-infra 0.1.2 already published, skipping\n")
      if (path.includes("93357835326")) return textResponse("dawn-app 0.1.0 already published, skipping\n")
      throw new Error(`unexpected log ${path}`)
    }
    if (path.endsWith("/commits/main")) {
      return jsonResponse({ sha: mainReads++ === 0 ? defaultSha : options.finalHead ?? defaultSha })
    }
    ...
    if (path.includes("/actions/artifacts?")) return jsonResponse({ artifacts: [], total_count: 0 })
    ...
    if (path.includes("/actions/runs/31356780088/jobs")) {
      return jsonResponse({
        jobs: [{ conclusion: "cancelled", id: 93357835724, name: "release", status: "completed", steps: firstSteps }],
        total_count: 1,
      })
    }
    if (path.includes("/actions/runs/31356940801/jobs")) return jsonResponse({ jobs: [], total_count: 0 })
    if (path.includes("/actions/runs/31357014583/jobs")) {
      return jsonResponse({
        jobs: [{ conclusion: "cancelled", id: 93359214159, name: "release", status: "completed", steps: [] }],
        total_count: 1,
      })
    }
...
async function collectWithGithubOptions(options: any) {
  const budget = createEvidenceBudget({
    maxBytes: 4 * 1024 * 1024,
    maxPages: 30,
    maxRecords: 1000,
...
function npmTransport(calls: string[]) {
  return async ({ url }: { url: string }) => {
    ...
    return npmResponse(200, { "dist-tags": { latest: "0.8.21" }, name })
  }
}

function run(id: number, workflowId: number, path: string, name: string, conclusion: string, headSha: string) {
  return {
    conclusion,
    event: "push",
    head_sha: headSha,
...
function textResponse(body: string) {
  return { body, bodyBytes: Buffer.byteLength(body) + 64, link: null, status: 200 }
}
