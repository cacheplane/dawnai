...
const auditExpectationPath = resolve(testDir, "fixtures/audit-upstream-boundaries.json")
const execFileAsync = promisify(execFile)
const defaultSha = "d2404dc7b138db151ae58f0b36788dfa08e2008e"
const auditLockfileSha256 = "1".repeat(64)
const expectedNumbers = [
  122, 124, 125, 160, 162, 163, 164, 170, 171, 172, 176, 178, 179, 180, 181, 191, 192, 193, 194,
  195, 196, 197, 198, 199, 200, 201, 204, 205, 206, 207, 208, 209, 210, 211, 212, 213, 214, 215,
...
describe("shared reconciliation input schemas", () => {
  ...
  it.each([
    ...
    ["lockfile digest", (receipt: any) => (receipt.lockfileSha256 = "e".repeat(64))],
  ...
  it.each(sharedSchemaMutationCorpus)(
    ...
    ({ mutateAudit, mutateDependabot }) => {
      ...
      const collectionAudit = schemaOutcome(() => validateAuditExpectation(auditCandidate))
      ...
      const reconciliationAudit = schemaOutcome(() => {
        const result = validateReconciliationFileInputs({
          ...
          auditReceiptBytes: canonicalJsonBytes(auditReceiptFromExpectation(auditForReceipt)),
          expectedObservationHeadSha: "d".repeat(40),
      ...
      const dependabotCandidate = mutateDependabot(schemaDependabotExpectation())
      const collectionDependabot = schemaOutcome(() =>
        validateDependabotExpectation(dependabotCandidate),
      )
      const dependabotInputs = reconciliationFileInputs(defaultSha, dependabotCandidate)
...
describe("Dependabot baseline identities", () => {
  it("loads the exact complete 59-alert fixture", async () => {
    ...
    expect(new Set(fixture.open.map((alert: any) => alert.number))).toHaveLength(59)
...
describe("complete open-set reader", () => {
  ...
  it.each([
    ...
    async (_name, phase, expectedCounts) => {
      ...
      await expect(
        reconcileDependabot({
          ...reconciliationEvidenceArgs(baseSha, mergeSha, reconciliationFixtureWithThree(baseSha)),
...
describe("merged-head reconciliation", () => {
  ...
  it("produces the complete bracketed canonical reconciliation receipt", async () => {
    ...
    expect(receipt.digests.inputs).toEqual({
      auditExpectationFixtureSha256: sha256Bytes(fileInputs.auditExpectationFixtureBytes),
      ...
      dependabotIdentitiesFixtureSha256: sha256Bytes(fileInputs.dependabotIdentitiesFixtureBytes),
    })
    expect(receipt.audit.digest).toBe(receipt.digests.inputs.auditReceiptSha256)
  ...
  it("produces and seals the complete reviewed 59-alert receipt", async () => {
    const fixture = validateDependabotExpectation(JSON.parse(await readFile(fixturePath, "utf8")))
    const reviewedBaseSha = defaultSha
    const reviewedHeadSha = "b".repeat(40)
    const mergeSha = "c".repeat(40)
    const mergedAt = "2026-08-10T18:00:00Z"
    const fixedNumbers = new Set([
      124, 125, 160, 162, 163, 164, 170, 171, 172, 176, 178, 179, 180, 181, 191, 192, 193, 194, 195,
      196, 197, 198, 199, 200, 201, 236,
    ])
    ...
    const open = fixture.open.filter((alert) => !fixedNumbers.has(alert.number))
    ...
    expect(receiptBytes.byteLength).toBeLessThanOrEqual(MAX_RECONCILIATION_RECEIPT_BYTES)
    const receiptGzipBase64 = encodeReconciliationReceiptGzipBase64(receiptBytes)
    ...
    expect(
      JSON.stringify({
        ...
        receiptGzipBase64: "A".repeat(Math.ceil(MAX_RECONCILIATION_RECEIPT_GZIP_BYTES / 3) * 4),
    ...
    try {
      ...
      expect(
        await readFile(resolve(root, "sealed", "dependency-security-reconciliation.json")),
  ...
  it("does not start a second verification-run query after the first returns at deadline", async () => {
    ...
    await expect(
      reconcileDependabot({
        ...fileInputs,
        collectPublication: async () => publicationSnapshot(observationSha, observationSha),
        expectedFixedNumbers: [2],
        expectedMergeSha: mergeSha,
        expectedObservationHeadSha: observationSha,
        expectedOpenNumbers: [1],
  ...
  it.each([
    ["missing", undefined],
    ["malformed", "not-a-sha"],
  ])("rejects a %s observation head before reading GitHub", async (_name, candidate) => {
    const audit = auditReceipt()
    let requests = 0
    await expect(
      reconcileDependabot({
  ...
  it.each([
    ...
    "collectPublication",
  ])("requires complete provenance input %s before GitHub I/O", async (missing) => {
    ...
    for (const key of missing === "all" ? keys : [missing]) delete evidence[key]
    let requests = 0
    await expect(
      reconcileDependabot({
        ...evidence,
  ...
  it.each([
    ...
    ["drift", { headAfter: "e".repeat(40), observationSha }, "DEFAULT_HEAD_DRIFT"],
  ...
  it("polls through partial fixed-alert convergence", async () => {
    ...
    const receipt = await reconcileDependabot({
      ...reconciliationFileInputs(baseSha, reconciliationFixtureWithThree(baseSha), mergeSha),
    ...
    expect(receipt.dependabot.fixed.map((alert: any) => alert.number)).toEqual([2, 3])
    expect(receipt.dependabot.open.map((alert: any) => alert.number)).toEqual([1])
  })
  ...
  it("keeps the default polling timer alive through attempt two", async () => {
    const moduleUrl = new URL("../../scripts/security/dependabot-reconcile.mjs", import.meta.url)
      .href
    const script = [
  ...
  it.each([
    ...
    const audit = auditReceipt()
    const expectedBaselineSha = "baselineSha" in options ? options.baselineSha : baseSha
    const github = createGitHubReader({
      budget: createEvidenceBudget({
        maxPages: 10,
        maxRecords: 100,
  ...
  it("rejects non-canonical audit bytes and schema drift without a free digest", async () => {
    ...
    await expect(
      reconcileDependabot({
        ...common,
        auditReceiptBytes: Buffer.concat([common.auditReceiptBytes, Buffer.from(" ")]),
        github: github(),
      }),
    ).rejects.toThrow(/UNPROVABLE/u)
...
describe("offline reconciliation receipt sealing", () => {
  ...
  it.each([
    ...
    ["wrong first merge parent", (value: any) => (value.pr.mergeParentShas[0] = "e".repeat(40))],
    [
    ...
    [
      ...
      (value: any) => (value.observation.completedAt = "2026-08-10T18:01:01.123Z"),
    ],
    ["missing run event", (value: any) => delete value.verificationRuns[0].event],
    ["wrong run event", (value: any) => (value.verificationRuns[0].event = "schedule")],
    ["wrong run branch", (value: any) => (value.verificationRuns[0].headBranch = "feature")],
    ["wrong run head", (value: any) => (value.verificationRuns[0].headSha = "e".repeat(40))],
    ["wrong run status", (value: any) => (value.verificationRuns[0].status = "queued")],
    ["wrong run conclusion", (value: any) => (value.verificationRuns[0].conclusion = "failure")],
    [
      "duplicate run ID",
      (value: any) => (value.verificationRuns[1].runId = value.verificationRuns[0].runId),
    ],
    ["unsorted runs", (value: any) => value.verificationRuns.reverse()],
    ["missing workflow/head product", (value: any) => value.verificationRuns.pop()],
    ["publication head mismatch", (value: any) => (value.observationHead = "e".repeat(40))],
    ["audit digest mutation", (value: any) => (value.audit.digest = "0".repeat(64))],
    [
      "audit input digest mutation",
      (value: any) => (value.digests.inputs.auditReceiptSha256 = "0".repeat(64)),
    ],
    [
      "fixed output digest mutation",
      (value: any) => (value.digests.outputs.fixedAlertsSha256 = "0".repeat(64)),
    ],
    [
      "open A output digest mutation",
      (value: any) => (value.digests.outputs.openSnapshotASha256 = "0".repeat(64)),
    ],
    [
      "open B output digest mutation",
      (value: any) => (value.digests.outputs.openSnapshotBSha256 = "0".repeat(64)),
    ],
    [
      "publication-before digest mutation",
      (value: any) => (value.digests.outputs.publicationBeforeSha256 = "0".repeat(64)),
    ],
    [
      "publication-after digest mutation",
      (value: any) => (value.digests.outputs.publicationAfterSha256 = "0".repeat(64)),
    ],
  ...
  for (const section of ["inputs", "outputs"] as const) {
    ...
    it.each(keys)(`rejects a missing or malformed ${section} digest %s`, (key) => {
  ...
  it("writes the exact receipt and a separate canonical uploader manifest", async () => {
    ...
    try {
      ...
      expect(await readFile(result.manifestPath)).toEqual(canonicalJsonBytes(result.manifest))
  ...
  it.each([
    ["non-base64 input", (args: any) => ({ ...args, receiptGzipBase64: "***" })],
    [
      "non-gzip input",
      (args: any) => ({
        ...args,
    ...
    [
      ...
      (args: any) => ({
        ...args,
        receiptGzipBase64: Buffer.alloc(MAX_RECONCILIATION_RECEIPT_GZIP_BYTES + 1).toString(
          "base64",
        ),
      }),
    ],
    ["wrong digest", (args: any) => ({ ...args, receiptSha256: "0".repeat(64) })],
    ["wrong main", (args: any) => ({ ...args, expectedMainSha: "d".repeat(40) })],
    ["wrong merge", (args: any) => ({ ...args, expectedMergeSha: "d".repeat(40) })],
    ["wrong PR", (args: any) => ({ ...args, expectedPrNumber: 43 })],
    ["wrong base", (args: any) => ({ ...args, expectedReviewedBaseSha: "d".repeat(40) })],
    ["wrong head", (args: any) => ({ ...args, expectedReviewedHeadSha: "d".repeat(40) })],
    ["wrong repository", (args: any) => ({ ...args, expectedRepository: "cacheplane/other" })],
  ...
  it("rejects a gzip payload that expands beyond the receipt bound", async () => {
    ...
    try {
      ...
      await expect(
        sealReconciliationReceipt({
          ...sealArguments(root),
          receiptGzipBase64: gzipSync(expanded, { level: 9 }).toString("base64"),
        }),
  ...
  it("rejects an oversized receipt before gzip encoding", () => {
    expect(() =>
      encodeReconciliationReceiptGzipBase64(Buffer.alloc(MAX_RECONCILIATION_RECEIPT_BYTES + 1)),
  ...
  it("rejects non-canonical JSON and invalid UTF-8 even with matching digests", async () => {
    ...
    try {
      ...
      await expect(
        sealReconciliationReceipt({
          ...sealArguments(root),
          receiptGzipBase64: encodeReconciliationReceiptGzipBase64(nonCanonical),
          receiptSha256: createHash("sha256").update(nonCanonical).digest("hex"),
        }),
      ...
      await expect(
        sealReconciliationReceipt({
          ...sealArguments(root),
          receiptGzipBase64: encodeReconciliationReceiptGzipBase64(invalidUtf8),
  ...
  it("rejects output escape, an existing directory, and a symlink target", async () => {
    ...
    try {
      ...
      await expect(sealReconciliationReceipt(sealArguments(root))).rejects.toThrow(
        /UNPROVABLE: INVALID_RECEIPT_OUTPUT/u,
      )
      ...
      await expect(readFile(resolve(root, "sealed"), "utf8")).resolves.toBe("preserve-me")
      ...
      await expect(
        sealReconciliationReceipt({
          ...
          writerCheckpoint: async (name: string, context: any) => {
            if (name === "beforeClose") {
              await writeFile(resolve(context.outputDirectory, "extra.txt"), "extra")
            }
          },
        }),
      ).rejects.toThrow(/UNPROVABLE: INVALID_RECEIPT_OUTPUT/u)
        ...
        sealReconciliationReceipt({
          ...
          writerCheckpoint: async (name: string, context: any) => {
            if (name === "beforeMkdir") {
              ...
              await writeFile(resolve(context.outputDirectory, "preserve.txt"), "preserve-me")
            }
          },
        }),
      ).rejects.toThrow(/UNPROVABLE: INVALID_RECEIPT_OUTPUT/u)
      await expect(readFile(resolve(root, "sealed/preserve.txt"), "utf8")).resolves.toBe(
        "preserve-me",
      )
    } finally {
      await rm(root, { force: true, recursive: true })
  ...
  it("enforces the trusted-root contract and accepts only a protected sticky ancestor", async () => {
    ...
    try {
      ...
      await expect(sealReconciliationReceipt(sealArguments(rootLink))).rejects.toThrow(
        /UNPROVABLE: INVALID_RECEIPT_OUTPUT/u,
      )
      await rm(rootLink)
      ...
      await expect(sealReconciliationReceipt(sealArguments(root))).resolves.toBeTruthy()
    } finally {
  ...
  it.each([0o755, 0o1777])(
    ...
    async (mode) => {
      const module = await import("../../scripts/security/dependabot-reconcile.mjs")
      expect(() => module.validateTrustedAncestorPolicy(mode, 777, 501, 501)).toThrow(
        /UNPROVABLE: INVALID_RECEIPT_OUTPUT/u,
      )
    },
  )
...
function auditReceiptFromExpectation(expectation: any, sourceSha = "d".repeat(40)) {
...
function reconcileTransport(options: any) {
  ...
  return async ({ url }: { url: string }) => {
    ...
    if (path.endsWith("/commits/main")) {
      ...
      const sha = headReads++ === 0 ? expected : (options.headAfter ?? expected)
        ...
        workflow_runs: [
          {
            ...
            head_sha: options.verificationHead ?? api.searchParams.get("head_sha"),
    ...
    if (path.includes("/dependabot/alerts?state=open")) {
      ...
      return jsonResponse(
        options.pendingAttempts === undefined && openReads === 1 ? openBefore : openAfter,
      )
    }
    if (path.endsWith("/dependabot/alerts/2")) {
      ...
      if (fixedReads++ < (options.pendingAttempts ?? 0)) return jsonResponse(alert)
      alert.state = "fixed"
    ...
    if (path.endsWith("/dependabot/alerts/3")) {
      ...
      if (options.partialConvergence && fixedReads++ === 1) return jsonResponse(alert)
      alert.state = "fixed"
    ...
    github: {
      list: async (path: string) => {
        if (path.startsWith("actions/workflows/")) {
          const [encodedWorkflowPath] = path.slice("actions/workflows/".length).split("/runs?")
          if (encodedWorkflowPath === undefined) {
            throw new Error("missing workflow path")
          }
          const headSha = new URL(`https://example.test/?${path.split("?")[1]}`).searchParams.get(
            "head_sha",
          )
          return [
            {
      ...
      object: async (path: string) => {
        ...
        if (match !== null) {
          ...
          if (options.phase === "fixed" && fixedCalls === 1) options.advanceDeadline()
          return alert
        }
