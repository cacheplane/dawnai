...
function processIsRunning(pid: number): boolean {
  try {
    ...
    if (error !== null && typeof error === "object" && Reflect.get(error, "code") === "ESRCH") {
      return false
    }
    throw error
  }
...
describe("shared evidence budget clock", () => {
  ...
  it.each([
  ...
  ])("rejects a %s reserve sample before transport", async (_name, createSamples) => {
    ...
    await expect(reader.object("commits/main")).rejects.toThrow(/UNPROVABLE: INVALID_CLOCK/u)
    expect(calls).toBe(0)
  })
...
describe("GitHub evidence pagination", () => {
  it("rejects an adversarial Link prefix within a bounded worker deadline", async () => {
    const worker = new Worker(
				...
				`,
      {
        ...
        workerData: {
          ...
          moduleUrl: new URL("../../scripts/security/github-evidence.mjs", import.meta.url).href,
        },
      },
    )
    try {
      ...
      expect(result).toEqual({
        kind: "rejected",
        messages: ["UNPROVABLE: MALFORMED_LINK", "UNPROVABLE: MALFORMED_LINK"],
      })
    } finally {
  ...
  it.each([
    ["before cursor", `<${alertsUrl}&before=opaque>; rel="next"`, alertsUrl, new Set([alertsUrl])],
    [
  ...
  it("fails when a response arrives at the exact deadline", async () => {
    ...
    await expect(reader.object("commits/main")).rejects.toThrow(/UNPROVABLE: EVIDENCE_TIMEOUT/u)
  })

  it("charges response headers against the aggregate byte budget", async () => {
    const raw =
      "HTTP/2.0 200 OK\nContent-Type: application/json\nX-Padding: " + "x".repeat(128) + "\n\n{}"
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async ({ task }) => {
      ...
      try {
        ...
        await expect(processResult).rejects.toThrow(/UNPROVABLE: PROCESS_TIMEOUT/u)
