...
describe("audit expectation fixtures", () => {
  it("binds the exact reviewed full and production multisets", async () => {
    ...
    assertExactAuditTuples(expectation.production.records, expectedProductionAuditTuples)
  ...
  it("detects tuple drift even when counts and severity totals are unchanged", async () => {
    ...
    expect(countSeverity(mutated)).toEqual(countSeverity(expectation.full.records))
    expect(() => assertExactAuditTuples(mutated, expectedFullAuditTuples)).toThrow()
  })
  ...
  it.each([
    ...
    for (const expectedMode of [expectation.full, expectation.production]) {
      expect(() =>
        normalizeAuditDocument(mutate(auditDocument(expectedMode)), expectedMode, 1),
...
describe("dependency audit normalization", () => {
  ...
  it("normalizes every affected version grouped under one advisory", () => {
    ...
    expect(normalizeAuditDocument(document, expectation, 1).records).toEqual(expectation.records)
  })

  it("accepts exit zero only for an exact empty mode", () => {
    const expectation = mode([])
    expect(normalizeAuditDocument(auditDocument(expectation), expectation, 0).status).toBe("clean")
...
describe("fixed audit subprocess contract", () => {
  ...
  ])("rejects an invalid %s clock before starting a subprocess", async (_name, now) => {
    let calls = 0
    await expect(
      collectAuditEvidence({
        ...auditProvenance,
...
describe("dependency evidence CLI", () => {
  it("formats live clock samples as canonical UTC seconds and rejects out-of-range dates", () => {
    expect(formatUtcSeconds(Date.parse("2026-08-10T18:00:00Z") + 347)).toBe("2026-08-10T18:00:00Z")
    expect(() => formatUtcSeconds(Number.MAX_SAFE_INTEGER)).toThrow(/UNPROVABLE: INVALID_CLOCK/u)
  })
  ...
  it("assembles a canonical baseline only from matching provenance and exact identities", async () => {
    const fixture = await import("../../scripts/security/dependabot-reconcile.mjs").then(
      ({ loadDependabotExpectation }) =>
        loadDependabotExpectation(resolve(testDir, "fixtures/dependabot-baseline.json"), {
          root: repositoryRoot,
        }),
    )
  ...
  it("writes one canonical audit receipt and prints only a bounded summary", async () => {
    ...
    try {
      ...
      await runDependencyEvidenceCli({
        ...
        now: () => capturedAt,
        runProcess: async () => {
          const modeExpectation = calls++ === 0 ? expectation.full : expectation.production
          return {
            exitCode: 1,
            stderr: "npm_token_secret",
      ...
      const result = await runDependencyEvidenceCli({
        ...
        runProcess: async () => ({
          ...
          stdout: JSON.stringify(
            auditDocument(calls++ === 0 ? expectation.full : expectation.production),
          ),
        }),
        writeStdout: () => {},
      })

      expect(result.output).toBe(resolve(await realpath(canonicalParent), "audit.json"))
      await expect(readFile(result.output, "utf8").then(JSON.parse)).resolves.toMatchObject({
        kind: "pnpm-audit",
        schemaVersion: 2,
      })
    } finally {
  ...
  it("pins a baseline inventory read to the resolved source when symbolic HEAD moves", async () => {
    const temporary = await mkdtemp(resolve(testDir, ".dependency-baseline-inventory-pin-"))
    try {
      const sourceSha = "a".repeat(40)
      ...
      await expect(
        runDependencyEvidenceCli({
          argv: [
            ...
            sourceSha,
            "--expected-default-sha",
            "d2404dc7b138db151ae58f0b36788dfa08e2008e",
            "--current-version",
            "0.8.21",
            "--target-version",
            "0.8.22",
  ...
  it("dispatches reconcile with exact typed identities and bounded file bytes", async () => {
    const temporary = await mkdtemp(resolve(testDir, ".dependency-reconcile-cli-"))
    try {
      const dependabotFixture = resolve(testDir, "fixtures/dependabot-baseline.json")
      ...
      const reviewedBaseSha = "d2404dc7b138db151ae58f0b36788dfa08e2008e"
      ...
      expect(request.auditExpectationFixtureBytes).toEqual(await readFile(auditExpectation))
      expect(request.auditReceiptBytes).toEqual(await readFile(auditReceipt))
      expect(request.baselineReceiptBytes).toEqual(await readFile(baselineReceipt))
      expect(request.dependabotIdentitiesFixtureBytes).toEqual(await readFile(dependabotFixture))
      ...
      expect(stdout.join("")).toMatch(/reconciliation receipt .*fixed=1 open=1/u)
  ...
  it.each([
    ...
    ["max-attempts", "60"],
  ])("rejects a noncanonical reconcile %s before local or remote I/O", async (key, value) => {
    let gitCalls = 0
...
function assertExactAuditTuples(
  ...
  expect(
    records
      .map((record) => [record.package, record.version, record.ghsa, record.severity] as const)
...
function reconcileCliArguments(overrides: Record<string, string> = {}) {
  const values = {
    ...
    "audit-receipt": resolve(testDir, "fixtures/audit-provider-utils-only.json"),
    "baseline-receipt": resolve(testDir, "fixtures/audit-provider-utils-only.json"),
    "current-version": "0.8.21",
    "expected-fixed": "2",
    "expected-identities": resolve(testDir, "fixtures/dependabot-baseline.json"),
    "expected-open": "1",
    "inventory-ref": "HEAD",
    "max-attempts": "61",
  ...
  return ["reconcile", ...Object.entries(values).flatMap(([key, item]) => [`--${key}`, item])]
}
...
function sealableReconciliationReceipt(baselineOpen: any[]) {
  ...
  const publication = publicationReceiptFixture(observationHead, observationHead)
