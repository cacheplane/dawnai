...
**Goal:** Extract Dawn's author-facing contract from `@dawn-ai/langgraph` into a new backend-neutral `@dawn-ai/sdk` package, and replace the `route.ts` + `workflow.ts`/`graph.ts` convention with a single `index.ts` per route whose kind is inferred from named exports.

**Architecture:** New `@dawn-ai/sdk` owns backend-neutral types (`defineTool`, `ToolDefinition`, `ToolContext`, `RuntimeContext`, `RuntimeTool`, `ToolRegistry`, `RouteConfig`, `RouteKind`). `@dawn-ai/langgraph` slims to the LangGraph execution adapter (`normalizeRouteModule`, `RouteModule`, `defineEntry`) and re-exports SDK types. `@dawn-ai/core` discovery scans for `index.ts` files, inferring route kind from whether `workflow` or `graph` is exported. `@dawn-ai/cli` collapses its execution path to a single lane.
...
**Modified files:**
- `packages/langgraph/package.json` — add `@dawn-ai/sdk` dep, drop subpath exports
- `packages/langgraph/src/index.ts` — re-export from `@dawn-ai/sdk`
...
- `packages/core/package.json` — add `@dawn-ai/sdk` dep
...
## Task 1: Create `@dawn-ai/sdk` package scaffold
...
Create `packages/sdk/vitest.config.ts` with the same shape. Aliases must self-resolve `@dawn-ai/sdk` to `./src/index.ts` and exclude `*.contract.ts` from test runs.
...
Run: `pnpm install`
Expected: pnpm resolves `@dawn-ai/sdk` in the workspace graph. No errors.
...
Run: `pnpm --filter @dawn-ai/sdk typecheck`
Expected: PASS.

- [ ] **Step 10: Run sdk lint**

Run: `pnpm --filter @dawn-ai/sdk lint`
Expected: PASS.
...
git commit -m "feat(sdk): scaffold @dawn-ai/sdk package"
```

---

## Task 2: Move tool authoring to `@dawn-ai/sdk`
...
Copy the file. Change the import from `@dawn-ai/langgraph` to `@dawn-ai/sdk`:

```ts
import { defineTool } from "@dawn-ai/sdk"
```
...
Copy the file. Change import from `@dawn-ai/langgraph` to `@dawn-ai/sdk`.
...
Run: `pnpm --filter @dawn-ai/sdk typecheck`
Expected: PASS — `tool-context.contract.ts` type-checks against `@dawn-ai/sdk` exports.

- [ ] **Step 8: Add `@dawn-ai/sdk` as a dependency of `@dawn-ai/langgraph`**
...
```ts
export { defineTool, type ToolContext, type ToolDefinition } from "@dawn-ai/sdk"
```
...
Run: `pnpm --filter @dawn-ai/langgraph lint`
Expected: PASS.
...
git commit -m "refactor(sdk): move defineTool and ToolContext to @dawn-ai/sdk"
```

---

## Task 3: Move runtime context to `@dawn-ai/sdk`
...
Run: `pnpm --filter @dawn-ai/sdk test && pnpm --filter @dawn-ai/sdk typecheck`
Expected: PASS.
...
Run: `pnpm --filter @dawn-ai/langgraph test && pnpm --filter @dawn-ai/langgraph typecheck && pnpm --filter @dawn-ai/langgraph lint`
Expected: PASS.
...
git commit -m "refactor(sdk): move RuntimeContext and RuntimeTool to @dawn-ai/sdk, add ToolRegistry"
```

---

## Task 4: Move `RouteConfig` and `RouteKind` to `@dawn-ai/sdk`, delete `defineRoute`
...
Run: `pnpm --filter @dawn-ai/sdk test && pnpm --filter @dawn-ai/sdk typecheck && pnpm --filter @dawn-ai/sdk lint`
Expected: PASS.
...
Run: `pnpm --filter @dawn-ai/langgraph test && pnpm --filter @dawn-ai/langgraph typecheck && pnpm --filter @dawn-ai/langgraph lint && pnpm --filter @dawn-ai/langgraph build`
Expected: PASS. `define-route.test.ts` gone; `route-module.test.ts` updated for new error strings; imports from `@dawn-ai/sdk` resolve.

- [ ] **Step 11: Commit**

```bash
git add packages/sdk packages/langgraph
git commit -m "refactor(sdk): move RouteConfig and RouteKind to @dawn-ai/sdk, drop defineRoute"
```

---

## Task 5: Update `@dawn-ai/core` types to use `RouteKind` from sdk

**Files:**
- Modify: `packages/core/package.json` — add `@dawn-ai/sdk` dep
...
- [ ] **Step 1: Add `@dawn-ai/sdk` to `packages/core/package.json` dependencies**
...
git commit -m "refactor(core): adopt RouteKind from @dawn-ai/sdk, drop bound entry fields"
```

---

## Task 6: Rewrite `@dawn-ai/core` discovery for `index.ts`-based model
...
If the only hit is inside `@dawn-ai/core` itself (and its own test that we rewrote), remove the export and the function. If there are external callers, the shim stays.
...
Run: `pnpm --filter @dawn-ai/core typecheck && pnpm --filter @dawn-ai/core lint`
Expected: PASS.
...
## Task 8: Simplify `@dawn-ai/cli` runtime — single execution path
...
import { findDawnApp } from "@dawn-ai/core"
import { normalizeRouteModule } from "@dawn-ai/langgraph"
...
Run: `pnpm --filter @dawn-ai/devkit lint`
Expected: PASS.
...
- [ ] **Step 3: Update `@dawn-ai/sdk` package README if exists**
