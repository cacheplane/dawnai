...
**Architecture:** Change `tool-discovery.ts` to accept bare function exports and derive names from filenames. Remove `defineTool`, `ToolDefinition`, and `ToolContext` from `@dawn/sdk` and `@dawn/langgraph`. Update template and test fixtures.
...
Run: `pnpm --filter @dawn/cli exec tsc -p tsconfig.json --noEmit`
...
### Task 3: Remove `defineTool` from `@dawn/sdk` and `@dawn/langgraph`
...
Run: `pnpm --filter @dawn/langgraph exec tsc -p tsconfig.json --noEmit`
Expected: PASS
