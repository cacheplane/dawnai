...
| `packages/vite-plugin/package.json` | **Modified.** Add `@dawn-ai/core` dependency. |
...
### Task 1: Add TypeScript dependency to `@dawn-ai/core`
...
git commit -m "chore: add typescript dependency to @dawn-ai/core for tool type extraction"
```
...
- [ ] **Step 1: Add `@dawn-ai/core` dependency to vite-plugin**

In `packages/vite-plugin/package.json`, add `@dawn-ai/core` to dependencies:
...
- Verify that `@dawn-ai/core`'s `typescript` dependency doesn't cause version conflicts
