...
**Architecture:** Use a `pnpm` workspace monorepo coordinated by `turbo`, with TypeScript project references and `tsc -b` for package builds. Keep Dawn-specific behavior in `@dawn/core` and compose it into `@dawn/cli` and `create-dawn-app`, while leaving runtime-facing route authoring thin and native-first.
...
Create root script references so `pnpm --filter @dawn/config-typescript typecheck` and `pnpm --filter @dawn/config-biome check` fail before the packages exist.

- [ ] **Step 2: Run checks to verify they fail**

Run: `pnpm --filter @dawn/config-typescript typecheck`
Expected: FAIL because package is missing.

Run: `pnpm --filter @dawn/config-biome check`
Expected: FAIL because package is missing.

- [ ] **Step 3: Write the minimal shared config packages**

Create `@dawn/config-typescript` with JSON config exports covering:
- base TypeScript settings
- library build settings with declarations
- Node package settings
- Next.js app settings

Create `@dawn/config-biome` with a reusable `biome.json` that enables formatting and linting defaults suitable for the monorepo.
...
### Task 3: `@dawn/core` Discovery and Typegen
...
Run: `pnpm --filter @dawn/core test -- --run packages/core/test/discover-routes.test.ts`
Expected: FAIL because `@dawn/core` does not exist yet.
...
Run: `pnpm --filter @dawn/core test -- --run packages/core/test/render-route-types.test.ts`
Expected: FAIL because the typegen module does not exist yet.

- [ ] **Step 5: Write the minimal implementation**

Implement `@dawn/core` with:
...
Run: `pnpm --filter @dawn/core typecheck`
Expected: PASS.

Run: `pnpm --filter @dawn/core build`
Expected: PASS.
...
### Task 4: `@dawn/langgraph` Integration Contracts
...
- the package exposes types and helpers that `@dawn/core` and the template app can consume without inventing a second runtime

- [ ] **Step 2: Run tests to verify they fail**

Run: `pnpm --filter @dawn/langgraph test -- --run packages/langgraph/test/define-entry.test.ts`
...
Run: `pnpm --filter @dawn/langgraph build`
Expected: PASS.
...
Run: `pnpm --filter @dawn/cli test -- --run packages/cli/test/routes-command.test.ts`
Expected: FAIL because the CLI package does not exist yet.

- [ ] **Step 3: Write the minimal implementation**

Implement the CLI with Commander and compose `@dawn/core` helpers. Support:
...
Run: `pnpm --filter @dawn/cli build`
Expected: PASS.
...
### Task 6: `@dawn/devkit` and `create-dawn-app`
...
Implement `@dawn/devkit` template copying helpers and `create-dawn-app` CLI entrypoint. Support one template only: `basic`.
...
Create the package entry and root workspace references so `pnpm --filter @dawn/web build` fails before the app exists.
