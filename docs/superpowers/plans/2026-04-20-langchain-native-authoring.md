...
**Goal:** Add LangChain LCEL runnables as a second execution backend, proving Dawn is a meta-framework. Introduce `BackendAdapter` interface, `@dawn-ai/langchain` package, `@dawn-ai/vite-plugin` for build-time schema inference, and SSE streaming in `dawn dev`.

**Architecture:** `@dawn-ai/sdk` gains a `BackendAdapter` type. The CLI owns route discovery for all kinds (`graph`, `workflow`, `chain`). `@dawn-ai/langchain` provides a thin adapter that handles `.invoke()` / `.stream()` on LCEL runnables, auto-binds Dawn-discovered tools, and runs a Dawn-owned tool execution loop. `@dawn-ai/vite-plugin` infers Zod schemas from TypeScript function signatures at build time. Streaming uses NDJSON for `dawn run` and SSE for `dawn dev`.
...
|---------|------|----------------|
| `@dawn-ai/langchain` | `packages/langchain/` | BackendAdapter for `chain` routes, tool converter |
| `@dawn-ai/vite-plugin` | `packages/vite-plugin/` | Build-time TS→Zod schema inference for tools |
...
| `packages/cli/src/lib/runtime/load-route-kind.ts` | CLI-owned normalization replacing `@dawn-ai/langgraph` import |
...
| `packages/cli/package.json` | Add `@dawn-ai/langchain` dependency |
| `packages/cli/vitest.config.ts` | Add `@dawn-ai/langchain` alias |
...
### Task 1: Expand `RouteKind` and `BackendAdapter` in `@dawn-ai/sdk`
...
Run: `pnpm --filter @dawn-ai/sdk exec tsc --noEmit`
Expected: PASS
...
git commit -m "feat: add BackendAdapter interface and chain route kind to @dawn-ai/sdk"
```
...
Run: `pnpm --filter @dawn-ai/core exec vitest run test/discover-routes-chain.test.ts`
...
Run: `pnpm --filter @dawn-ai/cli exec tsc -p tsconfig.json`
Expected: PASS
...
### Task 6: Scaffold `@dawn-ai/langchain` package
...
git commit -m "feat: scaffold @dawn-ai/langchain package with peerDep on @langchain/core"
```
...
Run: `pnpm --filter @dawn-ai/langchain exec vitest run test/tool-converter.test.ts`
...
Run: `pnpm --filter @dawn-ai/langchain exec vitest run test/tool-loop.test.ts`
Expected: FAIL
...
Run: `pnpm --filter @dawn-ai/langchain exec vitest run test/chain-adapter.test.ts`
Expected: FAIL
...
git commit -m "feat: implement BackendAdapter for chain routes in @dawn-ai/langchain"
```

---

### Task 10: Scaffold `@dawn-ai/vite-plugin` package
...
git commit -m "feat: scaffold @dawn-ai/vite-plugin package for build-time schema inference"
```
...
Run: `pnpm --filter @dawn-ai/vite-plugin exec vitest run test/type-extractor.test.ts`
Expected: FAIL
...
Run: `pnpm --filter @dawn-ai/vite-plugin exec vitest run test/zod-generator.test.ts`
Expected: FAIL
...
Run: `pnpm --filter @dawn-ai/vite-plugin exec vitest run test/jsdoc-extractor.test.ts`
Expected: FAIL
...
Run: `pnpm --filter @dawn-ai/vite-plugin exec vitest run test/plugin.test.ts`
Expected: FAIL
...
Run: `pnpm --filter @dawn-ai/vite-plugin test`
Expected: PASS
...
### Task 15: Wire `@dawn-ai/langgraph` as a BackendAdapter
...
Run: `pnpm --filter @dawn-ai/langgraph exec tsc --noEmit`
Expected: PASS
...
### Task 18: Add `@dawn-ai/langchain` to CLI dependencies and wire adapter

**Files:**
- Modify: `packages/cli/package.json`
- Modify: `packages/cli/vitest.config.ts`

- [ ] **Step 1: Add `@dawn-ai/langchain` to CLI `package.json`**
...
Add the `@dawn-ai/langchain` alias:
...
git commit -m "feat: add @dawn-ai/langchain dependency to CLI package"
```
...
Run: `pnpm --filter @dawn-ai/cli exec vitest run test/chain-route.test.ts`
Expected: PASS
...
In `docs/next-iterations-roadmap.md`, update Phase 2 section to reflect implementation status. Add a note that Phase 2 core is implemented with `@dawn-ai/langchain`, `@dawn-ai/vite-plugin`, BackendAdapter interface, SSE streaming, and chain route support.
