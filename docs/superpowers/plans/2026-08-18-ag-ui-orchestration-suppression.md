...
5. `settle(interruptToolCallId?)` — terminal: every remaining **unresolved** candidate becomes **fallback**, EXCEPT one whose id equals `interruptToolCallId`, which is **dropped** (frames discarded). Then drain everything in order and return it, and clear all tracking state. `settle` is idempotent and leaves the ledger empty, so anything routed afterwards passes straight through unheld. It deliberately does NOT set the disabled flag: a candidate opened after a settle would still be correct, because every terminal path settles immediately before its terminal event. (Do not "fix" this by disabling suppression in `settle` — that would silently degrade suppression for the rest of a run if the outbound generator ever streams events while parked.) The `interruptToolCallId` reaches the ledger from the AG-UI `Interrupt.toolCallId`, which `toAguiInterrupt` sources from the Dawn envelope's `callId` (Dawn's name for the same model tool-call id) with `toolCallId` taking precedence when both are present.
...
5. **defers unrelated events behind an unresolved candidate and releases them in order** — `onToolCall("call_w", "writeTodos", …)` → `[]`; `onPassthrough(text("a"))` → `[]`; `onToolCall("call_s", "searchCorpus", frames("call_s","searchCorpus"))` → `[]`; then the correlated activity returns `[text("a"), …call_s frames, activity]` — **source order**: the activity is routed after the events that arrived before it, so it emits last. (Rule 6 governs; an activity never jumps ahead of earlier deferred events.)
...
10. **parallel candidates drain only a resolved prefix** — track `call_w1` then `call_w2`; correlate `call_w2` FIRST → returns `[]` (blocked by w1); correlate `call_w1` → returns `[activity2, activity1]` — **source order** of the two activity chunks, since w2's activity was routed first. The point of the case is that nothing emits until the head resolves, not that correlation order rewrites the stream.
