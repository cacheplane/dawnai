...
| A cancelled route whose cleanup never settles holds its thread's slot forever | Accepted. Cancelling stops us *consuming* the route, not necessarily the route itself — a generator suspended at a non-abortable await keeps going until it settles, so the slot is released only when the source genuinely unwinds. A route that hangs forever therefore leaves that one thread permanently 409. That is strictly better than the alternative it replaced (releasing early, admitting a second run, and interleaving checkpoint writes with the still-running first one) and matches this design's general preference for a clear error over silent data loss. A bounded wait plus an operator-visible log is the natural follow-up. |
