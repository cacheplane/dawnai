...
Prove that the Dawn-owned authoring contract can support a real LangChain-native path. Introduce `@dawn/langchain` as the second backend adapter, a unified `BackendAdapter` interface in `@dawn/sdk`, a Vite plugin for build-time tool schema inference, and SSE streaming in `dawn dev`.

## Background

Dawn currently supports two route kinds (`graph` and `workflow`) through a single backend adapter (`@dawn/langgraph`). The authoring contract (`@dawn/sdk`) is backend-neutral by design, but there is no second backend to prove it.
...
A third backend (`@dawn/deepagents`) is planned for Phase 3, which validates investing in a formal adapter interface now.

## Design

### BackendAdapter interface

`@dawn/sdk` gains a new type-only `BackendAdapter` interface:
...
- Both `@dawn/langgraph` and `@dawn/langchain` export a constant or factory that satisfies `BackendAdapter`
- `@dawn/sdk` remains a pure type layer — `BackendAdapter` is a type export only

`RouteKind` expands from `"graph" | "workflow"` to `"graph" | "workflow" | "chain"`.

Three known adapters will implement this interface:
- `@dawn/langgraph` — handles `graph` and `workflow` (existing, refactored)
- `@dawn/langchain` — handles `chain` (new)
- `@dawn/deepagents` — Phase 3, future route kind TBD
...
- `loadRouteKind()` in `load-route-kind.ts` replaces its current call to `normalizeRouteModule()` from `@dawn/langgraph` with CLI-owned logic:
...
- `normalizeRouteModule()` in `@dawn/langgraph` becomes internal or is removed — the CLI no longer imports it for discovery
...
`tool-converter.ts` in `@dawn/langchain` converts each `DiscoveredToolDefinition` to a `DynamicStructuredTool`:
...
New package `@dawn/vite-plugin` at `packages/vite-plugin/`.
...
### `@dawn/langchain` package structure

```
packages/langchain/
  package.json          # peerDeps: @langchain/core; deps: @dawn/sdk
  tsconfig.json
  src/
...
No runtime dependency on `@dawn/langgraph`. `@langchain/core` is a peer dependency (user installs it).

### `@dawn/vite-plugin` package structure
...
**Adapter registration:** CLI maintains a `RouteKind -> BackendAdapter` map. On startup, imports adapters from `@dawn/langgraph` and `@dawn/langchain`. If an import fails (package not installed), the error surfaces at execution time when a route of that kind is invoked.

**`@dawn/langgraph` refactor:** Existing `normalizeRouteModule()` and `invokeEntry()` logic moves behind the `BackendAdapter` interface. `@dawn/langgraph` exports a `BackendAdapter` handling `graph` and `workflow` kinds, plus `.stream()` support.
...
- `@dawn/sdk` remains a pure type layer (plus new `BackendAdapter` type)
...
- `@dawn/langgraph` existing behavior for `graph` and `workflow` routes
...
**New packages:**
- `packages/langchain/` — `@dawn/langchain` adapter package
- `packages/vite-plugin/` — `@dawn/vite-plugin` schema inference plugin
