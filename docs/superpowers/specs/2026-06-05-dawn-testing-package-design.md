...
```ts
import { defineAgentTest, conversation, expectToolCalled, expectFinalMessage, expectState } from "@dawn-ai/testing"
import { afterAll, it } from "vitest"

const t = await defineAgentTest({
...
it("filters open items newest-first", async () => {
  const run = await t.run({
    ...
    fixtures: conversation()
      ...
      .modelCallsTool("applyFilter", { filter: { status: "open" } })
      .modelReplies("Found 2 matching items."),
  })
...
### `defineAgentTest(options)` → `AgentTestHarness`

- **options:** `{ appRoot: string; route: string; fixtures?: FixtureSet; mode?: "in-process" | "http-inject" | "subprocess" }`. `mode` defaults to `"in-process"` (Layer A). The harness loads the app's real `dawn.config.ts` from `appRoot` (so summarization/permissions/offload behave exactly as in production); there is no parallel config-override surface in v1 — to test a different config, point `appRoot` at a fixture app that has it.
...
- `.modelCallsTool(name, args, opts?)` — the model responds with a tool call → an aimock fixture with `response.toolCalls` at the correct auto-assigned `turnIndex`, with a fixed `tool_call_id` (auto-generated stable id unless `opts.id` is given).
- `.modelReplies(content)` — the model responds with content → an aimock fixture at the next `turnIndex` with `hasToolResult: true` when it follows tool calls.
...
**v1 non-goals (research anti-patterns):** no LLM-as-judge / semantic matchers in the gating suite (nightly-only territory); no exact-matching on full prompt bodies in fixture match rules (match the minimal discriminator).
...
- **Drift detection (deferred to v1.1).** A scheduled (nightly) workflow re-records committed fixtures against the live API and flags divergence (model/provider wire changes), decoupled from PR gating so PRs stay fast and non-flaky. v1 leaves the hook (a documented `record()`-based comparison) but does not build the workflow.
...
- Public exports: `defineAgentTest`, `conversation`, `record`, and the `expect*` matchers + their result/fixture types.
...
- **Keep the SP7 interrupt→restart→resume** scenario as the one **Layer C** subprocess smoke.
...
- **Model client constructed before env patch** → prevented by `defineAgentTest` setting env and running typegen before first `run()`, and by building the agent lazily per run (Dawn already materializes the agent at execution time, not module load).
...
- **`runTypegen` failure** (bad app) → surfaced from `defineAgentTest` construction with the typegen error, not a downstream mismatch.

## Testing (of the package itself)

- **Unit:** `conversation()` builder compiles to the expected aimock fixture JSON (turnIndex/hasToolResult/fixed-ids); each `expect*` matcher passes/fails correctly against synthetic `AgentRunResult`s; `record()` invokes the aimock recorder with the right argv (mocked spawn).
...
- **Drift-detection workflow** (the scheduled live-API nightly compare) — v1.1; v1 ships only the `record()` hook.
