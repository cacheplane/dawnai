  ...
  it("replays from a sibling fixture file when the case has no inline fixtures", async () => {
    ...
    const root = await makeApp(
      [
        ...
        "    },",
        "  ],",
        '  scorers: [contains("Found 2 open items", { threshold: 1 })],',
        "  threshold: 1,",
        "})",
        "",
      ].join("\n"),
    ...
    // Write the sibling fixture file next to the eval using the correct slug.
    const evalFile = join(root, "src", "app", "chat", "evals", "filter.eval.ts")
    const baseDir = dirname(evalFile)
    const sibling = siblingFixturePath(evalFile, baseDir, caseName, 0)

    // Verify we compute the right path for this case name ("open" → "open").
    expect(caseSlug(caseName, 0)).toBe("open")

    // Write a hand-crafted fixture set: one turn where the agent replies with the
    // expected content. The fixture format is { fixtures: [...] }.
    const fixtureContent = JSON.stringify(
      {
        fixtures: [
          {
            match: { userMessage: "Filter open items", turnIndex: 0, hasToolResult: false },
            response: { content: "Found 2 open items." },
          },
        ],
      },
      null,
    ...
    await writeFile(sibling, fixtureContent, "utf-8")

    // Run the eval — it should replay from the sibling file and PASS.
    const lines: string[] = []
