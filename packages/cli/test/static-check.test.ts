...
// The EDGE manifest.
//
// `modules.edge.mjs` is a separate artifact with its own copy of every route's
// static imports, and an app configured for `hono` ALONE never emits
// `modules.mjs` at all — so a pass keyed on the node path was a silent no-op
// for exactly the deployment shape that cannot re-discover routes at runtime.
// A stale edge bundle is the more expensive of the two to ship: a node server
// would at least have re-walked the route tree.
// ---------------------------------------------------------------------------

const HONO_CONFIG = 'export default { build: { targets: ["hono"] } };\n'

/** A stale manifest naming a route the app no longer has. */
...
describe("dawn check — edge module manifest staleness", () => {
  test("stale modules.edge.mjs is caught even when no modules.mjs exists at all", async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain("Stale static module manifest")
    // Names the artifact that is actually stale, not the node one.
  ...
  test("a leftover edge manifest is ignored when hono is not a configured target", async () => {
    // An artifact from an abandoned experiment deploys nowhere. Failing over it
  ...
  test("both manifests are checked: a stale node manifest still fails under hono", async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain("Stale static module manifest")
    expect(result.stderr).toContain("modules.mjs")
  })
})
