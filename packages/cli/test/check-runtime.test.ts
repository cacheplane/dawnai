...
describe("gte", () => {
  test("pure MAJOR.MINOR.PATCH numeric compare", () => {
    // 22.9.0 < 24.0.0 <= 24.0.1 < 25.0.0
    expect(gte("22.9.0", "24.0.0")).toBe(false)
    expect(gte("24.0.0", "24.0.0")).toBe(true)
    expect(gte("24.0.1", "24.0.0")).toBe(true)
    expect(gte("25.0.0", "24.0.0")).toBe(true)
  })

  test("compares numerically, not lexically", () => {
    // "9" > "13" lexically but 9 < 13 numerically
    expect(gte("22.9.0", "24.0.0")).toBe(false)
    expect(gte("24.100.0", "24.13.0")).toBe(true)
  })
})
...
describe("checkRuntime", () => {
  ...
  test("Node at/above floor → passed", async () => {
    const result = await checkRuntime({ nodeVersion: "24.1.0" })
