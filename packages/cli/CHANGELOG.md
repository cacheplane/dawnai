...
- 0b1bf7c: Keep Agent Protocol SSE streams alive during silent runs with periodic comment
  frames, and mark run and resume streams `no-transform` for intermediaries.
- c8b8489: Invalidate and restore the CLI's bundled documentation correctly through the Turbo build cache.
- ba612fd: **Correction: the edge quickstart named the wrong module manifest, and
