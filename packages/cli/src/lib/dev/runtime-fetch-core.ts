...
async function handleResumeRequest(options: {
  ...
  try {
    ...
    // The ordinary resume endpoint deliberately keeps executing after a
    // client disconnect. The claim therefore belongs to route execution, not
    // to the Response consumer, and is released only from start's finally.
    const encoder = new TextEncoder()
    const stream = new ReadableStream<Uint8Array>({
      async start(controller) {
        try {
      ...
      cancel() {
        // Do not abort or release the claim here: execution continues after a
        // disconnected consumer and releases the claim when it actually ends.
      },
    })
