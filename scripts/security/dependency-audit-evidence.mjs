...
import { canonicalJsonBytes, EvidenceError, runBoundedProcess } from "./github-evidence.mjs"
...
export function formatUtcSeconds(milliseconds) {
  if (!Number.isSafeInteger(milliseconds) || milliseconds < 0) fail("INVALID_CLOCK")
  let timestamp
  try {
    timestamp = new Date(milliseconds).toISOString().replace(/\.[0-9]{3}Z$/u, "Z")
  } catch {
    fail("INVALID_CLOCK")
  }
