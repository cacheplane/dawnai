...
export async function loadDependabotExpectation(
	file,
	{ root = process.cwd() } = {},
) {
	let source;
...
export async function readDependabotOpen({
	expectedDefaultSha,
	expectedNumbers,
	fixture,
	github,
	...
	if (
		...
		JSON.stringify(expectedFixture.open.map((alert) => alert.number))
	) {
	...
	const rawAlerts = await github.list(
...
export async function reconcileDependabot({
	...
	if (
		...
		!isSha(expectedObservationHeadSha) ||
		typeof now !== "function" ||
	...
	const combined = [...fixedNumbers, ...openNumbers].sort(
		(left, right) => left - right,
	);
	if (
		JSON.stringify(combined) !==
		JSON.stringify(baseline.open.map((alert) => alert.number))
	) {
	...
	const baselineByNumber = new Map(
		baseline.open.map((alert) => [alert.number, alert]),
	);
	let finalOpen = null;
	let finalOpenB = null;
	let finalFixed = null;
	for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
		...
		clock.assertBeforeDeadline();
		if (headBefore.sha !== expectedObservationHeadSha)
			fail("DEFAULT_HEAD_MISMATCH");
		...
		if (openAResult.ready && fixedResult.ready) {
			...
			clock.assertBeforeDeadline();
			if (
				headAfter.sha !== expectedObservationHeadSha ||
				headAfter.sha !== headBefore.sha
			) {
		...
		if (attempt === maxAttempts)
			fail("DEPENDABOT_RECONCILIATION_ATTEMPT_LIMIT");
	...
	const publicationAfter = verifyPublicationSnapshot(
		await collectPublication({ phase: "after" }),
		{
			expectedDefaultSha: expectedObservationHeadSha,
		},
	);
...
async function readVerificationRuns({
	...
	const heads = [
		...new Set([expectedMergeSha, expectedObservationHeadSha]),
	].sort(compareText);
	const result = [];
...
async function readReconciliationOpen({
	...
	for (const alert of open) {
		...
		if (
			alert.state !== "open" ||
			JSON.stringify(alert) !== JSON.stringify(baseline)
		) {
...
async function readReconciliationFixed({
	...
	for (const number of expectedFixedNumbers) {
		...
		const alert = normalizeDependabotAlert(
			await github.object(`dependabot/alerts/${number}`),
		);
		assertWithinDeadline();
		const baseline = baselineByNumber.get(number);
		if (
			baseline === undefined ||
			!stableAlertIdentityMatches(alert, baseline)
		) {
...
function createReconciliationClock(now, timeoutMs) {
	...
	if (started > maximumDate - timeoutMs)
		fail("DEPENDABOT_RECONCILIATION_TIMEOUT");
