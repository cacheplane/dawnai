...
export function analyzeChartLog(log, expectedFact) {
  ...
  let occurrences = 0
  let offset = 0
  while ((offset = log.indexOf(expectedFact, offset)) !== -1) {
    occurrences += 1
...
async function collectNpmAbsence({
  ...
  for (const name of packages) {
    ...
    const targetDocument = await npmJsonRequest({ budget, npmRequest, requested, url: versionUrl })
    if (
    ...
    const packument = await npmJsonRequest({ budget, npmRequest, requested, url: packumentUrl })
    if (
      packument.status !== "OK" ||
...
async function npmJsonRequest({ budget, npmRequest, requested, url }) {
...
function createDefaultNpmRequest() {
  const http = createHttpGet({ maxResponseBytes: NPM_RESPONSE_LIMIT, timeoutMs: 300_000 })
...
function candidateRelease(release, version) {
  ...
  return isRecord(release) && [release.tag_name, release.name].some((value) => identities.has(value))
}
...
function candidateArtifact(name, version) {
  return (
    typeof name === "string" &&
    new RegExp(`(?:^|[-_.])${version.replaceAll(".", "\\.")}(?:$|[-_.])`, "u").test(name)
  )
}
