...
const CHART_FACTS = Object.freeze({
  ...
  "publish (dawn-sandbox-infra)": "dawn-sandbox-infra 0.1.2 already published, skipping",
})
...
export async function collectPublicationContainment({
  ...
  const normalizedInventory = normalizeInventoryInput(inventory, sourceSha, currentVersion)
  ...
  const workflowEvidence = {
    chart: summarizeWorkflowRuns(chartWorkflowObject, chartRuns, CHART_WORKFLOW, sourceSha),
    release: summarizeWorkflowRuns(releaseWorkflowObject, releaseRuns, RELEASE_WORKFLOW, sourceSha),
  }
  ...
  const candidateAbsence = {
    artifacts: !artifacts.some((artifact) => candidateArtifact(artifact?.name, targetVersion)),
    releases: !releases.some((release) => candidateRelease(release, targetVersion)),
  ...
  for (const [index, expected] of RELEASE_INCIDENTS.entries()) {
    ...
    releaseIncidents.push(await collectReleaseIncident(github, history, expected, index))
  }
  ...
  if (closingDefaultCommit.sha !== expectedDefaultSha) fail("CLOSING_DEFAULT_HEAD_DRIFT")
  const closingReleaseWorkflow = await github.object(`actions/workflows/${RELEASE_WORKFLOW.id}`)
  const closingChartWorkflow = await github.object(`actions/workflows/${CHART_WORKFLOW.id}`)
  ...
  const closingWorkflowEvidence = {
    chart: summarizeWorkflowRuns(closingChartWorkflow, closingChartRuns, CHART_WORKFLOW, sourceSha),
  ...
  if (
    canonicalJsonBytes(closingWorkflowEvidence).compare(canonicalJsonBytes(workflowEvidence)) !==
      0 ||
    canonicalRunHistory(closingReleaseRuns).compare(canonicalRunHistory(releaseRuns)) !== 0 ||
    canonicalRunHistory(closingChartRuns).compare(canonicalRunHistory(chartRuns)) !== 0
  ) {
...
function summarizeWorkflowRuns(workflow, runs, expected, sourceSha) {
  ...
  for (const run of runs) {
    if (!isRecord(run) || run.workflow_id !== expected.id || run.path !== expected.path) {
...
async function collectReleaseIncident(github, history, expected, index) {
  ...
  const jobs = await github.list(`actions/runs/${expected[0]}/jobs?per_page=100`, {
    field: "jobs",
    totalCount: true,
    uniqueKey: "id",
  })
...
async function collectChartIncident(github, history) {
  ...
  assertSameRunIdentity(history, run)
  const jobs = await github.list(`actions/runs/${CHART_RUN.id}/jobs?per_page=100`, {
    field: "jobs",
    totalCount: true,
    uniqueKey: "id",
  })
...
async function collectNpmAbsence({ budget, currentVersion, npmRequest, packages, targetVersion }) {
  const records = []
  ...
  for (const name of packages) {
    ...
    if (
      ...
      JSON.stringify(attestation.body) !== JSON.stringify({ error: "Not found" })
    ) {
...
async function npmJsonRequest({ bodyProjection = "complete", budget, npmRequest, requested, url }) {
...
function candidateTag(ref, version) {
  return candidateVersionIdentities(version).has(
    typeof ref === "string" && ref.startsWith("refs/tags/") ? ref.slice("refs/tags/".length) : "",
  )
}
...
function candidateRelease(release, version) {
  ...
  return (
    isRecord(release) && [release.tag_name, release.name].some((value) => identities.has(value))
  )
}
...
export function verifyPublicationSnapshot(value, { expectedDefaultSha } = {}) {
  ...
  if (Object.values(snapshot.candidateAbsence).some((value) => value !== true)) {
...
function verifyInventory(inventory, sourceSha) {
  assertExactKeys(inventory, ["currentVersion", "packages", "ref", "sourceSha", "targetVersion"])
  if (
...
function verifyNpm(npm) {
  ...
  if (JSON.stringify([...seen].sort(compareText)) !== JSON.stringify(INVENTORY_PACKAGES)) {
...
function verifyIncidents(incidents) {
  ...
  if (!Array.isArray(incidents.release) || incidents.release.length !== RELEASE_INCIDENTS.length) {
...
function assertExactKeys(value, expected) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(wanted)) fail("INVALID_PUBLICATION_SNAPSHOT")
}
