...
async function writeContainedReceiptPair({
	...
	try {
		...
		const ancestorChain = await assertTrustedAncestorChain(
			rootPath,
			effectiveUserId,
		);
		const rootFlags =
		...
		receiptFile = await writeExclusiveRegularFile(
			receiptPath,
			receiptBytes,
			effectiveUserId,
		);
		manifestFile = await writeExclusiveRegularFile(
			manifestPath,
			manifestBytes,
			effectiveUserId,
		);
...
async function assertFileHandleBytes(handle, expected) {
	...
	if (!actual.equals(expected) || actualDigest !== expectedDigest)
		fail("INVALID_RECEIPT_OUTPUT");
}
...
async function assertRootIdentity({
	...
	const currentAncestorChain = await assertTrustedAncestorChain(
		rootPath,
		effectiveUserId,
	);
	if (
...
export function validateTrustedAncestorPolicy(
	...
	if (
		![ancestorMode, ancestorUid, protectedChildUid, effectiveUserId].every(
