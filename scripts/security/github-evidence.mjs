...
export function safeEvidenceError(error) {
  return error instanceof EvidenceError ? error.message : "UNPROVABLE: EVIDENCE_OPERATION_FAILED"
}
...
function reserveRequest(budget) {
  ...
  return {
    ...
    timeoutMs: Math.max(1, Math.min(budget.deadline - current, MAX_PROCESS_TIMEOUT_MS)),
  }
}
...
export function createGitHubReader({ budget, repo, transport }) {
  ...
  return {
    ...
    async list(
      path,
      { cursorOnly = false, field, pageLimit = 10, totalCount = false, uniqueKey = "id" } = {},
    ) {
      ...
      let url = firstUrl
      for (;;) {
        if (pagesRead >= pageLimit || budget.remainingPages < 1) fail("PAGE_LIMIT")
        pagesRead += 1
  ...
  return new Promise((resolve, reject) => {
    ...
    const terminate = (code) => {
      ...
      if (platform === "win32") {
        ...
        killTimer = setTimeout(finishTaskkill, PROCESS_TASKKILL_TIMEOUT_MS + PROCESS_KILL_GRACE_MS)
