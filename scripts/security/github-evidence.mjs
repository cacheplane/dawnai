...
const SECRET_KEY = /(?:authorization|cookie|password|secret|token|private.?key)/iu
...
function canonicalize(value, state, depth) {
  ...
  try {
    if (Array.isArray(value)) {
      if (Object.getPrototypeOf(value) !== Array.prototype || Object.keys(descriptors).some((key) => key !== "length" && !/^(?:0|[1-9][0-9]*)$/u.test(key))) {
    ...
    if (prototype !== Object.prototype && prototype !== null) fail("INVALID_JSON_VALUE")
...
export function parseGhIncludedResponse(raw, responseType = "json") {
  if (typeof raw !== "string" || Buffer.byteLength(raw, "utf8") > MAX_PROCESS_BYTES) {
    fail("MALFORMED_GITHUB_RESPONSE")
  }
  const separator = /\r?\n\r?\n/u.exec(raw)
  if (separator === null || separator.index === 0) fail("MALFORMED_GITHUB_RESPONSE")
  const headerText = raw.slice(0, separator.index)
  const bodyText = raw.slice(separator.index + separator[0].length)
  const lines = headerText.split(/\r?\n/u)
  const statusMatch = /^HTTP\/(?:1\.1|2(?:\.0)?) ([1-5][0-9]{2})(?: .*)?$/u.exec(lines[0] ?? "")
...
export function parseNextLink(link, { cursorOnly = false, initialUrl, seen }) {
  ...
  for (const rawPart of link.split(",")) {
    ...
    let rest = match[2]
    while (rest.length > 0) {
      const parameter = /^\s*;\s*([!#$%&'*+.^_`|~0-9A-Za-z-]+)\s*=\s*(?:"([^"\\]*)"|([!#$%&'*+.^_`|~0-9A-Za-z-]+))/u.exec(rest)
  ...
  if (
    ...
    candidate.password !== "" ||
    candidate.hash !== "" ||
    paginationPathIdentity(candidate.pathname) !== paginationPathIdentity(initial.pathname)
  ) {
    fail("UNSAFE_PAGINATION_URL")
  }
  ...
  const fixed = new Map(
    [...initialQuery].filter(([key]) => !["page", "before", "after"].includes(key)),
  )
  ...
  for (const visited of seen) {
    ...
    if (
      ...
      paginationPathIdentity(visitedUrl.pathname) !== paginationPathIdentity(initial.pathname)
    ) {
      fail("INVALID_PAGINATION_STATE")
    }
  ...
  if (visitedUrls.some((visited) => paginationStateKey(visited) === candidateState)) {
...
export function createEvidenceBudget({
  ...
  for (const [value, maximum, label] of [
    ...
    if (!Number.isSafeInteger(value) || value < 1 || value > maximum) fail(`INVALID_${label}`)
  }
...
function reserveRequest(budget) {
  ...
  return {
    ...
    timeoutMs: Math.max(1, Math.min(budget.deadline - budget.now(), MAX_PROCESS_TIMEOUT_MS)),
  }
}
...
export function consumeEvidenceBytes(budget, bytes) {
  if (budget.deadline <= budget.now()) fail("EVIDENCE_TIMEOUT")
  if (!Number.isSafeInteger(bytes) || bytes < 0) fail("MALFORMED_EVIDENCE_RESPONSE")
...
export function createGitHubReader({ budget, repo, transport }) {
  if (!/^[A-Za-z0-9][A-Za-z0-9-]{0,38}\/[A-Za-z0-9][A-Za-z0-9._-]{0,99}$/u.test(repo)) {
  ...
  function initialUrl(path) {
    ...
    if (url.href.slice(0, repositoryBase.length) !== repositoryBase) fail("INVALID_GITHUB_PATH")
  ...
  return {
    async object(path) {
      ...
      const body = JSON.parse(canonicalJsonBytes(response.body).toString("utf8"))
      assertWithinDeadline(budget)
    ...
    async list(path, { cursorOnly = false, field, totalCount = false, uniqueKey = "id" } = {}) {
      ...
      let url = firstUrl
      for (;;) {
        if (budget.remainingPages < 1) fail("PAGE_LIMIT")
        ...
        if (field === undefined) {
          ...
          if (!isRecord(body) || !Array.isArray(body[field])) fail("MALFORMED_GITHUB_SCHEMA")
        ...
        for (const rawRecord of pageRecords) {
          const record = JSON.parse(canonicalJsonBytes(rawRecord).toString("utf8"))
          assertWithinDeadline(budget)
          if (!isRecord(record)) fail("MALFORMED_GITHUB_SCHEMA")
          const identity = record[uniqueKey]
          if (!((typeof identity === "string" && identity.length > 0) || Number.isSafeInteger(identity))) {
        ...
        const next = parseNextLink(response.link, { cursorOnly, initialUrl: firstUrl, seen })
        if (next === null) {
          if (totalCount && expectedTotal !== records.length) fail("TOTAL_COUNT_MISMATCH")
          assertWithinDeadline(budget)
          return records.sort((left, right) =>
            compareText(JSON.stringify(left[uniqueKey]), JSON.stringify(right[uniqueKey])),
          )
        }
        if (budget.remainingPages < 1) fail("PAGE_LIMIT")
        seen.add(next)
...
export function createGhApiTransport({ runProcess = runBoundedProcess } = {}) {
  ...
  return async ({ maxBytes, responseType, timeoutMs, url }) => {
    ...
    for (const [key] of parsed.searchParams) {
      if (/(?:authorization|cookie|password|secret|token|private.?key)/iu.test(key)) {
        fail("INVALID_GITHUB_URL")
      }
    }
...
export function runBoundedProcess({ args, command, cwd, env, maxBytes, timeoutMs }) {
  if (
