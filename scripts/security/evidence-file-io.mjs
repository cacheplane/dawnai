...
export async function readEvidenceInputBytes(file, { contained, cwd, maxBytes = 1024 * 1024 }) {
  if (
    typeof file !== "string" ||
    file.length === 0 ||
    file.includes("\0") ||
  ...
  try {
    ...
    handle = await open(canonicalPath, FS_CONSTANTS.O_RDONLY | (FS_CONSTANTS.O_NOFOLLOW ?? 0))
...
async function assertNoSymlinkInputComponents(root, file) {
  ...
  for (const component of components) {
    ...
    if ((await lstat(current)).isSymbolicLink()) fail("INVALID_RECONCILIATION_INPUT")
  }
}
...
export async function writeCanonicalEvidenceFile(file, value, { cwd }) {
  ...
  if (!parentStat.isDirectory() || parentStat.isSymbolicLink()) fail("INVALID_EVIDENCE_OUTPUT")
