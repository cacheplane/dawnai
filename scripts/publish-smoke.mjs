  ...
  for (const field of ["dependencies", "devDependencies"]) {
    ...
    for (const [name, value] of Object.entries(deps)) {
      ...
      if (String(value).startsWith("file:")) {
        throw new Error(`Default scaffold still emitted repo-local file dependency for ${name}: ${value}`);
      }
    }
  }
...
function packPackage(packageName, outputDir) {
  const result = run("pnpm", ["--filter", packageName, "pack", "--pack-destination", outputDir], repoRoot);
