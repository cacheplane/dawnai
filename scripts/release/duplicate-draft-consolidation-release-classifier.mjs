...
export function consolidationStageRule(stage) {
  ...
  if (rule === undefined) throw new TypeError("Consolidation authority stage is invalid")
  return rule
}
...
export function classifyConsolidationReleases(rawReleases, proposal, stage) {
  ...
  if (!Array.isArray(releases)) throw new TypeError("Release enumeration must be an array")
  ...
  for (const [index, release] of releases.entries()) {
    ...
    if (seenIds.has(id)) throw new Error("GitHub Release enumeration contains a duplicate ID")
    seenIds.add(id)
    ...
    const suspiciousMarkerBody = isSuspiciousMarkerBody(release.body, APPROVED_CANDIDATE)
    const partialMarker = markerSharesCandidateIdentity(marker, APPROVED_CANDIDATE)
    const managed = exactMarker || exactTag || incidentId || partialMarker || suspiciousMarkerBody
    ...
    if (!exactMarker) {
      throw new Error("Managed candidate Release marker or tag identity is malformed or ambiguous")
    }
    if (release.draft !== true || release.immutable !== false || release.published_at !== null) {
      throw new Error("Managed candidate Release is published or not an exact draft")
    }
    if (!expected.has(id)) {
      throw new Error(
  ...
  if (selected.size !== rule.releaseIds.length || rule.releaseIds.some((id) => !selected.has(id))) {
    throw new Error("Release enumeration is missing an exact remaining managed candidate draft")
  }
...
function assertApprovedProposalIdentity(proposal) {
  if (
    ...
    proposal.roles.duplicates.some((id, index) => id !== DUPLICATE_IDS[index]) ||
    ...
    throw new Error("Release classification proposal is not the approved incident identity")
  }
}
