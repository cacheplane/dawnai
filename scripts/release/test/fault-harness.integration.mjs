...
test("Verdaccio and the proxy abort in flight after listening and clean partial resources", async (t) => {
  ...
  for (const row of [
    ...
    void pending.catch(() => {})
    try {
      const identity = await withTestDeadline(marker.promise, 1_000, "built-in listening marker")
      assert.equal((await fetch(identity.url)).status, 200)
      controller.abort()
      await assert.rejects(pending, (error) => error.name === "AbortError")
      await assert.rejects(fetch(identity.url, { signal: AbortSignal.timeout(500) }))
...
test("built-in abort rollback surfaces sanitized aggregate cleanup failures", async (t) => {
  ...
  for (const row of [
    ...
    void pending.catch(() => {})
    try {
      const identity = await withTestDeadline(marker.promise, 1_000, "built-in rollback marker")
      controller.abort()
...
test("Git fixture abort terminates an active validated Git executable", async (t) => {
  ...
  try {
    const pid = Number.parseInt(
      await withTestDeadline(waitForFile(marker), 1_000, "active Git executable marker"),
      10,
    )
...
test("Verdaccio uses disposable loopback storage and releases its random port on failure cleanup", async () => {
  ...
  await assert.rejects(fetch(registry.url, { signal: AbortSignal.timeout(500) }))
...
test("workspace discovery follows validated fixture data across nested paths and added packages", async (t) => {
  ...
  await rename(join(directory, "packages", "base"), join(directory, "nested", "foundation"))
  ...
  const originalPolicy = JSON.parse(await readFile(join(directory, "fault-harness.json"), "utf8"))
  await writeFile(
    join(directory, "fault-harness.json"),
    JSON.stringify({
      packageDirectories: ["packages/gate", "independent", "nested/foundation", "packages/middle"],
      gateOrder: originalPolicy.gateOrder,
    }),
  )
  const changesets = JSON.parse(await readFile(join(directory, ".changeset", "config.json")))
  changesets.fixed = [["@fault/middle", ...originalPolicy.gateOrder, "@fault/alpha", "@fault/base"]]
  await writeFile(join(directory, ".changeset", "config.json"), JSON.stringify(changesets))
  ...
  assert.deepEqual(
    ...
    ["@fault/alpha", "@fault/base", "@fault/middle", ...originalPolicy.gateOrder],
  )
  assert.deepEqual(
...
test("package subprocesses reject lifecycle probes and isolate hostile ambient credentials", async (t) => {
  ...
  try {
    ...
    const workingManifest = JSON.parse(await readFile(workingManifestPath, "utf8"))
...
test("package tool probes share the harness startup deadline", async (t) => {
  ...
  await writeFile(tools.npm.executable, '#!/usr/bin/env node\nconsole.log("1.0.0")\n', {
    mode: 0o755,
  })
      ...
      async startVerdaccio() {
        ...
        return {
          ...
          async close() {
            ...
            if (registryCloseAttempts === 1) throw new Error("injected registry close secret")
      ...
      dependencies: {
        async startVerdaccio() {
          ...
          return {
            ...
            async close() {
              closeAttempts += 1
              if (closeAttempts === 1) throw new Error("first close failed before delegation")
              await registry.close()
            },
          }
        },
...
test("harness startup deadline cleans resources that arrive after timeout", async () => {
            ...
            setTimeout(
              () =>
                resolve({
                  async close() {
                    lateCloseAttempts += 1
                    if (lateCloseAttempts === 1) throw new Error("late close retry")
                    resolveLateClose()
                  },
                }),
              50,
  ...
  await Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error("late startup cleanup did not run")), 250),
    ),
  ])
...
test("the late acquisition supervisor closes a real server after the cleanup window", async () => {
  ...
  await assert.rejects(fetch(lateResource.url, { signal: AbortSignal.timeout(500) }))
...
test("the temporary Git fixture keeps identity local and production reads the advanced annotated tag", async (t) => {
  ...
  const inheritedGitEnvironment = new Map(
    Object.keys(hostileGitEnvironment).map((name) => [name, Reflect.get(process.env, name)]),
  )
...
async function readFixtureWorkspace(root) {
  const policy = JSON.parse(await readFile(join(root, "fault-harness.json"), "utf8"))
  return {
...
function assertSupervisorErrorProperty(error, supervisor) {
  ...
  assert.deepEqual(Object.getOwnPropertyDescriptor(error, "lateAcquisitionSupervisor"), {
    value: supervisor,
    enumerable: false,
    writable: false,
    configurable: false,
