...
test("proves three ordered mutable drafts have the exact production 45-asset escrow", async () => {
  ...
  assert.deepEqual(
    ...
    Array.from({ length: 3 }, () => semanticReleaseProjection(result.releases[0])),
  )
  assert.deepEqual(
    result.releases.map((release) => release.assets.map(semanticAssetProjection)),
    Array.from({ length: 3 }, () => result.releases[0].assets.map(semanticAssetProjection)),
  )
})
...
test("normalizes exact GitHub timestamps and rejects invalid raw calendar or precision forms", async (t) => {
  const cases = [
    ...
    ["asset creation", (release, value) => (release.assets[0].created_at = value)],
    ["asset update", (release, value) => (release.assets[0].updated_at = value)],
  ]
    ...
    await t.test(`${name} second precision`, async () => {
      ...
      const normalized = name.startsWith("Release")
        ? name.endsWith("creation")
          ...
          : evidence.updatedAt
        : name.endsWith("creation")
          ? evidence.assets.find(({ name }) => name === fixture.releases[0].assets[0].name)
              .createdAt
          : evidence.assets.find(({ name }) => name === fixture.releases[0].assets[0].name)
              .updatedAt
...
test("enforces namespace-specific declared-size caps before any asset download", async (t) => {
  const categories = [
    ["release record", "release-record.json", RELEASE_PAYLOAD_LIMITS.releaseRecordBytes],
    ...
    ["package", (name) => name.endsWith(".tgz"), RELEASE_PAYLOAD_LIMITS.tarballBytes],
  ]
  ...
  for (const [label, selector, maximum] of acceptedBoundaries) {
    await t.test(`${label} applicable boundary reaches its download`, async () => {
      const fixture = createDuplicateDraftConsolidationFixture()
      const asset = fixture.releases[0].assets.find(({ name }) =>
        typeof selector === "string" ? name === selector : selector(name),
      )
      ...
      assert.equal(fixture.operations.includes(`download:${fixture.survivorId}:${asset.id}`), true)
    })
  }
})
...
test("rejects prepared-package and bundle aggregate overflow before the offending download", async (t) => {
  await t.test("prepared packages", async () => {
    const fixture = createDuplicateDraftConsolidationFixture()
    const asset = fixture.releases[0].assets.find(({ name }) => name.endsWith(".tgz"))
  ...
  await t.test("attestation bundles", async () => {
    ...
    await assert.rejects(inspectEquivalentDrafts(INPUT(fixture)), /attestation|bundle|payload/iu)
    assert.equal(fixture.downloadCount, 0)
  })
})
...
test("excludes only recorded Release and asset service volatility from equality", async (t) => {
  const cases = [
    ...
    ["Release creation", (release) => (release.created_at = "2026-08-30T00:00:00.000Z")],
    ["Release update", (release) => (release.updated_at = "2026-08-30T00:01:00.000Z")],
    ["derived Release URL", (release) => (release.html_url = "https://example.invalid/changed")],
    ["asset id", (release) => (release.assets[0].id = 777_777_777)],
    ["asset node id", (release) => (release.assets[0].node_id = "RA_changed")],
    ["asset creation", (release) => (release.assets[0].created_at = "2026-08-30T00:00:00.000Z")],
    ["asset update", (release) => (release.assets[0].updated_at = "2026-08-30T00:01:00.000Z")],
    ["download count", (release) => (release.assets[0].download_count = 999)],
    [
      "derived asset URL",
      (release) => (release.assets[0].browser_download_url = "https://example.invalid/asset"),
    ],
  ]
  for (const [name, mutate] of cases) {
...
test("every included Release semantic field blocks parity drift", async (t) => {
  const cases = [
    ...
    ["target commitish", (release) => (release.target_commitish = "f".repeat(40))],
    ["draft", (release) => (release.draft = false)],
    ["immutable", (release) => (release.immutable = true)],
    ["prerelease", (release) => (release.prerelease = true)],
    ["published at", (release) => (release.published_at = "2026-09-01T00:00:00.000Z")],
    ...
    ["author node id", (release) => (release.author.node_id = "MDQ6VXNlcjIwNDg=")],
  ]
  for (const [name, mutate] of cases) {
    await t.test(name, async () => {
      const fixture = createDuplicateDraftConsolidationFixture()
...
test("every included asset semantic field blocks parity drift", async (t) => {
  const cases = [
    ...
    ["content type", (asset) => (asset.content_type = "application/octet-stream")],
    ...
    ["uploader node id", (asset) => (asset.uploader.node_id = "uploader-changed")],
  ]
  for (const [name, mutate] of cases) {
    await t.test(name, async () => {
      const fixture = createDuplicateDraftConsolidationFixture()
...
test("rejects noncanonical core evidence and incorrect downloaded bytes", async (t) => {
  const cases = [
    ["malformed marker", (fixture) => (fixture.releases[0].body = "not a release marker")],
    ["noncanonical marker", (fixture) => (fixture.releases[0].body += "\n")],
    [
      "release record bytes",
      (fixture) =>
        fixture.replaceAssetBytes(fixture.survivorId, "release-record.json", Buffer.from("{}\n"), {
          updateMetadata: true,
        }),
    ],
    [
      "manifest bytes",
      (fixture) =>
        fixture.replaceAssetBytes(fixture.survivorId, "manifest.json", Buffer.from("{}\n"), {
          updateMetadata: true,
        }),
    ],
    [
      "manifest package order drift",
      (fixture) => mutateManifest(fixture, (manifest) => manifest.packageOrder.reverse()),
    ],
    [
      "manifest package name drift",
      (fixture) =>
        mutateManifest(fixture, (manifest) => (manifest.packages[0].name = "@dawn-ai/not-real")),
    ],
    [
      "manifest package hash drift",
      (fixture) =>
        mutateManifest(fixture, (manifest) => (manifest.packages[0].sha256 = "f".repeat(64))),
    ],
    [
      "download mismatch",
      (fixture) =>
        fixture.replaceAssetBytes(fixture.survivorId, "manifest.json", Buffer.from("changed")),
    ],
    [
      "bundle set mismatch",
      (fixture) =>
...
test("rejects a canonical shared marker whose release-record digest is wrong", async () => {
  ...
  await assert.rejects(inspectEquivalentDrafts(INPUT(fixture)), /marker|record|digest/iu)
})
...
test("rejects malformed or non-exact Release asset inventories before destructive evidence exists", async (t) => {
  const cases = [
    ...
    [
      ...
      (fixture) => (fixture.releases[0].assets[1].name = fixture.releases[0].assets[0].name),
    ],
    [
      "duplicate asset id",
      (fixture) => (fixture.releases[0].assets[1].id = fixture.releases[0].assets[0].id),
    ],
    ["non-uploaded asset", (fixture) => (fixture.releases[0].assets[0].state = "new")],
    ["missing GitHub digest", (fixture) => (fixture.releases[0].assets[0].digest = null)],
    [
      "malformed GitHub digest",
      (fixture) => (fixture.releases[0].assets[0].digest = `sha256:${"A".repeat(64)}`),
    ],
    [
...
test("rejects a fourth matching draft, a published candidate Release, wrong roles, or wrong author", async (t) => {
  await t.test("fourth matching draft", async () => {
    ...
    await assert.rejects(inspectEquivalentDrafts(INPUT(fixture)), /exactly three|fourth|managed/iu)
    assert.equal(fixture.downloadCount, 0)
  })
  await t.test("published candidate", async () => {
    ...
    await assert.rejects(inspectEquivalentDrafts(INPUT(fixture)), /published/iu)
  })
  await t.test("reordered roles", async () => {
    const fixture = createDuplicateDraftConsolidationFixture()
    fixture.duplicateIds.reverse()
    await assert.rejects(inspectEquivalentDrafts(INPUT(fixture)), /approved|order|role/iu)
  })
  await t.test("wrong author", async () => {
    const fixture = createDuplicateDraftConsolidationFixture()
    for (const release of fixture.releases) release.author.login = "wrong-owner"
  ...
  await t.test("wrong author node id", async () => {
    const fixture = createDuplicateDraftConsolidationFixture()
    for (const release of fixture.releases) release.author.node_id = "MDQ6VXNlcjIwNDg="
    await assert.rejects(inspectEquivalentDrafts(INPUT(fixture)), /author/iu)
  })
  await t.test("failed attestation verification", async () => {
    const fixture = createDuplicateDraftConsolidationFixture()
    fixture.failVerification()
    await assert.rejects(inspectEquivalentDrafts(INPUT(fixture)), /attestation|verification/iu)
  })
})
...
test("direct target reads allow approved service volatility and return its latest metadata", async (t) => {
  const cases = [
    ...
    ["Release creation", (release) => (release.created_at = "2026-08-29T00:00:00.000Z")],
    ["Release update", (release) => (release.updated_at = "2026-09-01T00:00:00.000Z")],
    ["asset id", (release) => (release.assets[0].id = 777_777_777)],
    ["asset node id", (release) => (release.assets[0].node_id = "RA_latest")],
    ["asset creation", (release) => (release.assets[0].created_at = "2026-08-29T00:00:00.000Z")],
    ["asset update", (release) => (release.assets[0].updated_at = "2026-09-01T00:00:00.000Z")],
    ["download count", (release) => (release.assets[0].download_count = 999)],
  ]
  for (const [name, mutate] of cases) {
    await t.test(name, async () => {
...
test("direct target reads reject nonmonotone clocks and every included parity drift", async (t) => {
  ...
  const cases = [
    ...
    ["immutable", (release) => (release.immutable = true)],
    ["prerelease", (release) => (release.prerelease = true)],
    ["published", (release) => (release.published_at = "2026-09-01T00:00:00.000Z")],
    ...
    ["asset content type", (release) => (release.assets[0].content_type = "text/plain")],
    ["asset size", (release) => (release.assets[0].size += 1)],
    ["asset digest", (release) => (release.assets[0].digest = `sha256:${"f".repeat(64)}`)],
...
test("public evidence parsers reject hostile shapes without invoking accessors and return owned frozen data", async () => {
  ...
  assert.deepEqual(assertEvidenceEqualsProposal(parsed, result.releases[0]), parsed)
  ...
  assert.deepEqual(assertEvidenceEqualsProposal(volatile, parsed), parseReleaseEvidence(volatile))
  ...
  assert.throws(() => parseReleaseEvidence(oversized), /escrow|payload|limit/iu)
  ...
  assert.throws(() => semanticReleaseProjection(accessor), /accessor|data|field|plain|snapshot/iu)
  assert.throws(() => semanticAssetProjection(accessor), /accessor|data|field|plain|snapshot/iu)
})
...
test("strict evidence IDs accept only canonical positive decimal strings", async () => {
  ...
  for (const value of invalid) {
    for (const mutate of [
      ...
      mutate(evidence)
      assert.throws(() => parseReleaseEvidence(evidence), /decimal|id|identity/iu)
    }
  }
})
