...
test("composition exposes only one safe authority capture entrypoint and no raw authority surface", async () => {
  ...
  for (const forbidden of [
    ...
    assert.equal(Reflect.ownKeys(adapters).includes(forbidden), false)
    assert.equal(Reflect.get(adapters, forbidden), undefined)
  }
  assert.deepEqual(Reflect.ownKeys(new Proxy(adapters, {})), Reflect.ownKeys(adapters))
...
test("GitHub reads use exact trusted endpoints, headers, pagination, and normalized evidence", async () => {
  ...
  assert.deepEqual(await adapters.github.listNonterminalWorkflowRuns(workflowQuery()), {
    query: workflowQuery(),
...
async function createGuardedWriter(overrides = {}) {
  const fetchImpl = overrides.fetchImpl ?? (async () => new Response(null, { status: 204 }))
