	...
	const github = Object.freeze({
		async downloadReleaseAsset({ assetId, maximumBytes, releaseId }) {
			...
			const bytes =
				asset === undefined
					? undefined
					: payloads.get(`${String(releaseId)}:${asset.name}`);
		...
		async getRelease({ releaseId }) {
			...
			if (release === undefined)
				throw new Error("fixture Release does not exist");
...
function createArtifact() {
	...
	const packages = CANONICAL_RELEASE_PACKAGE_ORDER.map((name) => {
		...
		const bytes = Buffer.from(
			`package:${name}:${DUPLICATE_DRAFT_CANDIDATE.commitSha}\n`,
			"utf8",
		);
	...
	const statement = {
		...
		predicate: {
			runDetails: {
				metadata: {
					invocationId:
						"https://github.com/cacheplane/dawnai/actions/runs/8004/attempts/1",
				},
			},
		},
	};
