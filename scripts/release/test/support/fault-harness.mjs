  ...
  const supervisor = Object.freeze({
    ...
    async waitForIdle(options) {
      ...
      try {
        ...
        throw Object.assign(new Error("Late acquisition supervisor wait exceeded its deadline"), {
...
export async function createFaultHarness({
  ...
  try {
    ...
    proxy = await acquireWithDeadline(
      ({ signal }) => factories.startFaultProxy({ upstreamUrl: registry.url, signal }),
      Math.max(1, startupDeadline - Date.now()),
      cleanupTimeoutMs,
      "fault proxy startup",
      lateAcquisitionSupervisor,
    ...
    const cleanupErrors = await cleanupResources(resources, cleanupTimeoutMs).then(
      () => [],
      (cleanupError) =>
        cleanupError instanceof AggregateError ? cleanupError.errors : [cleanupError],
    )
    const initiatingError = sanitizedError(error, "Fault harness startup failed")
    ...
    throw attachLateAcquisitionSupervisor(initiatingError, lateAcquisitionSupervisor)
  }
}
...
function harnessFactories(value) {
  ...
  if (Object.values(factories).some((factory) => typeof factory !== "function")) {
    throw new TypeError("Fault harness dependencies are invalid")
  }
  return factories
}
...
async function cleanupResources(resources, timeoutMs) {
  ...
  if (errors.length > 0) throw new AggregateError(errors, "Fault harness cleanup failed")
}
...
async function cleanupResource(entry, deadline) {
  ...
  return lastError ?? sanitizedError({ code: "DEADLINE_EXCEEDED" }, `${entry.label} cleanup failed`)
}
...
async function acquireWithDeadline(
  ...
  const pending = Promise.resolve().then(() => factory({ signal: controller.signal }))
  try {
    ...
    if (outcome?.status === "fulfilled") {
      ...
      throw Object.assign(new Error(`${label} did not settle after cancellation`), {
...
function superviseLateAcquisition({ pending, cleanupTimeoutMs, label, supervisor }) {
...
function assertLifecycleTimeout(value, label) {
  if (!Number.isSafeInteger(value) || value < 10 || value > MAX_LIFECYCLE_TIMEOUT_MS) {
...
export async function discoverFaultWorkspace({ fixtureDirectory }) {
  ...
  const policy = parseObject(await readFile(join(root, "fault-harness.json"), "utf8"), "policy")
  if (!hasExactKeys(policy, ["packageDirectories", "gateOrder"])) {
    throw new TypeError("Fault workspace policy fields are invalid")
  }
  const packageDirectories = validatedStrings(policy.packageDirectories, "package directories")
  ...
  for (const configuredDirectory of packageDirectories) {
    ...
    if (!withinRoot(root, directory) || !(await lstat(directory)).isDirectory()) {
      throw new TypeError("Fault workspace package directory is invalid")
    }
  ...
  if (!sameSet(fixedNames, packageNames)) {
    throw new TypeError("Fault workspace inventory differs from its fixed group")
  }
  if (gateOrder.some((name) => !fixedNames.includes(name))) {
    throw new TypeError("Fault workspace gate policy is outside its fixed group")
  }
  const versions = new Set(packages.map(({ version }) => version))
  if (versions.size !== 1) throw new TypeError("Fault workspace versions are not uniform")
  return Object.freeze({
    gateOrder: Object.freeze([...gateOrder]),
    orderedPackages: Object.freeze(orderReleasePackages(packages, { gateOrder })),
  })
}
...
async function resolvePackageTools(workspaceDirectory, configured, environment, startupDeadline) {
  const rootManifest = parseObject(
    await readFile(join(workspaceDirectory, "package.json"), "utf8"),
    "root manifest",
  )
  const match = /^pnpm@([0-9]+\.[0-9]+\.[0-9]+)$/u.exec(rootManifest.packageManager)
  if (match === null) throw new TypeError("Fault workspace package manager is invalid")
  ...
  for (const descriptor of Object.values(descriptors)) {
    ...
    const observedVersion = (
      await command(descriptor.executable, [...descriptor.prefixArguments, "--version"], {
...
async function discoverPackageTools(expectedPnpmVersion) {
  ...
  if (pnpm === null || npm === null) throw new Error("Required package tool is unavailable")
  return {
...
async function findExecutable(name) {
  for (const directory of requiredPath().split(delimiter)) {
    if (!isAbsolute(directory)) throw new TypeError("Fault harness requires an absolute PATH")
...
function command(executable, args, { cwd, env, operation, timeoutMs = COMMAND_TIMEOUT_MS }) {
  return new Promise((resolve, reject) => {
    execFile(
      ...
      (error, stdout) => {
        if (error !== null) {
          reject(
            Object.assign(new Error(`Fault harness ${operation} command failed`), {
