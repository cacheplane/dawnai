...
export async function createGitFixture({ sourceDirectory, signal, gitExecutable }) {
  ...
  try {
    ...
    await git(workingDirectory, ["init", "--object-format=sha1", "-b", "main"], environment)
    await git(workingDirectory, ["config", "--local", "user.name", IDENTITY.name], environment)
    await git(workingDirectory, ["config", "--local", "user.email", IDENTITY.email], environment)
    await git(workingDirectory, ["add", "--all"], environment)
    await git(workingDirectory, ["commit", "-m", "fixture base"], environment, FIRST_DATE)
    ...
    await git(workingDirectory, ["commit", "-m", "advance main"], environment, SECOND_DATE)
    const mainCommitSha = await revParse(workingDirectory, "HEAD", environment)
    await git(workingDirectory, ["remote", "add", "origin", bareRemoteDirectory], environment)
    await git(workingDirectory, ["push", "--set-upstream", "origin", "main"], environment)
    await git(workingDirectory, ["push", "origin", "refs/tags/v1.2.3"], environment)
    cancellation?.throwIfAborted()
...
async function revParse(directory, ref, environment) {
  return (await git(directory, ["rev-parse", "--verify", ref], environment)).trim()
}
