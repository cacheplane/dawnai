...
export async function createDuplicateDraftConsolidationAdapters(options) {
  ...
  const runGh = async (args) => {
    if (!safeStringArray(args)) throw new TypeError("Attestation command arguments are invalid")
  ...
  const verifyAttestations = bindMethod(attestationVerifier, "verify", "attestation verifier")
  ...
  const writer = deepFreeze({
    async deleteDuplicate(input) {
      ...
      const releaseId = canonicalStringId(required(call, "releaseId", "Delete Release ID"))
  ...
  Object.defineProperty(
    ...
    hiddenMethod((input) => networkGuard.captureInspectionTerminal(rawGithub, input)),
  )
  Object.defineProperty(
    adapters,
...
export function createExactDuplicateDeleteEffect(options) {
  const value = exactDataOptions(options, DELETE_OPTION_FIELDS, "Delete effect options")
  if (required(value, "repository", "Delete repository") !== REPOSITORY) {
    throw new TypeError("Delete repository is not the approved incident repository")
  }
  ...
  const fetchImpl = requiredFunction(value, "fetchImpl", "Delete fetch implementation")
  ...
  const writer = deepFreeze({
    async deleteDuplicate(input) {
      const call = exactDataOptions(input, DELETE_CALL_FIELDS, "Delete call options")
      const releaseId = canonicalStringId(required(call, "releaseId", "Delete Release ID"))
      const permit = required(call, "permit", "Delete permit")
      const permitBinding =
      ...
      if (
        ...
        throw new Error("Delete permit has no active journal transaction lease")
      }
      permitBinding.assertActiveLease()
      if (releaseId === survivorId || !approved.has(releaseId)) {
        throw new TypeError("Release ID is the survivor or is not an approved duplicate")
      }
      ...
      if (beforeVerification > permitBinding.authorityExpiresAt) {
        throw new Error("Delete permit expired before final committed-state verification")
      }
      ...
      if (observedAt > permitBinding.authorityExpiresAt) {
        throw new Error("Delete permit expired at the writer's final pre-send clock")
      }
...
function createIncidentGitHubReader({ reader, fetchImpl, token, now, wallNow }) {
  ...
  const listReleaseAssets = bindMethod(reader, "listReleaseAssets", "GitHub reader")
  const downloadReleaseAsset = bindMethod(reader, "downloadReleaseAsset", "GitHub reader")
  ...
  return deepFreeze({
    async getRepository() {
      ...
      if (!isPlainRecord(body) || body.full_name !== REPOSITORY || body.default_branch !== "main") {
    ...
    async getAuthenticatedUser() {
      const body = await readDirectJson(http, `${API_ORIGIN}/user`, token, "authenticated user")
    ...
    async getWorkflowState() {
      const value = presentValue(await getWorkflow({ workflow: "release.yml" }), "workflow")
      if (
        !isPlainRecord(value) ||
    ...
    async listNonterminalWorkflowRuns(input) {
      ...
      const runs = await readNonterminalWorkflowRuns(http, token, wallNow, query)
    ...
    async getAnnotatedTag(input) {
      ...
      const ref = presentValue(await getRef({ ref: `tags/${APPROVED_TAG}` }), "ref")
      if (
        !isPlainRecord(ref) ||
      ...
      const tag = presentValue(await getGitTag({ tagSha: ref.object.sha }), "git-tag")
      if (
        !isPlainRecord(tag) ||
        ...
        throw new TypeError("GitHub annotated-tag object evidence is malformed")
      }
      return deepFreeze({
        name: APPROVED_TAG,
    ...
    async listReleases() {
      return rejectDuplicateIds(await listReleases(), "releases", "DUPLICATE_RELEASE_ID")
    },
    async getRelease(input) {
      return normalizedGitHubEnvelope(await getRelease(input), "release", "value")
    },
...
function exactWorkflowRunQuery(value) {
  if (!isPlainRecord(value) || !Object.isFrozen(value)) {
    throw new TypeError("Workflow-run query must be an exact deeply frozen object")
  }
  ...
  const statuses = snapshotStringArray(query.statuses, "Workflow-run query statuses")
  if (
    !arraysEqual(statuses, NONTERMINAL_STATUS_ORDER) ||
    query.perPage !== 100 ||
    query.maximumPages !== MAX_PAGES
  ) {
    throw new TypeError("Workflow-run query does not match the exact status or page bounds")
  }
...
function guardNetworkFacade(source, guard, label) {
  ...
  for (const name of Object.keys(source)) {
    ...
    facade[name] = (...args) => guard.runRequest(`${label} ${name}`, () => method(...args), args)
  }
...
function expectedAuthorityTrace(proposal, stage) {
  ...
  const steps = [
    ...
    exact("GitHub reader getWorkflowState", [], (actual) => {
      const { workflowId, path: workflowPath, state: workflowState } = proposal.workflowAuthority
      equals({ workflowId, path: workflowPath, state: workflowState }, "Workflow capture")(actual)
    }),
    exact(
    ...
    exact("GitHub reader getAnnotatedTag", [{ name: proposal.candidate.tag }], (actual) => {
    ...
    exact("GitHub reader listReleases", [], (actual, session) => {
      ...
      const classification = classifyConsolidationReleases(actual.value, proposal, stage)
...
function appendExpectedPayloadTrace(steps, proposal, stage, rawReleases, exact, equals) {
      ...
      steps.push(
        exact(
          ...
          (actual, session) => {
            ...
            if (!session.downloads.has(releaseId)) session.downloads.set(releaseId, new Map())
            session.downloads
              .get(releaseId)
    ...
    if (stage === "pre-delete-1" || stage === "final") {
      steps.push(
        exact(
          ...
          null,
          (actual) => {
            equals(proposal.payloadProof.attestationVerification, "Attestation capture")(actual)
          },
          (args, session) => assertCapturedAttestationArgs(args, proposal, releaseId, session),
        ),
      )
    }
  }
  if (stage !== "final") {
    steps.push(
      exact("terminal Release GET", [{ releaseId: targetReleaseId }], (actual, session) => {
        session.keyResults.set("terminal-release", actual)
      }),
      exact("terminal asset enumeration", [{ releaseId: targetReleaseId }], (actual, session) => {
...
function assertCapturedAttestationArgs(args, proposal, releaseId, session) {
  ...
  if (recordBytes === undefined) {
    throw new Error("Attestation capture is missing the guarded release record")
  }
  let record
  try {
  ...
  const expectedFiles = subjects.map(({ name }) => {
    ...
    if (bytes === undefined) throw new Error("Attestation subject was not downloaded by the guard")
  ...
  const expectedBundles = subjects.map(({ name }) => {
    ...
    if (bytes === undefined) throw new Error("Attestation bundle was not downloaded by the guard")
  ...
  if (!isDeepStrictEqual(normalized, expected)) {
    throw new Error("Attestation capture arguments differ from guarded proposal evidence")
  }
}
...
function assertCapturedAuthorityProjection(capture, authority) {
  ...
  const runs = capture.keyResults.get("GitHub reader listNonterminalWorkflowRuns")
  if (
    local === undefined ||
    main === undefined ||
    tag === undefined ||
    ...
    throw new Error("Authority projection differs from exact guard-recorded results")
  }
}
...
async function validateCapturedReleaseEvidence(capture, authority) {
  ...
  const capturedClassification = capture.keyResults.get("release-classification")
  const classification = classifyConsolidationReleases(rawReleases, proposal, capture.stage)
  if (
    capturedClassification === undefined ||
    capturedClassification.enumerationSha256 !== classification.enumerationSha256
  ) {
    throw new Error("Captured full Release enumeration classification differs from its trace")
  }
  for (const authorityRelease of authority.releases) {
    const proposed = proposal.releases.find(({ id }) => id === authorityRelease.id)
    const raw = classification.selected.find(({ id }) => String(id) === authorityRelease.id)
  ...
  if (capture.targetReleaseId !== null) {
    ...
    const targetAuthority = authority.releases.find(({ id }) => id === capture.targetReleaseId)
    if (
...
function createNetworkGuard({ cwd, now }) {
  ...
  const terminalNow = () => {
    ...
    return Reflect.apply(NATIVE_DATE_TO_ISO_STRING, Reflect.construct(NATIVE_DATE, [millis]), [])
  }
  ...
  const runRequest = async (label, operation, traceArgs) => {
    ...
    const terminalOwner = state === "terminal" && terminal !== null && owner === terminal.token
    const deleteOwner = state === "deleting" && owner === terminal?.deleteToken
    const allowedDeleteRequest =
      deleteOwner && (label === "approved DELETE" || label === "network transport")
    if (state !== "open" && !captureOwner && !terminalOwner && !allowedDeleteRequest) {
      invalidate()
    ...
    if (traced) {
      ...
      if (
        ...
        (expected.args !== null && !isDeepStrictEqual(snapshotJson(traceArgs), expected.args))
      ) {
        invalidate()
        throw new Error(`${label} is out of order in the authority capture trace`)
      }
    ...
    try {
      ...
      if (deleteOwner && terminal?.invalidated) {
        throw new Error(`${label} completed after a reentrant adapter invalidated DELETE`)
      }
      if (traced) {
        if (capture.invalidated || state === "invalidated") {
          throw new Error(`${label} completed after authority capture invalidation`)
        }
        const expected = capture.expected[capture.nextStep]
        ...
        capture.entries.push(canonicalTraceEntry(label, traceArgs, resultSnapshot))
        capture.nextStep += 1
  ...
  const sealedEpoch = (session) => {
    ...
    const descriptors = {
      ...
      bindAuthority: hiddenMethod((input) => bindCapturedAuthority(session, input)),
      toJSON: hiddenMethod(() => {
        throw new TypeError("Adapter network epoch capability cannot be serialized")
      }),
    }
  ...
  const bindCapturedAuthority = async (session, input) => {
    ...
    const authority = snapshotJson(required(value, "authority", "Captured authority"))
    ...
    const releaseEnumerationSha256 = await validateCapturedReleaseEvidence(capture, authority)
  ...
  const beginAuthorityCapture = (facade, rawGithub, input) => {
    ...
    const targetValue = required(value, "targetReleaseId", "Authority capture target")
    const targetReleaseId = targetValue === null ? null : canonicalStringId(targetValue)
    ...
    const scope = (source) => {
      const scoped = {}
      for (const name of Object.keys(source)) {
        scoped[name] = (...args) => context.run(session.token, () => source[name](...args))
      }
    ...
    Object.defineProperties(capability, {
      ...
      beginTerminalRead: hiddenMethod((terminalInput) =>
        context.run(session.token, () => beginTerminalRead(rawGithub, terminalInput)),
      ),
      ...
      toJSON: hiddenMethod(() => {
        throw new TypeError("Authority capture capability cannot be serialized")
      }),
    })
    return Object.freeze(capability)
  }
  ...
  const beginTerminalRead = (rawGithub, input) => {
    const call = exactDataOptions(input, new Set(["releaseId"]), "Terminal read options")
    const releaseId = canonicalStringId(required(call, "releaseId", "Terminal Release ID"))
    if (!DUPLICATE_IDS.includes(releaseId)) {
      invalidate()
      throw new TypeError("Terminal Release ID is not an approved duplicate")
    }
    const captureOwner = capture !== null && context.getStore() === capture.token
    if ((state !== "open" && !(state === "capturing" && captureOwner)) || activeRequests !== 0) {
      invalidate()
      throw new Error("Terminal network read cannot absorb a concurrent request")
    }
    const session = {
      token: Object.freeze({}),
      deleteToken: Object.freeze({}),
    ...
    const terminalStep = async (step, name, options, operation) => {
      let call
      try {
        call = exactDataOptions(options, new Set(["releaseId"]), `Terminal ${name} options`)
        ...
        if (requestedReleaseId !== session.releaseId) {
          throw new TypeError("Terminal request target differs from its session")
        }
      } catch {
      ...
      try {
        ...
        if (
          ...
          throw new Error("Terminal network read was invalidated while pending")
        }
    ...
    const github = deepFreeze({
      getRelease(options) {
        return terminalStep(0, "Release GET", options, (owned) => rawGithub.getRelease(owned))
      },
    ...
    Object.defineProperties(capability, {
      ...
      seal: hiddenMethod(() => {
        ...
        if (
          capture !== null &&
          (capture.invalidated || capture.inFlight || capture.nextStep !== capture.expected.length)
        ) {
          session.invalidated = true
          state = "invalidated"
  ...
  const captureInspectionTerminal = async (rawGithub, input) => {
    ...
    try {
      ...
      const candidate = snapshotJson(required(value, "candidate", "Inspection candidate"))
      const releases = snapshotJson(required(value, "releases", "Inspection releases"))
      if (
        ...
        throw new Error("Inspection terminal cannot start from this adapter state")
      }
      ...
      const terminalStep = async (step, name, options, operation) => {
        ...
        session.inFlight = true
        try {
          const result = await runRequest(`inspection terminal ${name}`, () => operation(call))
          if (
            state !== "terminal" ||
            terminal !== session ||
            session.invalidated ||
      ...
      const github = deepFreeze({
        getRelease(options) {
          return terminalStep(session.nextStep, "Release GET", options, (owned) =>
            rawGithub.getRelease(owned),
          )
        },
        listReleaseAssets(options) {
          return terminalStep(session.nextStep, "asset enumeration", options, (owned) =>
            rawGithub.listReleaseAssets(owned),
          )
        },
      })
  ...
  guardFacade = Object.freeze({
    ...
    bindWriter(writer) {
      ...
      if (identity === undefined || boundWriterIdentity !== null) {
        invalidate()
        throw new TypeError("Adapter network guard delete writer binding failed")
      }
    ...
    armTask6Transition(input) {
      try {
        const session = terminal
        if (session === null || capture === null || capture.bound === null || capture.invalidated) {
          throw new Error("Task6 transition has no bound authority capture trace")
        }
        assertSealed(session)
        if (
          ...
          throw new Error("Task6 transition requires a completed approved terminal target read")
        }
        ...
        if (targetReleaseId !== session.releaseId) {
          throw new Error("Task6 transition target differs from the terminal session")
        }
        ...
        const committedJournal = required(binding, "committedJournal", "Task6 committed journal")
        ...
        const committedHead = required(binding, "committedHead", "Task6 committed journal head")
        const predecessorProvenance = authenticatedPrivateRead(predecessorJournal, journalPath)
        const committedProvenance = authenticatedPrivateRead(committedJournal, journalPath)
        authenticatedPrivateRead(predecessorHead, journalHeadPath)
        authenticatedPrivateRead(committedHead, journalHeadPath)
        if (
          predecessorProvenance.identity.dev !== committedProvenance.identity.dev ||
          predecessorProvenance.identity.ino === committedProvenance.identity.ino
        ) {
          throw new Error("Task6 journal transition did not replace one authenticated predecessor")
        }
        const predecessorEnvelope = parseConsolidationJournal(predecessorJournal)
        ...
        const confirmation = required(binding, "confirmation", "Task6 confirmation")
        ...
        const releaseEnvelope = capture.keyResults.get("releases")
        if (releaseEnvelope === undefined) {
          throw new Error("Task6 transition has no guarded Release enumeration")
        }
        ...
        if (
          ...
          capture.bound.releaseEnumerationSha256 !== releaseClassification.enumerationSha256 ||
          capture.bound.targetReadSha256 !== traceSha256(normalizeTraceValue(authority.targetRead))
        ) {
          throw new Error("Task6 authority differs from the sealed guard-owned trace")
        }
        const expectedPredecessorHead = canonicalJournalHeadBytes(journalPath, predecessorEnvelope)
        const expectedCommittedHead = canonicalJournalHeadBytes(journalPath, committedEnvelope)
        if (
          !predecessorHead.equals(expectedPredecessorHead) ||
          !committedHead.equals(expectedCommittedHead)
        ) {
          throw new Error("Task6 authenticated journal heads do not bind the legal append")
        }
        if (
          predecessorEnvelope.record.proposedRecordSha256 !== proposedEnvelope.recordSha256 ||
          predecessorEnvelope.record.confirmationSha256 !== confirmationSha256 ||
          predecessorState.controllerSha !== proposedEnvelope.record.controller.headSha ||
        ...
        if (
          ...
          throw new Error("Task6 committed journal is not exactly one legal intent append")
        }
        ...
        Object.defineProperty(permit, "toJSON", {
          ...hiddenMethod(() => {
            throw new TypeError("Delete permit capability cannot be serialized")
          }),
        })
    ...
    runDelete: async (permit, releaseId, operation) => {
      const record =
        permit !== null && typeof permit === "object" ? permitRecords.get(permit) : undefined
      const permitBinding =
        permit !== null && typeof permit === "object"
          ? DELETE_PERMIT_BINDINGS.get(permit)
          : undefined
      if (
        ...
        invalidate()
        throw new Error("Delete requires the valid one-use adapter-bound permit")
      }
      record.used = true
      permitBinding.used = true
          ...
          return await writePrivateEnvelope.withExclusiveTransaction(
            ...
            async (assertActiveLease) => {
              ...
              const verifyCommittedState = async () => {
                ...
                try {
                  const currentHead = await readPrivateEnvelope(journalHeadPath, 16 * 1024)
                  if (
                    !sameAuthenticatedPrivateRead(
                      currentHead,
              ...
              if (beforeRead > record.authorityExpiresAt) {
                throw new Error("Delete permit expired with its absolute npm authority")
              }
    ...
    createAuthorityCapability(rawGithub, facade) {
      ...
      Object.defineProperties(capability, {
        ...
        validateFacade: hiddenMethod((candidate) => {
          if (candidate !== facade) {
            invalidate()
            throw new TypeError("Adapter authority capability is not bound to this facade")
          }
        }),
        beginAuthorityCapture: hiddenMethod((input) =>
          beginAuthorityCapture(facade, rawGithub, input),
        ),
        beginTerminalRead: hiddenMethod((input) => beginTerminalRead(rawGithub, input)),
        sealWithoutTarget: hiddenMethod(() => {
          if (state !== "open" || activeRequests !== 0) {
            invalidate()
            throw new Error("Final network epoch cannot absorb a concurrent request")
          }
          terminal = {
            token: Object.freeze({}),
            deleteToken: Object.freeze({}),
        ...
        toJSON: hiddenMethod(() => {
          throw new TypeError("Adapter authority capability cannot be serialized")
        }),
      })
      return Object.freeze(capability)
    },
...
function authenticatedPrivateRead(value, expectedPath) {
  const authenticate = Object.getOwnPropertyDescriptor(readPrivateEnvelope, "authenticate")?.value
...
function sameAuthenticatedPrivateRead(actual, expected, expectedPath) {
  const actualIdentity = authenticatedPrivateRead(actual, expectedPath).identity
  const expectedIdentity = authenticatedPrivateRead(expected, expectedPath).identity
  return actual.equals(expected) && sameFileIdentity(actualIdentity, expectedIdentity)
}
...
function createLocalGitReader({ cwd, environment, run, ownerAdapters }) {
  ...
  return deepFreeze({
    async readState() {
      ...
      const branch = singleLine(branchResult.stdout, "Local Git symbolic branch")
      if (branch !== "main") throw new TypeError("Local Git symbolic branch must be main")
      ...
      const originMainSha = singleLine(originResult.stdout, "Local Git origin/main SHA")
      if (!isSha(originMainSha)) throw new TypeError("Local Git origin/main SHA is malformed")
      return deepFreeze({
        headSha: head,
        branch,
        porcelainStatus: "",
...
async function readNonterminalWorkflowRuns(http, token, now, query) {
  ...
  for (let page = 1; page <= pages; page += 1) {
    ...
    if (
      ...
      throw new TypeError("GitHub workflow-run total or record bound is malformed")
    }
    if (page === 1) {
    ...
    const expected =
      total === 0 ? 0 : page < pages ? query.perPage : total - (pages - 1) * query.perPage
    ...
    if (page < pages) {
      if (nextUrl === null)
        throw new TypeError("GitHub workflow-run pagination is missing Link next")
      if (nextUrl !== workflowRunsUrl(page + 1, query.perPage)) {
        throw new TypeError("GitHub workflow-run Link next URL is not the expected trusted page")
      }
    } else if (nextUrl !== null) {
      throw new TypeError("GitHub workflow-run pagination has an unexpected Link next")
    }
    for (const run of body.workflow_runs) {
      if (!isPlainRecord(run)) throw new TypeError("GitHub workflow run is malformed")
      const id = canonicalId(run.id)
      if (rawIds.has(id)) throw new TypeError("GitHub workflow runs contain a duplicate ID")
      rawIds.add(id)
  ...
  if (rawIds.size !== total) throw new TypeError("GitHub workflow-run raw total is inconsistent")
  return deepFreeze(
...
function normalizeAttestationResult(value) {
  if (containsProxy(value)) throw new TypeError("Attestation evidence is malformed")
  let snapshot
  try {
...
function containsProxy(value, seen = new Set()) {
  ...
  for (const key of Reflect.ownKeys(value)) {
    ...
    if (isEnumerableData(descriptor) && containsProxy(descriptor.value, seen)) return true
  }
  return false
}
...
function canonicalId(value) {
  ...
  if (typeof normalized !== "string" || !ID_PATTERN.test(normalized)) {
    throw new TypeError("Identifier must be a canonical positive decimal string")
  }
  return normalized
}
...
function requiredMember(value, name, label) {
  const object = ownDataObject(value, label)
  const descriptor = Object.getOwnPropertyDescriptor(object, name)
  if (!isEnumerableData(descriptor)) throw new TypeError(`${label} member ${name} is invalid`)
...
function assertExactIncidentConfirmation(value, proposedEnvelope) {
  if (typeof value !== "string" || hasControlCharacters(value)) {
    throw new TypeError("Task6 confirmation must be an exact control-free string")
  }
...
function assertTransitionAuthorityMatchesProposal(authority, proposal, targetReleaseId) {
  const expectedStage = targetReleaseId === DUPLICATE_IDS[0] ? "pre-delete-1" : "pre-delete-2"
  ...
  if (
    ...
    throw new Error("Task6 authority target or npm evidence differs from the proposal")
  }
}
...
function safeStringArray(value) {
  return (
    Array.isArray(value) &&
    value.length <= 64 &&
    value.every((entry) => typeof entry === "string" && !entry.includes("\u0000"))
  )
}
