...
export async function createDuplicateDraftConsolidationAdapters(options) {
  ...
  const authenticatedFetch = githubFetch(fetchImpl)
  const timestampNow = () => canonicalTimestamp(callClock(now))
  ...
  const github = createIncidentGitHubReader({
    reader: githubReader,
    fetchImpl: authenticatedFetch,
    token,
    now: timestampNow,
  ...
  const npm = deepFreeze({
    async observePackageVersion(input) {
      const result = await observePackageVersion(input)
      return deepFreeze(
  ...
  const runGh = async (args) => {
    ...
    await executeExact(run, "gh", args, {
      cwd,
  ...
  const attestations = deepFreeze({
    async verify(input) {
      return normalizeAttestationResult(await verifyAttestations(input))
    },
  })
  const writer = createExactDuplicateDeleteEffect({
    repository: REPOSITORY,
    apiOrigin: API_ORIGIN,
  ...
  return deepFreeze({ local, github, npm, attestations, writer })
}
...
async function readNonterminalWorkflowRuns(http, token, now) {
  ...
  for (let page = 1; page <= pages; page += 1) {
    ...
    if (page === 1) {
      ...
      if (pages > MAX_PAGES) throw new TypeError("GitHub workflow-run page bound exceeded")
    } else if (body.total_count !== total) {
      throw new TypeError("GitHub workflow-run total is unstable")
    }
    const expected = total === 0 ? 0 : page < pages ? 100 : total - (pages - 1) * 100
    ...
    if (page < pages) {
      ...
      if (nextUrl !== workflowRunsUrl(page + 1)) {
    ...
    for (const run of body.workflow_runs) {
      ...
      rawIds.add(id)
      if (!NONTERMINAL_STATUSES.has(run.status)) continue
      if (
...
function workflowRunsUrl(page) {
  return `${API_ORIGIN}/repos/${REPOSITORY}/actions/workflows/${encodeURIComponent(RELEASE_WORKFLOW)}/runs?per_page=100&page=${page}`
}
...
function rejectDuplicateIds(value, operation, code) {
  ...
  for (const record of result.value) {
    ...
    if (ids.has(id)) {
      return deepFreeze({ status: "ERROR", operation, httpStatus: result.httpStatus, code })
    }
    ids.add(id)
  }
  return result
...
async function resolveGhToken({ cwd, environment, run }) {
  let result
  try {
    result = await executeExact(run, "gh", ["auth", "token"], { cwd, env: environment })
  } catch {
