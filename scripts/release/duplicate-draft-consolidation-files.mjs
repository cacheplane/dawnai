...
const DEPENDENCY_FIELDS = Object.freeze(["effectiveUserId", "fileSystem", "randomUUID"])
const READ_FILE_SYSTEM_METHODS = Object.freeze(["lstat", "open"])
const WRITE_FILE_SYSTEM_METHODS = Object.freeze([...READ_FILE_SYSTEM_METHODS, "rename", "unlink"])
...
const LOCK_TIMESTAMP_PATTERN = /^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{3}Z$/u
const PROCESS_START_IDENTITY = `${process.pid}:${Math.trunc(Date.now() - process.uptime() * 1000)}`
...
export async function writePrivateEnvelope(filePath, bytes, dependencies, expectedCurrent) {
  ...
  if (lockTarget !== null && expectedCurrent !== undefined) {
    ...
    return writeEvidence(target, bytes, dependencies, POLICIES.private, expectedCurrent)
  }
  return withPrivateWriteLock(target, dependencies, () =>
    writeEvidence(target, bytes, dependencies, POLICIES.private, expectedCurrent),
  )
}
...
Object.defineProperty(readPrivateEnvelope, "authenticate", {
  value(value, expectedPath) {
    ...
    const provenance =
      value !== null && typeof value === "object" ? PRIVATE_READ_PROVENANCE.get(value) : undefined
    if (
      ...
      throw new TypeError("Authenticated private read provenance or byte digest is invalid")
    }
    return provenance
  },
  enumerable: false,
...
async function readEvidence(filePath, maximumBytes, dependencies, policy) {
  ...
  const parentChain = await captureParentChain(runtime.operations, parentPath, policy.label)
  ...
  let result = null
  try {
    await assertParentChainCurrent(runtime.operations, parentChain, policy.label)
    handle = await openNoFollow(runtime.operations, target, fsConstants.O_RDONLY, policy.label)
    const handleOperations = snapshotHandleOperations(handle, ["close", "read", "stat"])
    ...
    while (offset < bytes.byteLength) {
      ...
      const readResult = await handleOperations.read(bytes, offset, requested, offset)
      const bytesRead = resultCount(readResult, "bytesRead", requested, `${policy.label} read`)
      if (bytesRead === 0) break
      offset += bytesRead
      if (offset > maximum) throw new Error(`${capitalize(policy.label)} exceeds its byte bound`)
    }
  ...
  throwCombined(primaryError, closeErrors, `${capitalize(policy.label)} read failed during cleanup`)
  return result
}

async function writeEvidence(filePath, inputBytes, dependencies, policy, expectedCurrent) {
  const target = snapshotPath(filePath)
  ...
  if (typeof identifier !== "string" || !UUID_PATTERN.test(identifier)) {
    throw new TypeError(`${capitalize(policy.label)} temporary identity is invalid`)
  }
  ...
  try {
    ...
    while (offset < bytes.byteLength) {
      const requested = Math.min(IO_CHUNK_BYTES, bytes.byteLength - offset)
      const writeResult = await temporaryOperations.write(bytes, offset, requested, offset)
      ...
      if (bytesWritten === 0) {
        throw new Error(`${capitalize(policy.label)} temporary write made no progress`)
      }
      offset += bytesWritten
    }
    await temporaryOperations.sync()
    temporaryIdentity = await temporaryOperations.stat({ bigint: true })
    assertTemporaryFile(temporaryIdentity, runtime.effectiveUserId, policy, bytes.byteLength)
    await assertParentChainCurrent(runtime.operations, parentChain, policy.label)
  ...
  if (temporaryHandle !== undefined) {
    try {
      const operations = temporaryOperations ?? snapshotHandleOperations(temporaryHandle, ["close"])
...
function snapshotDependencies(dependencies, needsWrite) {
  ...
  for (const key of Reflect.ownKeys(dependencies)) {
    const descriptor =
      typeof key === "string" ? Object.getOwnPropertyDescriptor(dependencies, key) : undefined
    if (
      typeof key !== "string" ||
      ...
      throw new TypeError("Consolidation file dependencies contain an unsafe field")
    }
  ...
  const fileSystemMethods = needsWrite ? WRITE_FILE_SYSTEM_METHODS : READ_FILE_SYSTEM_METHODS
  for (const method of fileSystemMethods) {
    operations[method] = dataMethod(fileSystem, method, "filesystem").bind(fileSystem)
  }
  const effectiveUserId = values.effectiveUserId ?? defaultEffectiveUserId
  if (typeof effectiveUserId !== "function" || utilTypes.isProxy(effectiveUserId)) {
    throw new TypeError("Consolidation file effective-user dependency is unsafe")
  }
  ...
  if (needsWrite && (typeof randomUUID !== "function" || utilTypes.isProxy(randomUUID))) {
    throw new TypeError("Consolidation file random identity dependency is unsafe")
  }
...
function snapshotHandleOperations(handle, names) {
  ...
  const operations = Object.create(null)
  for (const name of names) operations[name] = dataMethod(handle, name, "descriptor").bind(handle)
...
function dataMethod(object, name, label) {
  ...
  let current = object
  while (current !== null) {
    if (utilTypes.isProxy(current)) throw new TypeError(`Consolidation file ${label} is unsafe`)
...
function snapshotWriteBytes(value, label) {
  if (
    ...
    throw new TypeError(`${capitalize(label)} bytes must be one owned byte array`)
  }
  if (value.byteLength > DUPLICATE_DRAFT_CONSOLIDATION_LIMITS.finalReceiptBytes) {
    throw new Error(`${capitalize(label)} exceeds its write byte bound`)
  }
  try {
    return Buffer.from(value)
  } catch {
    throw new TypeError(`${capitalize(label)} bytes could not be safely copied`)
  }
}
...
function snapshotExpectedCurrent(value, policy, target) {
  ...
  if (policy !== POLICIES.private) {
    throw new TypeError("Only private envelopes support authenticated replacement")
  }
...
async function withPrivateWriteLock(target, dependencies, operation) {
  ...
  try {
    ...
    operations = snapshotHandleOperations(handle, ["chmod", "close", "stat", "sync", "write"])
    await operations.chmod(PRIVATE_MODE)
    const lockBytes = canonicalPrivateLockBytes({
      lockTarget,
      nonce: identifier,
    })
    const written = await operations.write(lockBytes, 0, lockBytes.byteLength, 0)
    if (
      resultCount(written, "bytesWritten", lockBytes.byteLength, "lock write") !==
    ...
    assertTemporaryFile(identity, runtime.effectiveUserId, POLICIES.private, lockBytes.byteLength)
    ...
    result = await PRIVATE_TRANSACTION_CONTEXT.run(lease, async () => {
      try {
        return await operation(() => assertActivePrivateLease(lockTarget, lease))
      } finally {
        lease.active = false
      }
    })
  ...
  if (identity !== undefined) {
    try {
      ...
      if (!sameFileState(identity, current)) {
        throw new Error("Private envelope lock ownership changed; retained fail-closed")
      }
      ...
      if (!sameFileObject(identity, released)) {
        throw new Error("Private envelope lock release identity changed; retained fail-closed")
      }
...
async function quarantineProvablyDeadLock({ runtime, lockTarget, identifier, parentGuard }) {
  ...
  const operations = snapshotHandleOperations(handle, ["close", "read", "stat"])
  let closed = false
  try {
    ...
    if (!sameFileObject(finalCurrent, quarantined)) {
      throw new Error("Quarantined private lock identity changed during recovery")
    }
    await parentGuard.operations.sync()
  } finally {
...
function parseCanonicalPrivateLock(bytes, lockTarget) {
  ...
  const keys = record !== null && typeof record === "object" ? Reflect.ownKeys(record) : []
  if (
    record === null ||
...
function assertProvablyDeadProcess(record) {
  ...
  throw new Error("Existing private lock owner is live or its PID may have been reused")
}
...
function assertPrivateLockFile(status, expectedUserId) {
  ...
  if (status.uid !== expectedUserId) {
    throw new Error("Existing private lock must have the current effective owner")
  }
...
async function readExactHandleBytes(operations, size, label) {
  ...
  if (offset !== size) throw new Error(`${capitalize(label)} read is incomplete`)
  return bytes
}
...
function assertActivePrivateLease(lockTarget, expectedLease) {
  ...
  if (
    ...
    throw new Error("Authenticated journal access requires the active exact transaction lease")
  }
  return lease
}
...
function privateLockTarget(target) {
  ...
  if (basename !== JOURNAL_BASENAME && basename !== JOURNAL_HEAD_BASENAME) return null
  return path.join(path.dirname(target), `.${JOURNAL_BASENAME}.lock`)
}

function requireNoFollowSupport(needsWrite) {
  const required = [fsConstants.O_RDONLY, fsConstants.O_NOFOLLOW, fsConstants.O_DIRECTORY]
  if (needsWrite) {
  ...
  if (
    ...
    throw new TypeError("Consolidation evidence no-follow containment is unavailable")
  }
}
...
async function captureParentChain(operations, parentPath, label) {
  ...
  for (const component of components) {
    ...
    if (!status.isDirectory() || status.isSymbolicLink()) {
      throw new Error(`${capitalize(label)} has an unsafe parent symlink or component`)
    }
    chain.push(Object.freeze({ dev: status.dev, ino: status.ino, path: current }))
  }
  if (chain.length === 0) {
    ...
    chain.push(Object.freeze({ dev: status.dev, ino: status.ino, path: parsed.root }))
  }
...
async function assertParentChainCurrent(operations, chain, label) {
  for (const expected of chain) {
    ...
    if (
      ...
      throw new Error(`${capitalize(label)} parent path changed during containment`)
    }
  }
}

async function openParentGuard(operations, parentPath, chain, label, needsSync) {
  const handle = await openNoFollow(
    operations,
    parentPath,
  ...
  try {
    ...
    if (!status.isDirectory() || status.dev !== expected.dev || status.ino !== expected.ino) {
      throw new Error(`${capitalize(label)} parent path changed while it was opened`)
    }
...
async function openNoFollow(operations, filePath, flags, label, mode) {
  try {
    return await operations.open(filePath, flags | fsConstants.O_NOFOLLOW, mode)
  } catch (error) {
...
function assertSourcePolicy(status, expectedUserId, policy) {
  if (!status.isFile()) throw new Error(`${capitalize(policy.label)} must be a regular file`)
  if (status.nlink !== 1n) throw new Error(`${capitalize(policy.label)} must have exactly one link`)
  if (status.uid !== expectedUserId) {
    throw new Error(`${capitalize(policy.label)} must have the current effective owner`)
  }
  ...
  if ((mode & 0o7000) !== 0) {
    throw new Error(`${capitalize(policy.label)} mode must not contain special permission bits`)
  }
  if ((mode & 0o111) !== 0) {
    throw new Error(`${capitalize(policy.label)} mode must be nonexecutable`)
  }
  if ((mode & 0o022) !== 0) {
    throw new Error(`${capitalize(policy.label)} mode must not be group or other writable`)
  }
}
...
function assertTemporaryFile(status, expectedUserId, policy, expectedBytes) {
  ...
  if (status.size !== BigInt(expectedBytes)) {
    throw new Error(`${capitalize(policy.label)} temporary write is incomplete`)
  }
}

async function inspectExistingDestination(runtime, target, policy, parentChain, expected) {
  let handle
  try {
    handle = await openNoFollow(runtime.operations, target, fsConstants.O_RDONLY, policy.label)
  } catch (error) {
    if (errorCode(error) === "ENOENT" && (expected === null || expected?.absent === true))
      return null
    if (errorCode(error) === "ENOENT") {
      throw new Error(`${capitalize(policy.label)} authenticated current file is missing`, {
        cause: error,
      })
    }
    throw error
  ...
  try {
    ...
    if (expected?.absent === true) {
      throw new Error(`${capitalize(policy.label)} appeared before authenticated creation`)
    }
    ...
    if (expected !== null) {
      ...
      while (offset < observed.byteLength) {
        const requested = Math.min(IO_CHUNK_BYTES, observed.byteLength - offset)
        const readResult = await operations.read(observed, offset, requested, offset)
    ...
    if (current.isSymbolicLink() || !sameFileState(status, current)) {
      throw new Error(`${capitalize(policy.label)} destination changed during inspection`)
    }
    assertSourcePolicy(current, runtime.effectiveUserId, policy)
    await assertParentChainCurrent(runtime.operations, parentChain, policy.label)
...
async function assertDestinationUnchanged(runtime, target, expected, policy) {
  ...
  if (expected === null || current.isSymbolicLink() || !sameIdentityRecord(expected, current)) {
    throw new Error(`${capitalize(policy.label)} destination changed before atomic replacement`)
  }
  assertSourcePolicy(current, runtime.effectiveUserId, policy)
}
...
async function assertTemporaryPathCurrent(
  ...
  if (current.isSymbolicLink() || !sameFileState(expected, current)) {
    throw new Error(`${capitalize(policy.label)} temporary path changed before publication`)
  }
...
async function verifyPublishedBytes(
  ...
  if (
    ...
    throw new Error(`${capitalize(policy.label)} changed during atomic publication`)
  }
  ...
  while (offset < observedBytes.byteLength) {
    const requested = Math.min(IO_CHUNK_BYTES, observedBytes.byteLength - offset)
    const readResult = await descriptor.read(observedBytes, offset, requested, offset)
    const bytesRead = resultCount(
      readResult,
      "bytesRead",
      requested,
  ...
  if (
    ...
    throw new Error(`${capitalize(policy.label)} bytes changed during atomic publication`)
  }
...
function errorCode(error) {
  if (error === null || typeof error !== "object" || utilTypes.isProxy(error)) return undefined
  let current = error
  while (current !== null) {
...
function throwCombined(primaryError, secondaryErrors, message) {
  ...
  if (secondaryErrors.length > 1) throw new AggregateError(secondaryErrors, message)
