...
export async function captureNpmInventory(input) {
  const value = exactInput(input, ["stage", "candidate", "npm", "now"], "npm inventory input")
  const stage = dataString(value, "stage", "npm inventory stage")
  if (!NPM_STAGES.has(stage)) throw new TypeError("npm inventory stage is invalid")
  const candidate = normalizeCandidate(dataValue(value, "candidate", "npm candidate"))
  ...
  for (const name of CANONICAL_RELEASE_PACKAGE_ORDER) {
    ...
    if (
      ...
      throw new Error("npm package-version absence evidence is incomplete or ambiguous")
    }
  ...
  assertTimestampOrder(packages.at(-1).observedAt, completedAt, "npm inventory completion")
  if (Date.parse(completedAt) - Date.parse(startedAt) > MAXIMUM_NPM_OPERATION_MS) {
...
export async function captureConsolidationAuthorityCore(input, authorityCapability) {
  const context = normalizeCaptureInput(input, authorityCapability)
  ...
  if (context.targetReleaseId !== stageRule.targetReleaseId) {
    throw new Error("Authority target is not the approved current next duplicate")
  }
  ...
  const localState = snapshotPlain(await adapters.local.readState(), "local checkout state")
  assertExactKeys(
    localState,
  ...
  if (
    ...
    throw new Error("Local checkout must be clean symbolic main at exact origin/main")
  }

  const repository = snapshotPlain(await adapters.github.getRepository(), "GitHub repository")
  const actor = snapshotPlain(await adapters.github.getAuthenticatedUser(), "GitHub actor")
  const githubMainSha = await adapters.github.getDefaultBranchSha()
  const workflow = snapshotPlain(await adapters.github.getWorkflowState(), "Release workflow")
  ...
  const nonterminalRuns = normalizeNonterminalRunRead(nonterminalRunRead, WORKFLOW_RUN_QUERY)
  const annotatedTag = snapshotPlain(
    await adapters.github.getAnnotatedTag({ name: CANDIDATE.tag }),
    "annotated tag",
  )
  const releaseEnvelope = snapshotPlain(await adapters.github.listReleases(), "Release enumeration")
  const listedReleases = presentValue(releaseEnvelope, "releases")
  if (!Array.isArray(listedReleases)) throw new TypeError("Release enumeration is malformed")
  ...
  if (
    ...
    throw new Error("Controller HEAD, origin/main, and GitHub main SHAs must match")
  }
  if (
    localState.headSha !== proposal.controller.headSha ||
    localState.originMainSha !== proposal.controller.originMainSha
  ) {
    throw new Error("Current controller SHA authority differs from the proposal")
  }
  ...
  if (stageRule.targetReleaseId !== null) {
    const targetIndex = releases.findIndex(({ id }) => id === stageRule.targetReleaseId)
    if (targetIndex < 0) throw new Error("Authority target is absent from the Release list")
    ...
    try {
      targetRead = await captureDirectTargetRead({
        ...
        releaseId: stageRule.targetReleaseId,
        role: "duplicate",
        expectedEvidence: proposal.releases.find(({ id }) => id === stageRule.targetReleaseId),
    ...
    if (!isDeepStrictEqual(targetRead.evidence, releases[targetIndex])) {
      throw new Error("Direct target evidence disagrees with the complete Release list")
    }
  ...
  try {
    ...
    if (transitionBoundary === null) throw new Error("Task6 transition boundary was not registered")
...
export function assertFreshWriterAuthority(authority, proposal, now) {
  ...
  const age =
    Date.parse(currentTimestamp) - Date.parse(normalizedAuthority.npmInventory.completedAt)
  if (age < 0) throw new Error("Writer authority contains a future npm observation")
...
function normalizeCaptureInput(input, authorityCapability) {
  ...
  if (!AUTHORITY_STAGES.has(stage)) throw new TypeError("Authority stage is invalid")
  const target = dataValue(value, "targetReleaseId", "authority target")
  const targetReleaseId = target === null ? null : canonicalId(target, "authority target")
  const rawAdapters = dataValue(value, "adapters", "consolidation adapters")
  const adapters = bindAdapterFacade(rawAdapters, authorityCapability)
  return {
    stage,
    proposal: snapshotPlain(dataValue(value, "proposal", "proposal"), "proposal"),
    targetReleaseId,
    adapters,
  }
}
...
function normalizeProposal(value) {
  return deepFreeze(
    createConsolidationEnvelope("proposed", snapshotPlain(value, "proposal")).record,
  )
}
...
function assertProductionProposal(proposal) {
  if (
    ...
    throw new Error("Proposal does not bind the approved production incident identity")
  }
}

function assertRepositoryAuthority({ repository, actor, proposal }) {
  assertExactKeys(repository, ["name", "id", "defaultBranch"], "GitHub repository")
...
function normalizeNonterminalRunRead(value, executedQuery) {
  ...
  const echoedQuery = snapshotPlain(value.query, "nonterminal workflow-run query echo")
  assertExactKeys(
    echoedQuery,
  ...
  if (!isDeepStrictEqual(echoedQuery, executedQuery)) {
    throw new Error("Nonterminal workflow-run query echo differs from the executed query")
  }
...
function normalizeWorkflowAuthority({ workflow, nonterminalRuns, query, observedAt }) {
  assertExactKeys(workflow, ["workflowId", "path", "state"], "Release workflow")
  if (
    !ID_PATTERN.test(workflow.workflowId) ||
    workflow.path !== WORKFLOW_PATH ||
    workflow.state !== "disabled_manually"
  ) {
    throw new Error("Release workflow authority is missing, malformed, or active")
  }
...
function normalizeAnnotatedTag(value) {
  ...
  return {
    ...
    observedAt: canonicalTimestamp(value.observedAt, "annotated tag observation"),
  }
}
...
function assertStableTagAndWorkflow(tag, workflow, proposal) {
  for (const field of ["name", "objectSha", "targetSha", "objectType"]) {
    if (tag[field] !== proposal.annotatedTag[field]) {
      throw new Error("Current annotated tag differs from the approved proposal")
    }
  }
  for (const field of ["workflowId", "path", "state"]) {
    if (workflow[field] !== proposal.workflowAuthority[field]) {
      throw new Error("Current Release workflow differs from the approved proposal")
    }
  }
}
...
async function verifyCurrentPayloadDownloads(releases, github) {
  ...
  for (const release of releases) {
    for (const asset of release.assets) {
      downloads += 1
      if (downloads > 135) {
        throw new Error("Current Release payload exceeded the download-count bound")
      }
      ...
      if (
        ...
        throw new Error("Current Release asset download is unavailable or ambiguous")
      }
      const maximumCharacters = Math.ceil(asset.size / 3) * 4
      if (envelope.contentBase64.length > maximumCharacters) {
        throw new Error("Current Release asset download exceeds its declared size")
      }
      ...
      if (
        ...
        throw new Error("Current Release asset bytes differ from the proven proposal")
      }
    }
  }
}
...
function normalizeAuthorityStage(value) {
  ...
  const event =
    stage === "final"
      ? {
          ...
          recordedAt: ownDataString(value, "observedAt", "authority observation"),
  ...
  return deepFreeze(canonicalEventEnvelope(event, null).event.payload.authority)
}
...
function assertAuthorityAgainstProposal(authority, proposal) {
  ...
  assertStableTagAndWorkflow(authority.annotatedTag, authority.workflowAuthority, proposal)
  if (
    ...
    throw new Error("Authority npm inventory does not bind the proposal candidate")
  }
  if (
    !isDeepStrictEqual(
  ...
  for (const release of authority.releases) {
    ...
    if (proposed === undefined) throw new Error("Authority Release is absent from the proposal")
  ...
  if (rule.targetReleaseId === null) {
    ...
    throw new Error("Authority direct target is not the approved current next duplicate")
  }
}

function assertAuthorityTemporalOrder(authority, ceiling) {
  const ceilingTimestamp = canonicalTimestamp(ceiling, "authority time ceiling")
  ...
  for (const observation of inventory.packages) {
    assertTimestampOrder(previousNpmTimestamp, observation.observedAt, "npm observation")
    previousNpmTimestamp = observation.observedAt
  }
  assertTimestampOrder(previousNpmTimestamp, inventory.completedAt, "npm inventory")
  ...
  for (const release of authority.releases) {
    assertTimestampOrder(release.createdAt, release.updatedAt, "Release service observation")
    assertTimestampOrder(release.updatedAt, ceilingTimestamp, "Release service observation")
    for (const asset of release.assets) {
      assertTimestampOrder(asset.createdAt, asset.updatedAt, "asset service observation")
      assertTimestampOrder(asset.updatedAt, ceilingTimestamp, "asset service observation")
    }
  }
  if (authority.targetRead !== null) {
    assertTargetReadChronology(authority, inventory.completedAt)
  } else {
    assertTimestampOrder(inventory.completedAt, authority.observedAt, "authority observation phase")
  }
}
...
function createNetworkEpoch({
  ...
  Object.defineProperties(capability, {
    consume: {
      ...
      writable: false,
      async value(input) {
        if (consumed) throw new Error("Network epoch has already been consumed")
        consumed = true
        const value = exactInput(
          input,
          [
        ...
        const consumedProposal = normalizeProposal(dataValue(value, "proposal", "epoch proposal"))
        const proposedEnvelope = createConsolidationEnvelope("proposed", consumedProposal)
        const confirmation = dataString(value, "confirmation", "operator confirmation")
        ...
        if (
          ...
          throw new Error("Network epoch authority, proposal, or target binding changed")
        }
        const intentPath = dataString(value, "intentPath", "journal intent path")
        if (intentPath !== adapterEpoch.journalPath) {
          throw new Error("Journal intent path is not the adapter-owned private path")
        }
        ...
        if (
          ...
          expectedCurrent.record.confirmationSha256 !== expectedConfirmationSha256 ||
          expectedCurrentState.controllerSha !== consumedAuthority.controller.headSha
        ) {
          throw new Error("Current journal does not bind proposal confirmation and controller")
        }
        return writePrivateEnvelope.withExclusiveTransaction(intentPath, async () => {
          let currentBytes
          try {
          ...
          if (
            ...
            throw new Error("Current journal file differs from the authenticated expected history")
          }
          ...
          if (
            ...
            !isDeepStrictEqual(expectedCurrentState.lastAuthority, consumedAuthority)
          ) {
            throw new Error("Current journal is not the exact legal delete-authority predecessor")
          }
          const beforeWriteTimestamp = readTrustedEpochClock(adapterEpoch)
          assertFreshWriterAuthority(consumedAuthority, consumedProposal, beforeWriteTimestamp)
          assertAdapterEpochSealed(adapterEpoch)
          ...
          const intentBytes = canonicalConsolidationEnvelopeBytes("journal", intentEnvelope)
          try {
            await writePrivateEnvelope(intentPath, intentBytes, undefined, currentBytes)
          } catch {
            throw new Error(
          ...
          try {
            ...
            if (!committedJournal.equals(intentBytes)) {
              throw new Error("Committed journal bytes differ from the legal intent")
            }
            const parsedCommitted = parseConsolidationJournal(committedJournal)
            if (!isDeepStrictEqual(parsedCommitted, intentEnvelope)) {
              throw new Error("Committed journal envelope differs from the legal intent")
            }
            const committedHeadBytes = canonicalJournalHeadBytes(intentPath, intentEnvelope)
            ...
            const committedHead = await readPrivateEnvelope(journalHeadPath, 16 * 1024)
            if (!committedHead.equals(committedHeadBytes)) {
              throw new Error("Committed journal head differs from the legal intent")
            }
            const completedTimestamp = readTrustedEpochClock(adapterEpoch)
            assertFreshWriterAuthority(consumedAuthority, consumedProposal, completedTimestamp)
            adapterEpoch.validate()
...
async function reconcileJournalHead({ journalHeadPath, journalPath, journal }) {
  ...
  try {
    ...
    if (!isExactJournalGenesis(journal)) {
      throw new Error("Missing durable journal head may bootstrap only exact operation genesis")
    }
    await writePrivateEnvelope(
      journalHeadPath,
  ...
  if (!journalHeadMatches(head, predecessor)) {
    throw new Error("Journal is not one legal append ahead of its durable head")
  }
  await writePrivateEnvelope(
    journalHeadPath,
    canonicalJournalHeadBytes(journalPath, journal),
...
function parseJournalHead(bytes, expectedJournalPath) {
  ...
  if (
    ...
    canonicalTimestamp(value.updatedAt, "journal head timestamp") !== value.updatedAt ||
...
function hasErrorCode(error, code) {
  if (error !== null && typeof error === "object") {
    ...
    if (Array.isArray(error.errors) && error.errors.some((entry) => hasErrorCode(entry, code)))
      return true
  }
  return false
}
...
function bindAdapterFacade(value, authorityCapability) {
  if (
    !isPlainObject(value) ||
    ...
    throw new TypeError("consolidation adapters must be an immutable plain non-proxy facade")
  }
  ...
  const captureDescriptor = descriptors.captureConsolidationAuthority
  if (
  ...
  return Object.freeze({
    local: bindBoundary(descriptors.local.value, ["readState"], "local Git reader"),
    github: bindBoundary(
    ...
    npm: bindBoundary(descriptors.npm.value, ["observePackageVersion"], "npm authority reader"),
    attestations: bindBoundary(
      descriptors.attestations.value,
      ["verify"],
      "attestation authority reader",
    ),
    writer: bindBoundary(descriptors.writer.value, ["deleteDuplicate"], "duplicate delete writer"),
    authorityEpoch,
...
function bindAuthorityCapability(value, facade) {
  ...
  const validateFacade = hiddenDataFunction(value, "validateFacade", "adapter facade validator")
  ...
  const sealWithoutTarget = hiddenDataFunction(value, "sealWithoutTarget", "final-stage capability")
  try {
...
function bindFinalEpoch(value) {
  ...
  const validate = hiddenDataFunction(value, "validate", "final epoch validator")
  const bindAuthority = hiddenDataFunction(value, "bindAuthority", "captured authority binder")
  return Object.freeze({
    now: () => now.call(value),
...
function bindSealedEpoch(value) {
  ...
  const validate = hiddenDataFunction(value, "validate", "sealed epoch validator")
  const bindAuthority = hiddenDataFunction(value, "bindAuthority", "captured authority binder")
  return Object.freeze({
    now: () => now.call(value),
...
function bindBoundary(value, methods, label) {
  if (!isPlainObject(value) || utilTypes.isProxy(value) || !Object.isFrozen(value)) {
...
function assertExactKeys(value, expected, label, { allowFunctions = false } = {}) {
  if (!isPlainObject(value) || utilTypes.isProxy(value))
...
function snapshotPlain(value, label) {
  ...
  if (Array.isArray(value)) {
    ...
    if (symbols.length !== 0) throw new TypeError(`${label} contains symbol properties`)
  ...
  if (value !== null && typeof value === "object") {
    ...
    for (const [key, descriptor] of Object.entries(descriptors)) {
      if (
        ...
        throw new TypeError(`${label} contains hidden, accessor, or function properties`)
      }
