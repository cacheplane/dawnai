...
const CONVERGENCE_BACKOFF_MS = Object.freeze([
	...
	try {
		...
		for (;;) {
			...
			if (state.currentTargetReleaseId !== context.targetReleaseId) {
				throw new Error(
					"Journal current target differs from the requested target",
				);
			}

			if (
				["delete-intent", "delete-outcome", "resume-absent"].includes(
					state.phase,
				)
			) {
				const observation = await observeConvergence(context, state);
				const resolved = await resolveConvergence(
					context,
					current.journal,
					state,
					observation,
...
function normalizeDeletionInvocation(input, dependencies) {
	...
	if (
		...
		path.basename(value.journalPath) !==
	...
	if (
		...
		(typeof runtime.faultAt !== "string" ||
			!FAULT_BOUNDARIES.has(runtime.faultAt))
	) {
...
function assertDeletionBinding(context, journal) {
	...
	const confirmationSha256 = createHash("sha256")
		.update(context.confirmation, "utf8")
		.digest("hex");
	if (
...
async function resolveConvergence(context, journal, state, observation) {
	if (observation.classification === "absent") {
		...
		if (currentState.phase === "delete-intent") {
			if (
				nextResumeAction(currentState, { classification: "absent" }) !==
				"reconcile-absence"
			) {
		...
		if (
			currentState.phase !== "resume-absent" &&
	...
	const liveTarget = {
		...
		...(state.phase === "delete-outcome"
			? { observations: observation.attempts }
			: {}),
	};
...
async function observeConvergence(context, state) {
	...
	for (let attempt = 1; attempt <= MAXIMUM_CONVERGENCE_ATTEMPTS; attempt += 1) {
		...
		if (directStatus === "PRESENT") {
			...
			if (
				present.operation !== "release" ||
				present.httpStatus !== 200 ||
				present.code !== null
			) {
			...
			const listed = classified.selected.find(
				({ id }) => String(id) === context.targetReleaseId,
			);
...
function validateRemainingConvergence(rawReleases, context) {
	for (const rawRelease of rawReleases) {
		...
		const expected = context.proposal.record.releases.find(
			({ id }) => id === releaseId,
		);
		if (expected === undefined) {
...
function assertRawReleaseSemanticEqualsProposal(rawRelease, expected) {
	...
	if (
		...
		!isDeepStrictEqual(semantic, expected.semantic)
	) {
...
function nextDeletionStage(state) {
	return state.currentTargetReleaseId === DUPLICATES[0]
		? "pre-delete-2"
		: "final";
}

async function appendDurableEvent(
	context,
	expectedJournal,
	type,
	payload,
	...
	return writePrivateEnvelope.withExclusiveTransaction(
		...
		async () => {
			...
			const appended = appendJournalEvent(
				current.journal,
				type,
				payload,
				timestamp,
			...
			const headBytes = canonicalJournalHeadBytes(
				context.journalPath,
				appended,
			);
			await writePrivateEnvelope(
			...
			const durableHead = await readPrivateEnvelope(
				current.headPath,
				16 * 1024,
			);
...
function monotoneEventTimestamp(state, value) {
	...
	return authorityTime !== undefined &&
		Date.parse(value) < Date.parse(authorityTime)
		? authorityTime
		: value;
}
