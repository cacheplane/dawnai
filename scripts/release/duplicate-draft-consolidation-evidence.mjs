...
export async function inspectEquivalentDrafts(input) {
	...
	const byId = new Map(
		managed.drafts.map((entry) => [
			canonicalId(entry.release.id, "Release id"),
			entry,
		]),
	);
	if (byId.size !== 3 || orderedIds.some((id) => !byId.has(id))) {
		throw new Error(
			"Managed Release roles do not match the approved exact IDs and order",
		);
	}
	...
	for (let index = 1; index < hydration.length; index += 1) {
		if (
			...
			!isDeepStrictEqual(
				hydration[index].baseAssetSet,
				hydration[0].baseAssetSet,
			) ||
			!isDeepStrictEqual(
				hydration[index].verifiedSubjects,
				hydration[0].verifiedSubjects,
			)
		) {
			throw new Error(
				"Candidate draft production payload proofs are not equal",
			);
		}
	}
	if (
		...
		throw new Error(
			"Duplicate draft download aggregate exceeded its exact bound",
		);
	}
...
export async function captureDirectTargetRead(input) {
	...
	if (typeof now !== "function")
		throw new TypeError("Direct target clock is invalid");
	...
	const releaseGetCompletedAt = canonicalTimestamp(
		now(),
		"Release GET completion",
	);
	...
	const assetsListCompletedAt = canonicalTimestamp(
		now(),
		"Asset list completion",
	);
...
export function semanticReleaseProjection(value) {
	...
	return deepFreeze(
		snapshotPlain(evidence.semantic, "Release semantic projection"),
	);
}
...
function snapshotInspectionInput(input) {
	...
	if (
		...
		!value.duplicateIds.every(
			(id, index) => id === APPROVED_DUPLICATE_IDS[index],
		)
	) {
...
function candidateReleases(releases, candidate) {
	...
	for (const [index, source] of releases.entries()) {
		...
		if (releaseIds.has(id))
			throw new Error("GitHub Release enumeration contains duplicate IDs");
		releaseIds.add(id);
		let marker = null;
		try {
		...
		if (!markerMatches && !exactTagMatches) {
			if (
				[APPROVED_SURVIVOR_ID, ...APPROVED_DUPLICATE_IDS].includes(id) &&
				marker === null
			) {
				throw new Error(
					"Approved managed candidate draft has a malformed Release marker",
				);
			}
			continue;
		}
	...
	for (const [releaseIndex, release] of releases.entries()) {
		...
		for (const asset of release.assets) {
			...
			if (ids.has(id) || names.has(name)) {
				throw new Error(
					"Release asset enumeration contains a duplicate identity",
				);
			}
			ids.add(id);
			names.add(name);
		...
		aggregate = checkedAdd(
			aggregate,
			releaseBytes,
			"Aggregate Release escrow payload",
		);
	}
	if (
		...
		throw new Error(
			"Duplicate draft aggregate payload or download limit was exceeded",
		);
	}
}
...
async function hydrateRelease({
	...
	if (
		...
		throw new Error(
			"Release assets do not match the exact canonical 45-name set",
		);
	}
	...
	for (const rawAsset of release.assets) {
		...
		counter.bytes = checkedAdd(
			counter.bytes,
			bytes.byteLength,
			"Downloaded Release payload",
		);
	...
	if (base.sha256 !== parsedMarker.baseAssetSetSha256) {
		throw new Error(
			"Canonical base asset set digest does not match the Release marker",
		);
	}
	...
	if (
		...
		throw new Error(
			"Verified attestation anchor does not match the canonical escrow",
		);
	}
	if (
		release.body !== canonicalReleaseBody({ marker: parsedMarker, manifest })
	) {
	...
	return {
		...
		verifiedSubjects: attestationSet.subjects.map(
			({ subjectName, subjectSha256 }) => ({
				name: subjectName,
...
function parseCandidateMarker(body, candidate, label) {
	...
	if (
		...
		throw new Error(
			`${label} marker does not bind the approved ESCROWED candidate`,
		);
	}
	return marker;
}
...
function validateRawReleasePolicy(release, candidate) {
	if (
		...
		throw new Error(
			"Managed candidate Release is not the expected mutable draft",
		);
	}
...
function parseRawAsset(value) {
	...
	if (value.state !== "uploaded")
		throw new Error("Release asset state is not uploaded");
	...
	return {
		...
		label:
			value.label === null ? null : stringValue(value.label, "Asset label"),
		state: value.state,
...
function parseReleaseSemantic(value) {
	...
	if (sha256(Buffer.from(body, "utf8")) !== bodySha256) {
		throw new TypeError(
			"Release body digest does not match its canonical bytes",
		);
	}
	return {
		name: nonemptyString(value.name, "Release name"),
		targetCommitish: exactString(
...
function parseAssetEvidence(value) {
	...
	if (value.state !== "uploaded")
		throw new TypeError("Asset evidence state is not uploaded");
...
function assertThreeWayParity(releases) {
	...
	for (const release of releases.slice(1)) {
		if (
			!isDeepStrictEqual(
				semanticReleaseProjection(release),
				releaseProjection,
			) ||
			!isDeepStrictEqual(
			...
			throw new Error(
				"Managed candidate Release or asset semantic parity failed",
			);
		}
	}
}
...
function exactDownloadBytes(envelope, maximumBytes) {
	...
	if (value.contentBase64.length > maximumBase64Characters) {
		throw new Error(
			"GitHub asset download base64 exceeds its declared-size bound",
		);
	}
