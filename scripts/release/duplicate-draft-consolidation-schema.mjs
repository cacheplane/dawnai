...
if (
  ...
  throw new Error("Duplicate-draft consolidation limits do not preserve required headroom")
}
...
const AUTHORITY_EVENT_TYPES = new Set(["delete-authority-observed", "final-authority-observed"])
...
const PERFORM_STAGES = new Set(["perform-initial", "pre-delete-1", "pre-delete-2", "final"])
...
export function createConsolidationEnvelope(kind, record) {
  return withCanonicalBudget(kindLimit(kind), `${kind} envelope`, () => {
    ...
    assertWithinKindLimit(kind, Buffer.byteLength(`${JSON.stringify(envelope)}\n`, "utf8"))
    return envelope
  })
}
...
export function parseConsolidationEnvelope(kind, bytes) {
  ...
  if (!(bytes instanceof Uint8Array)) throw new TypeError("Envelope bytes must be a byte array")
  if (bytes.byteLength > maximum) throw new Error(`${kind} envelope exceeds its byte limit`)
...
export function canonicalRecordSha256(record) {
  ...
  if (source === undefined) throw new TypeError("Record is not JSON serializable")
...
export function canonicalEventEnvelope(event, previousEventSha256) {
  return withCanonicalBudget(
    ...
    () => {
      ...
      const normalizedEvent = normalizeEvent(event, expectedSequence, previousEventSha256)
      return {
...
export function parseJournalEventEnvelope(value, expectedSequence, previousEventSha256) {
  return withCanonicalBudget(
    journalEventEnvelopeBudget(ownDataDiscriminator(ownDataDiscriminator(value, "event"), "type")),
    "journal event envelope",
    () => {
      value = assertExactFields(value, EVENT_ENVELOPE_FIELDS, "Journal event envelope")
      const event = normalizeEvent(value.event, expectedSequence, previousEventSha256)
      const eventSha256 = assertSha256(value.eventSha256, "Journal event digest")
      if (eventSha256 !== canonicalRecordSha256(event)) {
        throw new TypeError("Journal event digest does not match its canonical event")
      }
...
function normalizeEnvelope(kind, value) {
  return withCanonicalBudget(kindLimit(kind), `${kind} envelope`, () => {
    ...
    const recordSha256 = assertSha256(value.recordSha256, `${kind} record digest`)
    if (recordSha256 !== canonicalRecordSha256(record)) {
      throw new TypeError(`${kind} envelope digest does not match its canonical record`)
    }
    const normalized = { record, recordSha256 }
    assertCanonicalValueByteLength(normalized, kindLimit(kind), `${kind} envelope`)
    return normalized
  })
}
...
function normalizeProposedRecord(value) {
  ...
  const npmInventories = assertArray(value.npmInventories, "Proposed npm inventories", {
    exactLength: 2,
  ...
  const inspectedAt = assertTimestamp(value.inspectedAt, "Proposed inspection timestamp")
  ...
  for (const inventory of npmInventories) {
    if (inventory.packages.some(({ version }) => version !== candidate.version)) {
      throw new TypeError("Proposed npm observations must identify the candidate version")
    }
  }
  assertIdentityContract(candidate, roles, confirmation)
  if (annotatedTag.name !== candidate.tag || annotatedTag.targetSha !== candidate.commitSha) {
    throw new TypeError("Annotated tag does not identify the proposed candidate")
  }
  ...
  if (
    ...
    throw new TypeError("Proposed release evidence exceeds the asset download limit")
  }
...
function normalizeJournalRecord(value) {
  ...
  const confirmationSha256 = assertSha256(value.confirmationSha256, "Journal confirmation digest")
  const deletionOrder = normalizeIdentityArray(
    value.deletionOrder,
    "Journal deletion order",
    DUPLICATE_DRAFT_CONSOLIDATION_LIMITS.maximumTargets,
  )
  if (deletionOrder.length !== DUPLICATE_DRAFT_CONSOLIDATION_LIMITS.maximumTargets) {
    throw new TypeError("Journal deletion order must contain exactly two targets")
  }
  assertArrayEqual(deletionOrder, APPROVED_DUPLICATE_IDS, "Approved journal deletion order")
  const rawEvents = assertArray(value.events, "Journal events", {
    maximumLength: Math.floor(DUPLICATE_DRAFT_CONSOLIDATION_LIMITS.journalBytes / 64),
  })
  const events = []
  let previousEventSha256 = null
  for (let index = 0; index < rawEvents.length; index += 1) {
    const envelope = parseJournalEventEnvelope(rawEvents[index], index + 1, previousEventSha256)
  ...
  for (const { event } of events) {
    ...
    if (count > DUPLICATE_DRAFT_CONSOLIDATION_LIMITS.maximumDeleteAttempts) {
      throw new TypeError("Journal target exceeds the maximum number of delete intents")
    }
  ...
  if (events.length > 0) {
    ...
    if (
      ...
      throw new TypeError("Journal must begin with its bound operation-started event")
    }
    assertArrayEqual(first.payload.deletionOrder, deletionOrder, "Journal operation deletion order")
  }
  const updatedAt = assertTimestamp(value.updatedAt, "Journal update timestamp")
  return {
    schemaVersion: 1,
    repository,
    candidate,
...
function normalizeFinalRecord(value) {
  ...
  const proposedEnvelope = normalizeEnvelope("proposed", value.proposedEnvelope)
  const journalEnvelope = normalizeEnvelope("journal", value.journalEnvelope)
  if (journalEnvelope.record.proposedRecordSha256 !== proposedEnvelope.recordSha256) {
    throw new TypeError("Final receipt journal does not bind the embedded proposal")
  }
  ...
  if (finalAuthority.stage !== "final" || finalAuthority.targetRead !== null) {
    throw new TypeError("Final receipt authority must be the final no-target-read stage")
  }
  ...
  if (
    ...
    throw new TypeError("Final survivor must exactly match the final authority survivor")
  }
  const completedAt = assertTimestamp(value.completedAt, "Final receipt completion timestamp")
  return {
    schemaVersion: 1,
    proposedEnvelope,
    journalEnvelope,
...
function normalizeRepository(value) {
  value = assertExactFields(value, ["name", "id", "defaultBranch", "actor"], "Repository")
  return {
    name: assertNonemptyString(value.name, "Repository name"),
    id: assertId(value.id, "Repository id"),
    defaultBranch: assertNonemptyString(value.defaultBranch, "Repository default branch"),
...
function normalizeController(value) {
  value = assertExactFields(value, ["headSha", "originMainSha", "githubMainSha"], "Controller")
  const headSha = assertGitSha(value.headSha, "Controller head SHA")
  const originMainSha = assertGitSha(value.originMainSha, "Controller origin/main SHA")
  const githubMainSha = assertGitSha(value.githubMainSha, "Controller GitHub main SHA")
...
function normalizeCandidate(value) {
  value = assertExactFields(value, ["version", "commitSha", "tag"], "Candidate")
  const version = assertMatchingString(value.version, VERSION_PATTERN, "Candidate version")
  const commitSha = assertGitSha(value.commitSha, "Candidate commit SHA")
  const tag = assertNonemptyString(value.tag, "Candidate tag")
  if (tag !== `v${version}`) throw new TypeError("Candidate tag must match its version")
...
function normalizeRoles(value) {
  ...
  if (
    ...
    throw new TypeError("Release roles must identify one survivor and two ordered duplicates")
  }
  assertArrayEqual(duplicates, APPROVED_DUPLICATE_IDS, "Approved duplicate Release order")
...
function normalizeConfirmation(value) {
  ...
  const template = assertNonemptyString(value.template, "Confirmation template")
  if ((template.match(/<64-lowercase-hex-digest>/gu) ?? []).length !== 1) {
    throw new TypeError("Confirmation template must retain the digest placeholder")
  }
  return {
    version: assertMatchingString(value.version, VERSION_PATTERN, "Confirmation version"),
...
function normalizeAnnotatedTag(value) {
  ...
  const objectType = assertNonemptyString(value.objectType, "Annotated tag object type")
  if (objectType !== "tag") throw new TypeError("Candidate tag must be an annotated tag object")
  return {
    ...
    objectType,
    observedAt: assertTimestamp(value.observedAt, "Annotated tag observation timestamp"),
  }
}
...
function normalizeWorkflowAuthority(value) {
  ...
  if (query.perPage !== 100 || query.maximumPages !== 100) {
    throw new TypeError("Workflow query bounds must be the reviewed 100-by-100 values")
  }
  const nonterminalRuns = assertArray(value.nonterminalRuns, "Nonterminal workflow runs", {
    exactLength: 0,
  }).map(normalizeWorkflowRun)
  if (nonterminalRuns.length !== 0) throw new TypeError("Nonterminal workflow runs must be empty")
  if (value.path !== ".github/workflows/release.yml" || value.state !== "disabled_manually") {
    throw new TypeError("Release workflow must remain disabled at its canonical path")
  }
  return {
    ...
    nonterminalRuns,
    observedAt: assertTimestamp(value.observedAt, "Workflow authority observation timestamp"),
  }
}
...
function normalizeWorkflowRun(value) {
  ...
  return {
    ...
    headBranch: assertNonemptyString(value.headBranch, "Workflow run head branch"),
  }
}
...
function normalizeNpmInventory(value) {
  ...
  return {
    ...
    startedAt: assertTimestamp(value.startedAt, "npm inventory start timestamp"),
    completedAt: assertTimestamp(value.completedAt, "npm inventory completion timestamp"),
    packages,
  }
}
...
function normalizeNpmObservation(value) {
  ...
  if (value.status !== "ABSENT" || value.httpStatus !== 404 || value.code !== "E404") {
    throw new TypeError("npm package observation must be exact ABSENT/404/E404 evidence")
  }
  return {
    name: assertNonemptyString(value.name, "npm package name"),
    version: assertMatchingString(value.version, VERSION_PATTERN, "npm package version"),
    status: value.status,
...
function normalizeReleaseEvidenceValue(value) {
  ...
  if (assets.length !== 45 || new Set(assets.map(({ id }) => id)).size !== 45) {
    throw new TypeError("Release evidence must contain exactly 45 uniquely identified assets")
  }
  assertUniqueNames(
  ...
  if (!Number.isSafeInteger(aggregateSize) || aggregateSize > RELEASE_PAYLOAD_LIMITS.escrowBytes) {
    throw new TypeError("Release asset evidence exceeds the escrow payload limit")
  }
  const normalized = {
    role: value.role,
...
function normalizeReleaseSemantic(value) {
  ...
  if (
    ...
    throw new TypeError("Managed duplicate Release must remain a mutable non-prerelease draft")
  }
  const body = assertString(value.body, "Release body")
  const bodySha256 = assertSha256(value.bodySha256, "Release body digest")
  const targetCommitish = assertString(value.targetCommitish, "Release target commitish")
...
function normalizeAssetEvidence(value) {
  ...
  if (!/^sha256:[0-9a-f]{64}$/u.test(digest)) {
    throw new TypeError("Asset service digest must be canonical sha256 evidence")
  }
  const name = assertNonemptyString(value.name, "Asset name")
  if (Buffer.byteLength(name, "utf8") > RELEASE_PAYLOAD_LIMITS.archiveFilenameBytes) {
    throw new TypeError("Asset name exceeds the release archive filename limit")
  }
  const size = assertNonnegativeInteger(value.size, "Asset size")
  if (size > RELEASE_PAYLOAD_LIMITS.tarballBytes) {
    throw new TypeError("Asset size exceeds the release per-asset payload limit")
  }
  return {
    id: assertId(value.id, "Asset id"),
    nodeId: assertNonemptyString(value.nodeId, "Asset node id"),
    name,
    label: value.label === null ? null : assertString(value.label, "Asset label"),
    ...
    downloadCount: assertNonnegativeInteger(value.downloadCount, "Asset download count"),
    downloadSha256: assertSha256(value.downloadSha256, "Downloaded asset digest"),
  }
}
...
function normalizePayloadProof(value) {
  value = assertExactFields(
    value,
    ["baseAssetSet", "baseAssetSetSha256", "consolidationPayloadSha256", "attestationVerification"],
    "Payload proof",
  )
  ...
  const subjects = assertArray(attestationVerification.subjects, "Attestation subjects", {
    exactLength: 22,
  }).map(normalizeNamedDigest)
  if (subjects.length !== 22) throw new TypeError("Attestation verification requires 22 subjects")
  assertUniqueNames(
  ...
  return {
    baseAssetSet,
    baseAssetSetSha256: assertSha256(value.baseAssetSetSha256, "Base asset set digest"),
...
function normalizeAuthorityStageValue(value) {
  ...
  if (!PERFORM_STAGES.has(stage)) throw new TypeError("Authority stage name is invalid")
  ...
  const payloadProof = normalizePayloadProof(value.payloadProof)
  assertPayloadMatchesReleases(payloadProof, releases)
  const targetRead = value.targetRead === null ? null : normalizeTargetRead(value.targetRead)
  ...
  if (
    ...
    throw new TypeError("Target read evidence must exactly match an authority Release")
  }
  const normalized = {
    stage,
    ...
    payloadProof,
    targetRead,
    observedAt: assertTimestamp(value.observedAt, "Authority observation timestamp"),
  }
  assertCanonicalValueByteLength(
    normalized,
    DUPLICATE_DRAFT_CONSOLIDATION_LIMITS.authorityStageBytes,
...
function normalizeTargetRead(value) {
  ...
  const evidenceSha256 = assertSha256(value.evidenceSha256, "Target read evidence digest")
  ...
  const timestamps = [
    ...
    assertTimestamp(value.releaseGetCompletedAt, "Release GET completion timestamp"),
    assertTimestamp(value.assetsListStartedAt, "Asset list start timestamp"),
    assertTimestamp(value.assetsListCompletedAt, "Asset list completion timestamp"),
  ]
...
function normalizeEvent(value, expectedSequence, previousEventSha256) {
  ...
  const sequence = assertPositiveInteger(expectedSequence, "Expected journal event sequence")
  if (value.sequence !== sequence) throw new TypeError("Journal event sequence is not contiguous")
  const expectedPrevious =
    previousEventSha256 === null
      ? null
      : assertSha256(previousEventSha256, "Previous journal event digest")
  if (value.previousEventSha256 !== expectedPrevious) {
    throw new TypeError("Journal event does not bind the immediately previous digest")
  }
...
function normalizeEventPayload(type, value) {
  if (type === "operation-started") {
    ...
    assertArrayEqual(deletionOrder, APPROVED_DUPLICATE_IDS, "Approved operation deletion order")
    return {
      proposedRecordSha256: assertSha256(value.proposedRecordSha256, "Proposed record digest"),
      confirmationSha256: assertSha256(value.confirmationSha256, "Confirmation digest"),
      controllerSha: assertGitSha(value.controllerSha, "Operation controller SHA"),
      deletionOrder,
    }
  }
  if (type === "npm-observed") {
    ...
    if (!PERFORM_STAGES.has(inventory.stage)) {
      throw new TypeError("npm-observed event has an invalid perform-stage inventory")
    }
    return {
      targetReleaseId: assertId(value.targetReleaseId, "npm-observed target Release id"),
  ...
  if (type === "delete-authority-observed") {
    ...
    const targetReleaseId = assertId(value.targetReleaseId, "Authority target Release id")
    const authority = normalizeAuthorityStage(value.authority)
    if (authority.targetRead?.evidence.id !== targetReleaseId) {
      throw new TypeError("Delete authority target read must identify the deletion target")
    }
    return {
      targetReleaseId,
      attemptNumber: assertDeleteAttempt(value.attemptNumber),
  ...
  if (type === "delete-intent") {
    ...
    return {
      targetReleaseId: assertId(value.targetReleaseId, "Delete intent target Release id"),
      attemptNumber: assertDeleteAttempt(value.attemptNumber),
  ...
  if (type === "resume-reconciliation") return normalizeResumeReconciliation(value)
  if (type === "absence-converged") return normalizeAbsenceConverged(value)
  if (type === "final-authority-observed") {
    value = assertExactFields(value, ["authority"], "final-authority-observed payload")
    const authority = normalizeAuthorityStage(value.authority)
...
function normalizeDeleteOutcome(value) {
  ...
  return {
    targetReleaseId: assertId(value.targetReleaseId, "Delete outcome target Release id"),
    attemptNumber: assertDeleteAttempt(value.attemptNumber),
...
function normalizeResumeReconciliation(value) {
  ...
  const targetReleaseId = assertId(value.targetReleaseId, "Resume target Release id")
  let releaseEvidence
  if (value.classification === "present-unchanged-retryable") {
    ...
    if (releaseEvidence.id !== targetReleaseId) {
      throw new TypeError("Resume Release evidence does not identify its target")
    }
  } else if (value.classification === "absent-ambiguous" && value.releaseEvidence === null) {
    releaseEvidence = null
  } else {
    throw new TypeError("Resume classification and Release evidence are inconsistent")
  }
  return {
    targetReleaseId,
    attemptNumber: assertDeleteAttempt(value.attemptNumber),
    classification: value.classification,
    releaseEvidence,
    observedAt: assertTimestamp(value.observedAt, "Resume reconciliation timestamp"),
  }
}
...
function normalizeAbsenceConverged(value) {
  ...
  const attempts = assertPositiveInteger(value.attempts, "Absence convergence attempts")
  if (attempts > 6) throw new TypeError("Absence convergence exceeds six read attempts")
  return {
    targetReleaseId: assertId(value.targetReleaseId, "Absence target Release id"),
    attemptNumber: assertDeleteAttempt(value.attemptNumber),
    basis: value.basis,
    directGet404At: assertTimestamp(value.directGet404At, "Direct GET absence timestamp"),
    listAbsentAt: assertTimestamp(value.listAbsentAt, "Release list absence timestamp"),
    attempts,
    completedAt: assertTimestamp(value.completedAt, "Absence convergence timestamp"),
  }
}

function validateJournalEventTarget(event, deletionOrder) {
  const targetReleaseId = event.payload.targetReleaseId
  if (targetReleaseId !== undefined && !deletionOrder.includes(targetReleaseId)) {
    throw new TypeError("Journal event target is not in the fixed deletion order")
  }
}
...
function assertIdentityContract(candidate, roles, confirmation) {
  if (
    ...
    throw new TypeError("Confirmation does not bind the proposed candidate and survivor")
  }
  assertArrayEqual(confirmation.duplicates, roles.duplicates, "Confirmation duplicate identities")
}
...
function chargeCanonicalBytes(byteLength) {
  ...
  for (const budget of CANONICAL_BUDGETS) {
    ...
    if (!Number.isSafeInteger(budget.used) || budget.used > budget.maximum) {
      throw new TypeError(`Canonical value exceeds its cumulative ${budget.label} budget`)
    }
  }
}
...
function chargeCurrentCanonicalBytes(byteLength) {
  ...
  if (!Number.isSafeInteger(current.used) || current.used > current.maximum) {
    throw new TypeError(`Canonical value exceeds its cumulative ${current.label} budget`)
  }
}
...
function assertSchemaVersion(value, label) {
  if (value !== 1) throw new TypeError(`${label} schemaVersion must be integer 1`)
}
...
function assertArray(value, label, { exactLength, maximumLength } = {}) {
  ...
  for (let index = 0; index < length; index += 1) {
    ...
    if (keys[index] !== key) {
      throw new TypeError(`${label} must contain every canonical numeric index`)
    }
...
function kindLimit(kind) {
  ...
  if (maximum === undefined) throw new TypeError("Unknown consolidation envelope kind")
  return maximum
}

function assertWithinKindLimit(kind, byteLength) {
  const maximum = kindLimit(kind)
  if (byteLength > maximum) throw new Error(`${kind} envelope exceeds its byte limit`)
}
