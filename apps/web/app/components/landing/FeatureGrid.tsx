  ...
  return (
    <section className="py-20 px-8 border-t landing-border">
      <div className="text-center mb-10">
        ...
        <p className="landing-text-muted mt-2">And nothing you don&apos;t.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-[650px] mx-auto">
        {features.map((feature) => {
          ...
          return (
            <div
              key={feature.title}
              className={`relative landing-surface border rounded-lg p-5 ${
                isDawnCli ? "border-accent-amber/40" : ""
              }`}
            >
              {isDawnCli && (
                <span
              ...
              <h3 className="text-sm font-semibold landing-text">{feature.title}</h3>
              <p className="text-sm landing-text-muted mt-2 leading-relaxed">{feature.body}</p>
            </div>
          )
        })}
      </div>
