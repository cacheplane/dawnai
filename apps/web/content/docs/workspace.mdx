...
<Callout type="warn" title="Treat AGENTS.md as shared prompt input">
  Dawn re-reads `workspace/AGENTS.md` into prompt context on every turn for every consuming `agent()` route and subagent. A route with `writeFile` can persist instructions that affect later requests and other consuming agents sharing that workspace. Raw graph, workflow, and chain routes do not consume this prompt file.

  Use separate workspaces for different tenants or trust zones. Constrain or deny writes for untrusted agents, review seeded content, and never copy raw untrusted text into `workspace/AGENTS.md`.
</Callout>
