...
By default, tool results stream back through the dev server. The deployment protocol exposes streaming via `/runs/stream` and the LangSmith contract. See [Dev Server](/docs/dev-server) for the protocol details.
