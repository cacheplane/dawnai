...
const res = await fetch(`http://127.0.0.1:3001/threads/${thread_id}/runs/stream`, {
  ...
  body: JSON.stringify({
    ...
    input: { messages: [{ role: "user", content: "latest LLM benchmarks" }] },
  }),
})
...
while (true) {
  ...
  for (const frame of frames) {
    // Ignore SSE comments (for example, the `: ping` heartbeat) first.
    const lines = frame.split("\n").filter((line) => !line.startsWith(":"))
    // Event frames have an "event:" line and a "data:" line.
...
- **Heartbeats are comments.** A `: ping` heartbeat contains no `event:` or `data:` payload, so clients ignore it.
...
- **Retry has limits during streams.** Retry is available only before the first stream event; any emitted token or event commits the response. See [Retry](/docs/retry) for the streaming caveat.

## Related

<RelatedCards items={[
  { href: "/docs/dev-server/agent-protocol", title: "Agent Protocol", subtitle: "protocol endpoints and the assistant-id format" },
  ...
  { href: "/docs/deployment", title: "Deployment Options", subtitle: "building deployable route artifacts" },
]} />
