...
`tool_call_id` is the model's tool-call id for the `writeTodos` call that produced the update — the same id that call's tool events carry — and it is omitted when the model supplied no tool-call id.
