...
Offloading needs a filesystem to spill to, so `toolOutput` is [gated off the `hono` edge target](/docs/deployment#what-the-edge-cannot-serve) at build time, and a runtime with no filesystem raises `DAWN_E1005` per request rather than reading the settings and ignoring them. Summarization below has no such dependency and runs on every target.
