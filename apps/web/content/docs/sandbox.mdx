...
<Callout type="warn" title="Do not scale replicas without distributed run coordination">
  The Dawn HTTP runtime's one-run-per-thread gate and cancellation registry are process-local. Shared Postgres stores make thread data durable, but do not distribute that coordination. Keep one replica unless you provide sticky routing plus distributed per-thread serialization and route cancellation to the replica that owns the run.
</Callout>
