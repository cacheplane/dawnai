...
Connection and table-naming options shared by all three stores. Supply `connectionString` to have the store build and own a pool (`close()` ends it), or `pool` to inject an existing one (`close()` leaves it alone — the app that made the pool ends it). `schema` defaults to `DEFAULT_SCHEMA` and `tablePrefix` to `DEFAULT_TABLE_PREFIX`.
