...
Declares a route's typed long-term memory. Place the call as the default export of a `memory.ts` file next to the route's `index.ts` — that file's presence activates the memory capability, contributes `recall`, and contributes `remember` unless memory writes are configured as `"off"`. Semantic, episodic, and reflection kinds are wired; procedural remains deferred. `scope` lists the namespace dimensions the memory is partitioned by. `schema` is a [zod](https://zod.dev) schema validating each stored record. `identity` names the fields used for semantic write reconciliation; it defaults to `["subject", "predicate"]`. The model-facing `remember` call requires both schema-valid `data` and a human-readable `content` string. Semantic IDs hash namespace plus serialized data; append-only episodic and reflection IDs hash that basis plus the request timestamp, keeping identical data distinct across requests.
