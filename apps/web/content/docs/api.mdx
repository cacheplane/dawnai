...
The stores talk to Postgres through the standard `pg` driver, which opens a TCP connection. Node, Bun, and Vercel functions can use it directly; Cloudflare Workers provides no raw TCP socket, so a Workers deploy needs Hyperdrive or an HTTP-based driver in front of the database. Dawn does not currently ship a verified Workers configuration for these stores.
