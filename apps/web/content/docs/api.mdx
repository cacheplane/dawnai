...
Built-in capability markers detect route/app files and contribute prompt fragments, tools, or stream transformers. `createWorkspaceMarker()` activates when a `workspace/` directory exists or a runtime `workspaceRoot` is supplied; `createMemoryMarker()` contributes `recall` and `remember` when route memory context exists.
...
### `createCapabilityRegistry()` and `applyCapabilities()`
...
Build and apply the marker registry for a route. Runtime integrations use these helpers to collect built-in and custom capability contributions before materializing an agent route.
...
export function config<T extends DawnConfig>(value: T): T
```
...
export function assertDawnRoutesDir(appRoot: string): Promise<string>
export function toRouteSegments(pathname: string): readonly RouteSegment[]
export function isPrivateSegment(segment: string): boolean
export function isRouteGroupSegment(segment: string): boolean
```

Discovery helpers find the Dawn app root, validate the routes directory, scan route files, and parse filesystem route segments.
...
export function record(options: RecordOptions): Promise<void>
...
```ts
export function collectRunResult(...): AgentRunResult
...
export function createAgentProtocolInjector(...): AgentProtocolInjector
export function createSubprocessApp(...): SubprocessApp
```
...
export function resolveDataset(dataset: Dataset, baseDir?: string): Promise<readonly EvalCase[]>
export function runEval(def: EvalDefinition, options: RunEvalOptions): Promise<EvalReport>
```

`defineEval()` types an eval module. `resolveDataset()` supports inline cases, `.json`, `.jsonl`, and function datasets. `runEval()` drives a definition with a caller-provided `runCase` function and returns an aggregate report.
...
export function resolveGate(...): GateResult
```
