...
This entry imports `pg` as a *value*, which pulls `net`/`tls`/`dns` into the module graph — that is why it is a separate subpath. The main entry imports `pg` for types only and links on a runtime with no TCP sockets.
