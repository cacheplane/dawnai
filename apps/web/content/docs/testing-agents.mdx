...
`create-dawn-app` generates `test/agent.test.ts` that imports `@dawn-ai/testing` and covers the default agent route. Run it immediately after scaffolding:

```bash
pnpm test
```

The generated test uses an inline `script()` for the happy-path scenario. Grow it by:
...
it("handles the happy path", async () => {
  const run = await h.run({
    input: "List my tasks",
    fixtures: script().user("List my tasks").replies("You have 3 tasks."),
  })
  expectFinalMessage(run).toContain("3 tasks")
}, 60_000)
...
it("filters open items", async () => {
  const run = await h.run({
    ...
    fixtures: loadFixtures(
      fileURLToPath(new URL("fixtures/filter-open.fixture.json", import.meta.url)),
    ),
  })
  expectToolCalled(run, "applyFilter").withArgs({ status: "open" })
  expectFinalMessage(run).toContain("open")
}, 60_000)
```
