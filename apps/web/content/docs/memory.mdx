...
**`remember({ data, content, tags?, confidence? })`** requires a human-readable `content` string in the model-facing schema, validates `data` against the route schema, then writes a record. Semantic IDs are data-derived: `memory_` + the first 16 hex chars of `sha1(namespace | JSON(data))`, so a contradicting value (same identity, different data) gets a distinct id and can coexist with the old row. Append-only episodic and reflection IDs hash the same basis **plus the request timestamp**, so repeated events or insights on different requests do not collapse into one row. `confidence` defaults to `1` and `tags` to `[]`.
