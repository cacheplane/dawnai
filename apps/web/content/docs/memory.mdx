...
Because the watermark advances with each pass, re-running `reflect` is cheap: a namespace with nothing new since its last reflection is skipped without a model call. Reflections are **append-only** — a later insight never supersedes an earlier one (see [write governance](#write-governance)); distillation accumulates insights rather than overwriting them.
