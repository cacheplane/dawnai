...
Vector recall is a second stage on top of keyword recall; leaving `vector` unset keeps the default keyword-only path exactly as before. When enabled, the only change is one opt-in network embed of the query at recall time (and of the content at write time). With SQLite, recall stays deterministic under a fixed embedder — or aimock replay. More generally, Dawn's final scoring is deterministic for fixed candidate lists, timestamps, tuning, and embeddings.
