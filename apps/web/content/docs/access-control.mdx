...
Even a tool that's allowed and approved still runs somewhere. By default that's the local `workspace/` directory on the host. Adding a `sandbox` key to `dawn.config.ts` routes every `readFile`, `writeFile`, `listDir`, and `runBash` call for a thread into an isolated environment instead — filesystem, shell, and (optionally) network. Dawn ships a Docker reference provider and a [Kubernetes provider](/docs/sandbox/kubernetes) behind the same `SandboxProvider` contract. See [Execution Sandbox](/docs/sandbox) for the provider-neutral boundary.
...
<RelatedCards items={[
  ...
  { href: "/docs/configuration", title: "Configuration Reference", subtitle: "dawn.config.ts reference including permissions and sandbox" },
]} />
