...
The build enforces the size each marker enforces at runtime — 32 KiB for a `SKILL.md` or `memory.md`, 64 KiB for `plan.md` — and fails with `DAWN_E1005`, naming every oversized file, before it writes any artifact. A manifest that records skill names but carries no bodies for them (one built before this behavior existed, or composed by hand) still fails at boot with `DAWN_E1005` rather than dropping the skills silently; rebuild with `dawn build`.
