...
Each store exposes `close()`. A store built from `connectionString` owns its pool and `close()` ends it; a store built from an injected `pool` deliberately leaves that pool alone, so the shared-pool config above must end the pool itself:
