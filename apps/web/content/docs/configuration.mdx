...
export default {
  ...
  // Path to the env file used by `dawn dev`, `dawn start`, `dawn inspect`,
  // and `dawn verify`.
  // env: "./.env",  // default
}
```
...
| Type | Default | Scope |
|---|---|---|
| `string` | `"./.env"` | `dawn dev`, `dawn start`, `dawn inspect`, `dawn verify` |

Path to the `.env` file these app-aware CLI commands load, relative to the app root. For built Dawn runtimes launched another way, configure environment variables in the host or deployment platform. The generated LangSmith target separately records its detected environment-variable names in `.dawn/build/langgraph.json`; that file is specific to the LangSmith artifact, not the production environment contract for every build target. See [Deployment](/docs/deployment).
