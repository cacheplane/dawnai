...
export default {
  ...
  // Path to the env file loaded by `dawn dev` and `dawn inspect`.
  // `dawn verify` resolves the same path for readiness checks without loading it.
  // env: "./.env",  // default
}
```
...
Pattern maps are keyed by operation (for example `bash`, `readFile`, or `memory`). Values use prefix matching against the candidate command, path, or memory namespace; deny wins over allow. The reserved `tool` and `subagent` keys are different: their entries match exact tool names or serialized subagent identities, never prefixes.

Static `allow` and `deny` entries remain in `dawn.config.ts`; Dawn does not seed them into the runtime permissions file. Only an interactive **Always** decision adds a runtime allow entry to the configured store. The default Node store persists those decisions in `.dawn/permissions.json`; a custom store, including the Postgres store, may persist them elsewhere.
...
| Type | Default | Scope |
|---|---|---|
| `string` | `"./.env"` | loaded by `dawn dev` and `dawn inspect`; checked by `dawn verify` |

Path to the `.env` file that `dawn dev` and `dawn inspect` load into the process, relative to the app root. `dawn verify` resolves the same path and reads it to check for required provider keys, but does not load it. `dawn start` does not load `config.env`; pass runtime variables through the shell, host, or deployment platform.

The generated LangSmith target records an env-file path in `.dawn/build/langgraph.json`: `.env.example` when that file exists, otherwise `.env`. LangSmith uses the referenced file to identify variables. This is specific to the LangSmith artifact, not the environment contract for every build target. See [Deployment Options](/docs/deployment).
...
| Key | Type | Default | Description |
|---|---|---|---|
| `enabled` | `boolean` | — | Historical, inert compatibility field. It does not enable or disable memory; a route's `memory.ts` and `writes` govern capability exposure. |
...
| `vector` | `{ embedder; weights?; rrfK?; vectorK?; recencyWeight?; confidenceWeight? }` | absent (keyword-only) | Enables [hybrid semantic recall](/docs/memory#semantic-recall-opt-in). `embedder` is required when the block is present. Defaults: keyword/vector weights `1`/`1`, `rrfK: 60`, `vectorK: 64`, recency/confidence weights `0.3`/`0.1`. A custom store owns its retrieval tuning. |
| `episodes` | `{ enabled?; ttlMs?; cap?; includeFailedRuns?; embed? }` | disabled; 30-day TTL; cap `500`; include failures; no embeddings | Configures the [runtime episode recorder](/docs/memory#episodic-memory). `embed: true` is currently unsupported: Dawn warns once and resolves it to `false`, so recorded episodes use keyword and time-window recall. |
| `distill` | `{ model?; provider?; maxBatches?; consolidate?; reflect? }` | see description | Tunes explicitly invoked [distillation commands](/docs/memory#distillation); nothing runs automatically. Defaults: `gpt-5-mini`, provider inferred then `openai`, `maxBatches: 5`; consolidate after 7 days in batches of 5–50, summaries without expiry, source TTL 7 days; reflect after 10 new records over at most 100, writing `candidate` records (`reflect.writes` accepts only `"candidate"` or `"auto"`). |
...
`node` and `langsmith` are emitted by default. Naming `targets` **replaces** that list rather than adding to it: restrict to one — e.g. `{ build: { targets: ["node"] } }` — if you only deploy that way, and name every target you want otherwise (`{ build: { targets: ["node", "hono"] } }`). See [Deployment Options](/docs/deployment) for the full bridge.
...
- **The per-thread run gate and cancellation registry are in-process.** Postgres shares durable rows, not these coordinators: two replicas can each start a run on the same thread, and an explicit cancellation request can stop only the run registered in the instance that receives it. Use sticky routing or a distributed coordination layer when you need cross-replica serialization and cancellation guarantees.
...
<RelatedCards items={[
  ...
  { href: "/docs/deployment", title: "Deployment Options", subtitle: "production environment variables and the deploy workflow" },
]} />
