...
| Cloudflare Workers | **Not with a raw connection** | `pg` opens a TCP socket, which workerd does not provide. A Workers deploy needs [Hyperdrive](https://developers.cloudflare.com/hyperdrive/) or an HTTP-based driver in front of the database. Dawn does not currently ship a verified Workers configuration for these stores. |
