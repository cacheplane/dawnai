...
Yes. The `model` field on `agent()` accepts a `KnownModelId` — OpenAI and Google IDs are pre-typed, and `(string & {})` keeps the escape hatch open for any LangChain-compatible provider. Dawn does not wrap or proxy provider calls. See [API Reference](/docs/api).
