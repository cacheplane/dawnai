...
Dawn produces deployment artifacts; the framework itself is pre-1.0 and the API surface is still moving. `dawn build` writes a `.dawn/build/langgraph.json` plus per-route entry files for LangSmith-style deployment. Lock to a pinned version and read release notes between upgrades. See [Deployment](/docs/deployment).
...
It walks your routes, runs typegen, and writes `.dawn/build/langgraph.json` plus a per-route entry file under `.dawn/build/<routeSlug>.ts`. Each agent route's entry imports your default `agent()` descriptor, materializes it as a LangGraph graph, and wires in discovered tools. Each `assistant_id` is `<routeId>#<kind>` — for example `/hello/[tenant]#agent`. See [Deployment](/docs/deployment).
