...
No. Dawn is TypeScript-only and the type inference is the whole point — tool parameter types are read from function signatures at build time and emitted into `dawn.generated.d.ts`. A Python port would lose that. Use `langgraph` directly in Python; LangSmith deploys both.

### Is Dawn production-ready?

Dawn produces production-ready artifacts; the framework itself is pre-1.0 and the API surface is still moving. `dawn build` writes a `.dawn/build/langgraph.json` plus per-route entry files that LangSmith deploys without further translation. Lock to a pinned version and read release notes between upgrades. See [Deployment](/docs/deployment).
