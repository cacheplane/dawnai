...
## Edge runtimes (preview)

`@dawn-ai/cli/fetch` is a node-free entry point for the Dawn runtime: it exports `createRuntimeFetchHandler` with a module graph that contains no filesystem, SQLite, or CLI code of Dawn's own. Instead of discovering your app on disk, you hand it everything at construction time — the build-time module manifest, a `DawnConfig` object, and store instances:
...
<Callout type="warn" title="Requires a runtime with Node built-in shims">
  Dawn's supporting packages still import `node:path`, `node:fs`, `node:crypto`, and friends at module scope, so a bundle built from this entry **links** against those specifiers even though the injected request path never calls them. That makes it deployable today on Cloudflare Workers with `nodejs_compat` enabled, and not yet on runtimes with no `node:` shim layer at all — those fail when the module graph is instantiated, before any Dawn code runs. Removing that requirement means splitting `@dawn-ai/core`, `@dawn-ai/permissions`, `@dawn-ai/workspace`, and `@dawn-ai/langchain` the same way this release split the CLI; a purity test pins the current set so it can only shrink.
</Callout>

Node-only capabilities are unavailable on this path regardless of runtime: the execution sandbox, workspace file and shell tools, skills, and the SQLite-backed default stores. Supply Postgres-backed or in-memory implementations for state you want to keep. The build target that generates a ready-to-deploy edge entry point (`hono`, covering Workers, Vercel, and Node hosts) lands in a following release.
