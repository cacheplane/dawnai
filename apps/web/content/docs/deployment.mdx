...
- **`.dawn/build/modules.mjs`** — a generated static module manifest: every route, tool, state definition, and route memory module imported statically, with their schemas inlined. `server.mjs` boots from it, so production startup performs no route-tree scanning or per-file discovery — the app is wired at build time. Regenerated on every `dawn build`; `dawn check` fails if the manifest has drifted from the routes on disk (for example after a route rename without a rebuild).
