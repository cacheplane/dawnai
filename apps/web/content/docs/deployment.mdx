...
Dawn does not host or run production traffic itself. It owns local development (`dawn dev`, `dawn run`, `dawn test`) **and** the build step that produces deployable artifacts (`dawn build`). Production runs on **LangSmith**, which Dawn's dev server already speaks natively. What runs locally runs in production after `dawn build` materializes the deployment-ready entry files.
...
<Steps>
  ...
  <Step title="Per-route middleware (optional)">
    `src/middleware.ts` (default-exporting a function from `defineMiddleware`) runs before every `/runs/wait` and `/runs/stream` request — identically in `dawn dev` and on LangSmith. Use it to gate access by header, populate request-scoped context, or `reject(status, body)` unauthorized callers. See [Middleware](/docs/middleware) for the API.
  </Step>
  <Step title="Push to your LangSmith-connected remote">
    LangSmith builds the image from `.dawn/build/` and provisions assistants keyed by the `<routeId>#<kind>` ids in the generated `langgraph.json`. From there, your agents and harnesses can hit the deployed endpoints using the same protocol they already speak locally.
  </Step>
</Steps>
...
- **Type inference drifted** — run `dawn typegen` locally and commit `dawn.generated.d.ts`. The LangSmith build uses the committed file.

## Related

<RelatedCards items={[
  { href: "/docs/dev-server", title: "Dev Server", subtitle: "local protocol parity with production endpoints" },
  { href: "/docs/middleware", title: "Middleware", subtitle: "request gating that runs identically in dev and on LangSmith" },
