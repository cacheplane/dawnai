...
test("uppercaseReads forwards every other backend method", async () => {
  ...
  // Backend paths are absolute and inside the harness dir (the gate/jail
  // normally resolves them for you; here you call the backend directly).
  const file = join(h.dir, "a.txt")
  await h.backend.writeFile(file, "hello", h.ctx)
  expect(await h.backend.readFile(file, h.ctx)).toBe("HELLO")
