...
<Steps>
  ...
  <Step title="expected is deep-equal by default">
    Dawn compares the returned state deep-equal against `expected`. Use `expected.eq`, `expected.contains`, or custom matchers for partial or fuzzy matching (see `@dawnai.org/sdk/testing` for the full surface).
  </Step>
