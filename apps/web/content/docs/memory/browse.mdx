...
Call `validateBrowseQuery` at the untrusted boundary even though the in-repo stores validate defensively. The detailed limits, constants, and error codes belong in the [API Reference](/docs/api); map a `BrowseQueryError` to a client error rather than retrying it as a store outage.
