...
A callable `graph` function may explicitly accept the same Dawn `RuntimeContext` and use `ctx.tools`. A precompiled raw LangGraph object is different: its `.invoke()` treats the second argument as LangGraph `RunnableConfig`, not Dawn's typed `RuntimeContext`. That graph keeps or imports its own tools.
