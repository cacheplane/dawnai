...
A subagent does **not** inherit the parent's capability tools. By default it gets only its own route-local `tools/*.ts`; ambient capability tools such as `writeFile`, `runBash`, `writeTodos`, and `remember`/`recall` are withheld unless you name them in `allow`. Grant back exactly what the worker needs:

```ts title="src/app/research/subagents/researcher/index.ts"
// src/app/research/subagents/researcher/index.ts — read-only worker
// keeps its own tools/*.ts; writeFile/runBash withheld by default; grant readFile:
