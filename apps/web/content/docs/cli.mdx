...
- An unknown `build.targets` entry, and — when `"hono"` is one of them — every feature that target [cannot serve](/docs/deployment/edge#what-the-edge-cannot-serve), reported together as `DAWN_E1005`. The same gate `dawn build` applies, mirrored here so you learn about all of them before building.
...
This is what the `node` build target's generated `Dockerfile` runs (`CMD ["node", ".dawn/build/server.mjs"]`), and it's the only server that engages the [execution sandbox](/docs/sandbox) in production. See [Node and Docker](/docs/deployment/node).
...
Inspects and reattaches to Agent Protocol threads on a running Dawn server.

```
dawn threads tail <thread-id>
dawn threads tail <thread-id> --url http://127.0.0.1:3000
dawn threads tail <thread-id> --header "authorization: Bearer $TOKEN"
dawn threads tail <thread-id> --json
```
...
Flags:
- `--url <url>` — base URL of the running Dawn server (default
  `http://127.0.0.1:3000`).
...
<RelatedCards items={[
  ...
  { href: "/docs/dev-server/agent-protocol", title: "Agent Protocol", subtitle: "full HTTP contract for dawn dev" },
  { href: "/docs/testing", title: "Scenario Testing", subtitle: "scenario authoring for dawn test" },
  { href: "/docs/deployment", title: "Deployment Options", subtitle: "dawn build and dawn verify in the deploy workflow" },
]} />
