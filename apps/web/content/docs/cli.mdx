...
Prints the bundled, version-matched Dawn documentation to stdout.

```
dawn docs                # list available topics
dawn docs cli            # print the CLI reference
dawn docs <topic>        # print a single topic by slug
```

Docs are bundled into the CLI at build time, so `dawn docs` works offline and always matches the installed version. Topics correspond to the docs pages at `dawnai.org/docs/`.
