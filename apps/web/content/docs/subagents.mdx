...
<Callout type="info" title="Subagent tool inheritance">
  A child gets shared authored `src/tools/*` and its own route-local `tools/*` by default, but does not inherit the parent route's local tools. Active capability tools such as `writeFile` and `runBash` are withheld unless granted with `tools.allow`. Explicitly deny a sensitive shared authored tool, for example `tools: { deny: ["deployProd"] }`, when a child must not receive it. A child with a dispatchable child of its own receives the internal `task` mechanism independently; keep `task` outside tool policy. See [Scoping a route's tools](/docs/tools#scoping-a-routes-tools).
</Callout>
