...
- LangSmith deploys the build output without further translation.
...
The graph builder is identical. The folder path `src/app/support/` becomes the endpoint `/support`. The default export is the route entry. The `assistant_id` LangSmith deploys is `/support#graph`, generated for you.
...
- **LangSmith deploy.** Same target. `dawn build` emits the standard schema; LangSmith takes it without further translation.
...
3. **Cut over deployment last.** Both projects can deploy to LangSmith side by side under different `assistant_id`s. When every graph has a Dawn equivalent at parity, switch the production `assistant_id`s to the Dawn-built ones and retire the old project.
