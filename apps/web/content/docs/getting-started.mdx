...
4. Synthesize the findings into a cited report and save it with \`writeFile({ path: "reports/<slug>.md", content: "<report>" })\`.

Cite every claim with its source path in square brackets, e.g. [corpus/agent-architectures.md].`,
})
```
...
The dev server exposes the Agent Protocol. Create a thread, then run it:
...
Walks the routes declared via `dawn.config.ts`, runs typegen, and writes `.dawn/build/langgraph.json` plus per-route entry files. The generated `assistant_id`s match the local ids, such as `/research#agent`.
