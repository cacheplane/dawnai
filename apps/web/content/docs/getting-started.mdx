...
Run the commands in that order. `build` discovers routes from the app directory
(`appDir`, defaulting to `src/app`), runs typegen, and writes deployment
artifacts for the configured `build.targets`: by default, a `node` target
(`.dawn/build/server.mjs` plus a hardened `Dockerfile`) and a `langsmith` target
(`.dawn/build/langgraph.json` plus per-route entry files). The generated
`assistant_id`s match the local ids, such as `/research#agent`. An opt-in `hono`
target emits an edge entry point for Cloudflare Workers — see
[Deployment](/docs/deployment#edge-runtimes).

### Serve it in production

The generated `start` script loads `.env` when present and serves the emitted
`.dawn/build/server.mjs`; it does not build first. You should now see the agent
listening on `0.0.0.0:8000`, serving Agent Protocol and AG-UI and engaging the
[execution sandbox](/docs/sandbox) if configured. The `.env` loading comes from
the scaffold's `npm start` package script; bare `dawn start` does not load the
file named by `config.env`. To ship it as a container instead:
