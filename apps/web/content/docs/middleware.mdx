...
Middleware runs in the local dev server (`dawn dev`). Both local `/threads/:id/runs/wait` and `/threads/:id/runs/stream` requests invoke middleware before any route executes. The `/healthz` endpoint bypasses middleware because it is a liveness probe.
