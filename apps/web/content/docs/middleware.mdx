...
Middleware runs in the local dev server (`dawn dev`) and in the built runtime served by `dawn start`. These endpoints invoke it before any route executes:
...
`GET /threads/:id/pending_interrupts` hands back the human-in-the-loop prompts parked on a thread, so it is gated like a run — and it is the first gated endpoint whose `req.method` is not `"POST"`. Middleware that branches on `req.method === "POST"`, or that reads `req.params` to decide access, falls through on it. See [the endpoint reference](/docs/dev-server#agent-protocol-endpoints) for how its route identity is resolved.
</Callout>

`dawn build`'s `node` and `hono` targets mount this middleware in the runtime they emit, which is the same handler `dawn start` serves. The `langsmith` target does not materialize it into the generated entry files — treat middleware as a Dawn runtime feature unless your deployment target wires it in separately.
