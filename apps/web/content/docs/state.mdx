...
- `runs/wait` and `runs/stream` (locally under `dawn dev`, on LangSmith in prod) — JSON over HTTP keyed by the route id (`route` in the request body locally; `assistant_id` on LangSmith).
