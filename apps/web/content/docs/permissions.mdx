...
`runBash`, `readFile`, `writeFile`, and `listDir` keep their own pattern-aware allow/deny gates — putting them in `approve` is redundant and would double-prompt. `dawn check` warns when a route's `approve` list overlaps with these internally-gated tools, with `deny` (a dead entry, since deny wins), or grants approval to a tool a subagent hasn't also added to its own `allow` list (no-op unless allow-listed).
