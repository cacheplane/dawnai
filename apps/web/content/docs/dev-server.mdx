...
<Tabs>
  ...
  <Tab label="GET /threads/:thread_id/pending_interrupts">
    ...
    The two endpoints decide "no such thread" from different sources: this one from the thread record, `POST /threads/:thread_id/resume` from the checkpoint — so a thread that was created but never checkpointed answers `200 { "interrupts": [] }` here and `404` there.
