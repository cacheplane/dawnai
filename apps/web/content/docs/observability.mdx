...
If you do not have a LangSmith account, the dev server's `runs/stream` endpoint streams every event in real time over SSE — no tracing account needed. Each event has a type (`chunk`, `tool_call`, `tool_result`, `plan_update`, `interrupt`, `subagent.*`, `done`) and a JSON payload you can pipe to any terminal or client. See [Agent Protocol](/docs/dev-server/agent-protocol) for the full event-type table and connection instructions.

## Related

<RelatedCards items={[
  { href: "/docs/configuration", title: "Configuration Reference", subtitle: "dawn.config.ts reference — env key for .env file path" },
