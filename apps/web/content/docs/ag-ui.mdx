...
Agent Protocol has the opposite disconnect policy: closing a `runs/stream`,
`runs/wait`, or `resume` connection only disconnects the viewer, and the run
continues. Stop it explicitly with `POST /threads/:thread_id/cancel`; server
shutdown also aborts in-flight work.
