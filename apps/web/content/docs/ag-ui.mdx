...
Agent Protocol viewer disconnects continue the run. AG-UI viewer disconnects abort
the run. For Agent Protocol, closing a `runs/stream`, `runs/wait`, or `resume`
connection only detaches the viewer, so stop the run explicitly with
