...
**Runtime and deploy.** eve is a Vercel project. Its best trick is that `vercel deploy` ships the agent to production unchanged, exactly as it ran on your machine. Dawn is built on LangChain's [Agent Protocol](/docs/dev-server). `dawn dev` serves the Agent Protocol locally, and `dawn build` emits a `langgraph.json` you deploy to LangSmith.
...
- You need sandboxed compute as a shipped feature, not a roadmap item.
