...
**Update note (July 2026):** This June 2, 2026 post describes the 0.4 release as
it was written. For current Agent Protocol guidance, use the thread-scoped local
dev-server routes: `POST /threads`, then `POST /threads/:thread_id/runs/wait` or
...
- the local Agent Protocol stream is served at `/threads/:thread_id/runs/stream`
...
**Agent Protocol streaming is thread-scoped.** The dev server serves streaming runs at `POST /threads/:thread_id/runs/stream`, alongside blocking runs at `POST /threads/:thread_id/runs/wait`. The stream uses Dawn's local Agent Protocol event contract so dev-server tooling and deployed-runtime tooling can share the same route id and input shape.
